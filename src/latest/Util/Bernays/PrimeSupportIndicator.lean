...
noncomputable def primeSupportAF (S : ℕ → Prop) : ArithmeticFunction ℂ := by
  classical
  exact ⟨fun n => if 0 < n ∧ PrimeSupported S n then 1 else 0, by simp⟩
...
theorem primeSupportAF_isMultiplicative (S : ℕ → Prop) : (primeSupportAF S).IsMultiplicative := by
  ...
  · intro m n hm hn _
    simp only [primeSupportAF, ArithmeticFunction.coe_mk, Nat.pos_iff_ne_zero,
      primeSupported_mul_iff]
