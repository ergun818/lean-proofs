...
theorem saddleQuotient_ratio {k : ℕ} (hk : 0 < k) (c : ℕ) :
  ...
  have hYR : (lowerPrimeExponent N : ℝ) ≠ 0 := by
    exact_mod_cast (by omega : lowerPrimeExponent N ≠ 0)
  have hXR : (lowerSmoothExponent N : ℝ) ≠ 0 := by
...
theorem saddleQuotient_tendsto_atTop {k : ℕ} (hk : 0 < k) (c : ℕ) :
  ...
  tendsto_atTop_of_pos_ratio (by positivity) (saddleQuotient_ratio hk c)
    lowerSmoothParameter_tendsto_atTop
...
theorem saddleQuotient_log_cost {k : ℕ} (hk : 0 < k) (c : ℕ) :
  ...
  have hsmall := (log_scaleBase_div_log_tendsto_zero.const_mul (c : ℝ)).const_sub 1
  simp only [mul_zero, sub_zero] at hsmall
  have h := ((hsmall.mul scaleBase_saddle_relation).mul
    (log_saddleQuotient_ratio hk c)).div_const (k : ℝ)
