...
theorem residueProductMeasure_eq_uniform {ι : Type*} [Fintype ι] [DecidableEq ι]
  ...
  simp only [residueMeasure, uniformMeasure, PMF.toMeasure_apply_fintype,
    Set.indicator_singleton, PMF.uniformOfFintype_apply, ZMod.card, Finset.sum_pi_single',
    Finset.mem_univ, ↓reduceIte, Fintype.card_pi, Nat.cast_prod]
  symm
