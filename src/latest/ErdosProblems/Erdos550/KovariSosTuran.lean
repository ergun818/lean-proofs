...
theorem kst_double_count {a b : ℕ} (hb : 1 ≤ b)
  ...
  have h_double_sum :
      ∑ v : V, (G.degree v).choose a =
        ∑ S ∈ Finset.powersetCard a (Finset.univ : Finset V),
          (Finset.filter (fun v => S ⊆ G.neighborFinset v) (Finset.univ : Finset V)).card := by
  ...
  have h_term_le :
      ∀ S ∈ Finset.powersetCard a (Finset.univ : Finset V),
        (Finset.filter (fun v => S ⊆ G.neighborFinset v) (Finset.univ : Finset V)).card
          ≤ b - 1 := by
    intro S hS;
    convert! commonNbrs_card_le G hb hfree ( Finset.mem_powersetCard.mp hS |>.2 ) using 1;
    exact
        congr_arg Finset.card
          (Finset.ext fun x => by simp +decide [subset_neighborFinset_iff_mem_commonNbrs]);
  exact h_double_sum.symm ▸ le_trans ( Finset.sum_le_sum h_term_le ) ( by simp +decide [mul_comm,
    Finset.card_univ] )
...
theorem kst_star_maxDegree {b : ℕ} (hb : 1 ≤ b)
    ...
    G.degree v ≤ b - 1 := by
  classical
  convert! commonNbrs_card_le G hb hfree (S := {v}) (by simp) using 1
  exact congrArg Finset.card (by ext; simp [commonNbrs])
...
lemma kst_ratio_aux (a b : ℕ) (ha : 1 ≤ a) (ε' : ℝ) (hε' : 0 < ε') :
  ...
  -- (ii) $a \leq t$ (holds when $\epsilon' N \geq a$, i.e., $N \geq a / \epsilon'$), so that
  -- $t.choose a > 0$;
  -- (iii) $(b - 1) * (2 / \epsilon')^a \leq \epsilon' N$ (holds when $N \geq (b-1)*(2 /
  -- \epsilon')^a / \epsilon'$).
  obtain ⟨N₁, hN₁⟩ : ∃ N₁ : ℕ,
      ∀ N ≥ N₁,
        ε' * (N : ℝ) - a ≥ ε' * (N : ℝ) / 2 ∧
          a ≤ ⌈ε' * (N : ℝ)⌉₊ ∧ (b - 1) * (2 / ε') ^ a ≤ ε' * (N : ℝ) := by
    refine
        ⟨⌈(2 * a) / ε'⌉₊ + ⌈(a : ℝ) / ε'⌉₊ + ⌈((b - 1) * (2 / ε') ^ a) / ε'⌉₊ + 1, fun N hN =>
          ⟨?_, ?_, ?_⟩⟩;
    · nlinarith [Nat.le_ceil (2 * a / ε'), mul_div_cancel₀ (2 * a : ℝ) hε'.ne',
          show (N : ℝ) ≥ ⌈2 * a / ε'⌉₊ + ⌈a / ε'⌉₊ + ⌈(b - 1) * (2 / ε') ^ a / ε'⌉₊ + 1 by
            exact_mod_cast hN];
    · apply Nat.le_of_lt_succ
      rw [← @Nat.cast_lt ℝ]
      push_cast
      have hN' : (N : ℝ) ≥ ⌈(a : ℝ) / ε'⌉₊ + 1 := by
        norm_cast
        linarith
      nlinarith [Nat.le_ceil (ε' * N), Nat.le_ceil ((a : ℝ) / ε'),
        mul_div_cancel₀ (a : ℝ) hε'.ne', hN']
    · nlinarith [Nat.le_ceil (((b - 1) * (2 / ε') ^ a) / ε'),
          mul_div_cancel₀ ((b - 1 : ℝ) * (2 / ε') ^ a) hε'.ne',
          show (N : ℝ) ≥ ⌈((b - 1) * (2 / ε') ^ a) / ε'⌉₊ + 1 by exact_mod_cast by linarith];
  -- Therefore, the ratio $\frac{N.choose a}{t.choose a} \leq \left(\frac{N}{\epsilon' N -
  -- a}\right)^a \leq \left(\frac{N}{\epsilon' N / 2}\right)^a =
  -- \left(\frac{2}{\epsilon'}\right)^a$.
  have h_ratio :
      ∀ N ≥ N₁, ((N.choose a : ℝ) / ((Nat.ceil (ε' * (N : ℝ))).choose a)) ≤ (2 / ε')^a := by
    -- Using the bounds from `hN₁`, we can derive the inequality for the ratio.
    intros N hN
    have h_ratio_bound :
        ((N.choose a : ℝ) / ((Nat.ceil (ε' * (N : ℝ))).choose a)) ≤
          ((N : ℝ) ^ a / (Nat.factorial a)) / (((ε' * (N : ℝ) - a) ^ a / (Nat.factorial a))) := by
      gcongr;
      · exact
            div_pos (pow_pos (by nlinarith [hN₁ N hN, show (a : ℝ) ≥ 1 by norm_cast]) _)
              (by positivity);
      · rw [ le_div_iff₀ ( by positivity ) ];
        rw_mod_cast [ Nat.mul_comm ];
        rw [ ← Nat.descFactorial_eq_factorial_mul_choose ] ; exact Nat.descFactorial_le_pow _ _;
      · have h_ratio_bound :
          ((Nat.ceil (ε' * (N : ℝ))).descFactorial a : ℝ) ≥ ((ε' * (N : ℝ) - a) ^ a) := by
          have h_ratio_bound :
              ((Nat.ceil (ε' * (N : ℝ))).descFactorial a : ℝ) ≥
                (∏ i ∈ Finset.range a, (ε' * (N : ℝ) - i)) := by
            ...
            exact Finset.prod_le_prod
                (fun _ _ =>
                  sub_nonneg_of_le <| by
                    nlinarith [hN₁ N hN,
                      show (↑‹ℕ› : ℝ) + 1 ≤ a by norm_cast; linarith [Finset.mem_range.mp ‹_›]])
                fun i hi => by
                rw [Nat.cast_sub <| by linarith [Finset.mem_range.mp hi, hN₁ N hN]];
                exact sub_le_sub_right (Nat.le_ceil _) _;
          refine le_trans ?_ h_ratio_bound;
          exact le_trans ( by norm_num ) (Finset.prod_le_prod
              (fun _ _ =>
                sub_nonneg.mpr <| by nlinarith [hN₁ N hN, show (a : ℝ) ≥ 1 by norm_cast])
              fun i hi =>
              show (ε' * N - i : ℝ) ≥ ε' * N - a by
                linarith [show (i : ℝ) + 1 ≤ a by norm_cast; linarith [Finset.mem_range.mp hi]]);
        convert! div_le_div_of_nonneg_right h_ratio_bound (Nat.cast_nonneg (a.factorial)) using
            1;
    ...
    rw [ ← div_pow ];
    exact
        pow_le_pow_left₀ (div_nonneg (Nat.cast_nonneg _) (by nlinarith [hN₁ N hN]))
          (by rw [div_le_div_iff₀] <;> nlinarith [hN₁ N hN, show (a : ℝ) ≥ 1 by norm_cast]) _;
  obtain ⟨N, hN₁, hN₂⟩ := h_contra N₁
  specialize h_ratio N hN₁
  specialize hN₁
  specialize hN₂
  simp_all +decide only [mul_comm, div_eq_mul_inv, mul_left_comm, ge_iff_le, mul_assoc]
  ...
  · nlinarith [mul_inv_cancel₀ (ne_of_gt hε'),
        mul_le_mul_of_nonneg_left h_ratio (Nat.cast_nonneg N)]
...
theorem kovari_sos_turan (a b : ℕ) (ha : 1 ≤ a) (hb : 1 ≤ b) (ε : ℝ) (hε : 0 < ε) :
  ...
  have ht : a ≤ t := by
    ...
    rw [← @Nat.cast_lt ℝ]
    push_cast
    have hN' : (N : ℝ) ≥ N₁ + ⌈(a : ℝ) / ε'⌉₊ + ⌈2 / (3 * ε)⌉₊ + 1 := by
      exact_mod_cast hN₀
    nlinarith [Nat.le_ceil ((a : ℝ) / ε'), Nat.le_ceil (2 / (3 * ε)),
      mul_div_cancel₀ (a : ℝ) (show ε' ≠ 0 by positivity),
      mul_div_cancel₀ (2 : ℝ) (show (3 * ε : ℝ) ≠ 0 by positivity), hN',
      mul_div_cancel₀ (ε' * (N : ℝ)) (show (1 : ℝ) ≠ 0 by positivity),
      Nat.le_ceil (ε' * (N : ℝ))]
  have hm : m * (t.choose a : ℝ) ≤ (b - 1 : ℝ) * (N.choose a : ℝ) := by
    have hm :
        ∑ v ∈ Finset.univ.filter (fun v => t ≤ G.degree v), (G.degree v).choose a ≤
          (b - 1 : ℕ) * (N.choose a : ℕ) := by
      exact
            le_trans (Finset.sum_le_sum_of_subset (Finset.filter_subset _ _))
              (by simpa using! kst_double_count G hb hfree);
    norm_cast;
    exact
        le_trans
          (by
            simpa using!
              Finset.sum_le_sum
                fun x (hx : x ∈ Finset.filter (fun v => t ≤ G.degree v) Finset.univ) =>
                Nat.choose_le_choose a <| Finset.mem_filter.mp hx |>.2)
          hm
  have h_sum : ∑ v, G.degree v ≤ N * t + m * N := by
    have h_sum :
        ∑ v, G.degree v ≤
          ∑ v ∈ Finset.univ.filter (fun v => G.degree v < t), t +
            ∑ v ∈ Finset.univ.filter (fun v => t ≤ G.degree v), N := by
      rw [ Finset.sum_filter, Finset.sum_filter ];
      simpa only [← Finset.sum_add_distrib] using!
          Finset.sum_le_sum fun v _ => by
            split_ifs <;> linarith [show G.degree v < N from G.degree_lt_card_verts v];
  ...
  have h_final' : 2 * G.edgeFinset.card ≤ 2 * ε' * (N : ℝ) ^ 2 + N := by
    have h_final' :
        (N : ℝ) * t + m * N ≤
          (N : ℝ) * (ε' * (N : ℝ) + 1) +
            (b - 1 : ℝ) * (N.choose a : ℝ) / ((⌈ε' * (N : ℝ)⌉₊).choose a : ℝ) * (N : ℝ) := by
      gcongr;
      · exact Nat.ceil_lt_add_one ( by positivity ) |> le_of_lt;
      · rwa [ le_div_iff₀ ( Nat.cast_pos.mpr <| Nat.choose_pos ht ) ];
    have := hN₁ N ( by linarith ) ; norm_num at * ; nlinarith [show
          (2 * G.edgeFinset.card : ℝ) ≤ N * t + m * N by exact_mod_cast h_final] ;
  have h_final'' : G.edgeFinset.card ≤ ε * (N : ℝ) ^ 2 := by
    have h_final'' : (N : ℝ) ≥ 2 / (3 * ε) := by
      exact le_trans ( Nat.le_ceil _ ) ( mod_cast by linarith );
    rw [ge_iff_le, div_le_iff₀] at h_final'' <;>
        nlinarith [mul_div_cancel₀ (2 : ℝ) (by positivity : (3 * ε) ≠ 0)]
  exact h_final''
