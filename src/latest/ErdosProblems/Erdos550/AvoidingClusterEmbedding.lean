...
-- The induction checks weighted child degrees and avoids a separate forbidden set per cluster.
    ...
    ∃ f : α → V, Set.InjOn f S ∧
      ...
      (∀ a ∈ S,
          ∀ j ∈ (univ.filter (fun x => parent x = some a)).image clu,
            (dcap j - ε) * ((C j).card : ℝ) ≤
              (((C j).filter (fun b => G.Adj (f a) b)).card : ℝ)) ∧
          ...
          · obtain ⟨a, haS, ha_max⟩ : ∃ a ∈ S, ∀ b ∈ S, rank b ≤ rank a := by
            ...
            have himC :
                ((Finset.image f' (S.erase a) ∩ C (clu a)).card : ℝ) ≤
                  ((Finset.filter (fun x => clu x = clu a) Finset.univ).card : ℝ) :=
                by
                refine
                  le_trans
                    (Nat.cast_le.mpr
                      (Finset.card_le_card
                        (show
                          Finset.image f' (S.erase a) ∩ C (clu a) ⊆
                            Finset.image f' (Finset.filter (fun x => clu x = clu a) Finset.univ)
                          from ?_)))
                    ?_
                · intro y hy
                  ...
                  refine
                    Finset.mem_image.mpr ⟨x, Finset.mem_filter.mpr ⟨Finset.mem_univ x, ?_⟩, rfl⟩
                  by_contra hne'
            ...
            · obtain ⟨w, hwmem, hwnotU, hwgood⟩ := exists_good_unused_clusters_weighted (dtar :=
                dcap) G hε0 hε1 (hne (clu a)) (fun j hj => hne j) (fun j hj => huni (clu a) j (by
                simp +zetaDelta only [ne_eq, mem_image, mem_filter, mem_univ, true_and,
                  forall_exists_index, and_imp, forall_apply_eq_imp_iff₂, coe_erase, mem_erase,
                  Nat.cast_le] at *;
                obtain ⟨x, hx, rfl⟩ := hj
                ...
                obtain ⟨ x, hx, rfl ⟩ := hj; exact hhom x a hx |> SimpleGraph.Adj.symm)) (show
                    (((Finset.image f' (S.erase a) ∩ C (clu a)) ∪ Uav (clu a)).card : ℝ) +
                        (((univ.filter (fun x => parent x = some a)).image clu).card : ℝ) *
                          (ε * ((C (clu a)).card : ℝ)) <
                      ((C (clu a)).card : ℝ)
                    by
                    have hcardU :
                    ...
                    have hdcaple :
                      (dcap (clu a) - ε) * ((C (clu a)).card : ℝ) ≤ ((C (clu a)).card : ℝ) := by
              ...
              refine ⟨fun x => if x = a then w else f' x, ?_, ?_, ?_, ?_, ?_⟩ <;>
                simp +decide only [Set.InjOn, SetLike.mem_coe, mem_image, mem_filter,
                  mem_univ, true_and, forall_exists_index, and_imp, forall_apply_eq_imp_iff₂];
                        ...
                        (hwnotimg
                          (h.symm ▸
                            Finset.mem_image_of_mem _
                              (Finset.mem_erase_of_ne_of_mem hx₂a hx₂)));
                · exact
                      hwnotimg
                        (h ▸
            ...
            · obtain ⟨b, hb⟩ : ∃ b, parent a = some b := by
              ...
              obtain ⟨w, hw⟩ : ∃ w ∈ C (clu a),
                  G.Adj (f' b) w ∧
                    w ∉ ((Finset.image f' (S.erase a) ∩ C (clu a)) ∪ Uav (clu a)) ∧
                      ∀ j ∈ (univ.filter (fun x => parent x = some a)).image clu,
                        (dcap j - ε) * ((C j).card : ℝ) ≤
                ...
                have hU :
                    (((Finset.image f' (S.erase a) ∩ C (clu a)) ∪ Uav (clu a)).card : ℝ) +
                        (((univ.filter (fun x => parent x = some a)).image clu).card : ℝ) *
                          (ε * ((C (clu a)).card : ℝ)) <
                      (dcap (clu a) - ε) * ((C (clu a)).card : ℝ) := by
                ...
                apply good_fresh_neighbor_clusters_weighted
                  (di := dcap (clu a)) (dtar := dcap) G hε0 hε1 (hne (clu a))
              ...
              · intro x hx y hy hxy;
                ...
                · exact
                      False.elim
                        (hwnotimg
                          (hxy.symm ▸
                            Finset.mem_image_of_mem _ (Finset.mem_erase_of_ne_of_mem hy' hy)));
                · exact
                      False.elim
                        (hwnotimg
                          (hxy ▸
                            Finset.mem_image_of_mem _ (Finset.mem_erase_of_ne_of_mem hx' hx)));
                · exact
              ...
              · intro x hx; by_cases hx' : x = a;
                ...
                · rw [Function.update_of_ne hx'];
                    exact hf'.2.2.2.2 x (Finset.mem_erase_of_ne_of_mem hx' hx);
  obtain ⟨ f, hf₁, hf₂, hf₃, hf₄, hf₅ ⟩ := hkey univ fun a _ b hab => by simp;
  exact
      ⟨f, fun a b hab => hf₁ (Finset.mem_univ a) (Finset.mem_univ b) hab, fun a =>
        hf₂ a (Finset.mem_univ a), fun a => hf₅ a (Finset.mem_univ a), fun a b hab =>
        hf₄ a (Finset.mem_univ a) b hab⟩
