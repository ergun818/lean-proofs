...
lemma mem_indexed_component_iff
  ...
  · unfold componentNonseedVertices at h; simp_all +decide only [ConnectedComponent.mem_supp_iff,
      mem_filter, mem_univ, true_and, not_false_eq_true, exists_true_left] ;
...
lemma nonseedComponentOf_eq_of_adj
  ...
  classical
  convert!
        nonseedComponentOf_eq_iff T S hv hw |>.2
          (seedComponent_eq_of_adj_of_nonseed T S hvw hv hw)
...
theorem tree_tau_fine_indexed_data
  ...
  · exact
        ⟨componentNonseedVertices_pairwise_disjoint T S, fun a b hab => by
          simpa using! tauFine_indexed_edge_classification T S hab⟩
