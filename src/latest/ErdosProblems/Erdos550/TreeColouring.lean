...
exactly one.
-/
lemma IsTree.dist_root_adj_succ {α : Type*} [Finite α]
...
-/
lemma IsTree.exists_two_colouring {α : Type*} [Finite α]
  ...
  intro a b hab;
  rcases Erdos550.IsTree.dist_root_adj_succ T hT r a b hab with h | h <;> simp_all +decide;
  all_goals grind
...
lemma IsTree.exists_rooted_two_colouring {α : Type*} [Finite α]
    (T : SimpleGraph α) (hT : T.IsTree)
    (parent : α → Option α)
