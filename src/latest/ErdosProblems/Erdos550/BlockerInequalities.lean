...
-- The separation proof tracks the common neighbourhood bounds across every reservoir.
theorem aset_separation (q : ℕ) (_hq : 2 ≤ q) (m : Fin (q + 1) → ℕ)
  ...
  · intro i j
    ...
    · exact fun _ => Disjoint.mono_left hS_subX (hdisjX _)
    · exact fun _ => (Disjoint.mono_left hS_subX (hdisjX _)).symm
  ...
  · intro j U hU hU_card
    ...
    · obtain ⟨i₀, hi₀⟩ : ∃ i₀ : Fin q, j = Fin.succ i₀ := by
      ...
      -- The number of vertices in $W i₀$ that are not adjacent to some $u \in U \setminus S$ is
      -- at most $(U \setminus S).card * ζ₀ * (W i₀).card$.
      have h_not_adj :
          ((W i₀).filter (fun v => ∃ u ∈ U \ S, ¬Gr.Adj v u)).card ≤
            (U \ S).card * (δ / (2 * (∑ i, m i) + 2)) * (W i₀).card := by
        have h_not_adj :
            ((W i₀).filter (fun v => ∃ u ∈ U \ S, ¬Gr.Adj v u)).card ≤
              ∑ u ∈ U \ S, ((W i₀).filter (fun v => ¬Gr.Adj u v)).card := by
          have h_not_adj :
              ((W i₀).filter (fun v => ∃ u ∈ U \ S, ¬Gr.Adj v u)) ⊆
                Finset.biUnion (U \ S) (fun u => (W i₀).filter (fun v => ¬Gr.Adj u v)) := by
            intro v hv
            rcases Finset.mem_filter.mp hv with ⟨hvW, u, hu, huv⟩
            exact Finset.mem_biUnion.mpr
              ⟨u, hu, Finset.mem_filter.mpr ⟨hvW, fun h => huv h.symm⟩⟩
        ...
        refine
            le_trans
              (Finset.sum_le_sum fun x hx =>
                show
                  (_ : ℝ) ≤
                    δ / (2 * ∑ i : Fin (q + 1), (m i : ℝ) + 2) * (W i₀ |> Finset.card : ℝ)
                  from ?_)
              ?_;
        · obtain ⟨i, hi, hi'⟩ := hU x (Finset.mem_sdiff.mp hx |>.1);
            simp_all +decide only [ne_eq, Nat.cast_sum, Order.add_one_le_iff, not_nonempty_iff,
              Fin.cons, Fin.succ_ne_zero, not_false_eq_true, mem_sdiff];
            cases i using Fin.inductionOn <;> simp_all +decide only [Fin.cases, Fin.succ_pos,
              Fin.induction_zero, Fin.succ_lt_succ_iff, Fin.induction_succ];
            exact hblue _ _ (ne_of_lt hi) _ hi';
        · norm_num [ mul_assoc ];
      -- Therefore, the number of vertices in $W i₀$ that are adjacent to all vertices in $U$ is
      -- at least $(δ - M*ζ₀) * (W i₀).card$.
      have h_adj :
          ((W i₀).filter (fun v => ∀ u ∈ U, Gr.Adj v u)).card ≥
            (δ - (∑ i, m i) * (δ / (2 * (∑ i, m i) + 2))) * (W i₀).card := by
        have h_adj :
            ((W i₀).filter (fun v => ∀ u ∈ U, Gr.Adj v u)).card ≥
              ((commonRedNbhd Gr S (W i₀)).filter (fun v => ∀ u ∈ U \ S, Gr.Adj v u)).card := by
          apply Finset.card_mono
          intro v hv
          rcases Finset.mem_filter.mp hv with ⟨hvCommon, hvU⟩
          simp only [commonRedNbhd, Finset.mem_filter] at hvCommon
          refine Finset.mem_filter.mpr ⟨hvCommon.1, ?_⟩
          intro u hu
          by_cases huS : u ∈ S
          · exact (hvCommon.2 u huS).symm
          · exact hvU u (Finset.mem_sdiff.mpr ⟨hu, huS⟩)
        have h_adj :
            ((commonRedNbhd Gr S (W i₀)).filter (fun v => ∀ u ∈ U \ S, Gr.Adj v u)).card ≥
              (commonRedNbhd Gr S (W i₀)).card -
                ((W i₀).filter (fun v => ∃ u ∈ U \ S, ¬Gr.Adj v u)).card := by
        ...
        have h_adj :
            ((commonRedNbhd Gr S (W i₀)).filter (fun v => ∀ u ∈ U \ S, Gr.Adj v u)).card ≥
              (δ * (W i₀).card) - ((U \ S).card * (δ / (2 * (∑ i, m i) + 2)) * (W i₀).card) := by
        ...
        · rw [ sub_mul ];
          ...
          grind +extAll;
        · exact_mod_cast
              ‹#({v ∈ W i₀ | ∀ u ∈ U, Gr.Adj v u}) ≥
                  #({v ∈ commonRedNbhd Gr S (W i₀) | ∀ u ∈ U \ S, Gr.Adj v u})›;
      ...
      have h_final : ((W i₀).filter (fun v => ∀ u ∈ U, Gr.Adj v u)).card ≥ (∑ i, m i) :=
          by
          have h_final :
            (δ - (∑ i, m i) * (δ / (2 * (∑ i, m i) + 2))) * (W i₀).card ≥
              (δ / 2) * (W i₀).card :=
            by field_simp; exact mul_le_mul_of_nonneg_left (by linarith) (Nat.cast_nonneg _);
          exact_mod_cast h_half.trans (h_final.trans h_adj);
      exact
          le_trans (Finset.single_le_sum (fun a _ => Nat.zero_le (m a)) (Finset.mem_univ _))
            (h_final.trans (Finset.card_mono <| by aesop_cat))
...
-/
lemma bip_sides_of_contained {V : Type*} (Gr : SimpleGraph V)
    (a b : ℕ) (Y : Finset V)
...
-/
lemma dense_minus_slack {V : Type*} (Gr : SimpleGraph V)
  ...
  norm_cast;
  have h_card_filter :
      (D.filter (fun v => ∃ w ∈ Bad, ¬Gr.Adj v w)).card ≤
        ∑ w ∈ Bad, (Wj.filter (fun v => ¬Gr.Adj v w)).card := by
    refine le_trans ?_ ( Finset.card_biUnion_le );
    exact Finset.card_le_card fun x hx => by aesop;
  have hpartition := Finset.card_filter_add_card_filter_not (s := D)
    (fun v => ∃ w ∈ Bad, ¬Gr.Adj v w)
  simp only [not_exists, not_and, not_not] at hpartition
  omega
    ...
    m (k.succ.succ) ≤ ((W (i.succAbove k)).filter
      (fun v => (∀ s ∈ S0, Gr.Adj v s) ∧ (∀ s ∈ S1, Gr.Adj v s) ∧
        ∀ u ∈ U, Gr.Adj v u)).card := by
            refine
              le_trans ?_
                (Finset.card_le_card (s :=
                  commonRedNbhd Gr E (W (Fin.succAbove i k)) |>
                    Finset.filter fun v =>
                      ∀ w ∈ (S0 ∪ S1 ∪ U).filter (fun w => w ∉ E), Gr.Adj v w)
                  ?_)
            · have h_dense_minus_slack :
                (Finset.card (commonRedNbhd Gr E (W (Fin.succAbove i k))) : ℝ) -
                    (∑ w ∈ (S0 ∪ S1 ∪ U).filter (fun w => w ∉ E),
                        ((W (Fin.succAbove i k)).filter (fun v => ¬Gr.Adj v w)).card :
                      ℝ) ≥
                  (δ / 2) * (W (Fin.succAbove i k)).card :=
                by
                have h_dense_minus_slack :
                  (∑ w ∈ (S0 ∪ S1 ∪ U).filter (fun w => w ∉ E),
                        ((W (Fin.succAbove i k)).filter (fun v => ¬Gr.Adj v w)).card :
                      ℝ) ≤
                    ((S0 ∪ S1 ∪ U).filter (fun w => w ∉ E)).card *
                        (δ / (4 * ∑ x, (m x : ℝ) + 4)) *
                      (W (Fin.succAbove i k)).card :=
                  by
                  refine
                    le_trans
                      (Finset.sum_le_sum fun w hw =>
                        show
                          (_ : ℝ) ≤
                            (δ / (4 * ∑ x : Fin (q' + 2 + 1), (m x : ℝ) + 4)) *
                              (W (Fin.succAbove i k) |> Finset.card : ℝ)
                          from ?_)
                      ?_;
                  · by_cases hwW : w ∈ W i;
                    · convert! hblue i (Fin.succAbove i k) (by simp +decide []) w hwW using 1;
                      simp +decide only [adj_comm];
                    · obtain ⟨k', hk'⟩ : ∃ k' < k, w ∈ W (Fin.succAbove i k') := by grind;
                      convert! hblue (Fin.succAbove i k') (Fin.succAbove i k) _ w hk'.2 using 1;
                      · simp +decide only [adj_comm];
                      · exact fun h => hk'.1.ne (by simpa [Fin.succAbove_ne] using! h);
                  · simp +decide only [union_assoc, sum_const, nsmul_eq_mul, mul_assoc,
                      Std.le_refl];
                have h_card_filter :
                  ((S0 ∪ S1 ∪ U).filter (fun w => w ∉ E)).card ≤ 2 * ∑ x, (m x : ℝ) :=
                  by
                  have h_card_filter :
                    ((S0 ∪ S1 ∪ U).filter (fun w => w ∉ E)).card ≤ (S0 ∪ S1 ∪ U).card := by
                    exact Finset.card_filter_le _ _;
                  have h_card_filter : (S0 ∪ S1 ∪ U).card ≤ m 0 + m 1 + ∑ x, (m x : ℝ) := by
                    exact_mod_cast
                      le_trans (Finset.card_union_le _ _)
                        (add_le_add
                          (Finset.card_union_le _ _ |> le_trans <|
                            add_le_add hS0card.le hS1card.le)
                          hUcard);
                  refine le_trans (Nat.cast_le.mpr ‹_›) (h_card_filter.trans ?_);
                  rw [Finset.sum_eq_add_sum_sdiff_singleton_of_mem (Finset.mem_univ 0)];
                  linarith [show (m 1 : ℝ) ≤ ∑ x ∈ Finset.univ \ {0}, (m x : ℝ) from
                      mod_cast
                        Finset.single_le_sum (fun x _ => Nat.zero_le (m x)) (by simp +decide)];
                have := h_contra (Fin.succAbove i k) (by simp +decide [Fin.succAbove_ne]);
                refine le_trans ?_ (sub_le_sub_left h_dense_minus_slack _); field_simp;
                nlinarith [show 0 ≤ δ * (Finset.card (W (Fin.succAbove i k)) : ℝ) by positivity,
                  show
                    0 ≤
                      δ * (Finset.card (W (Fin.succAbove i k)) : ℝ) *
                        (∑ x : Fin (q' + 2 + 1), (m x : ℝ))
                    by positivity];
              have h_card_filter :
                (Finset.card
                      (Finset.filter
                        (fun v => ∀ w ∈ (S0 ∪ S1 ∪ U).filter (fun w => w ∉ E), Gr.Adj v w)
                        (commonRedNbhd Gr E (W (Fin.succAbove i k)))) :
                    ℝ) ≥
                  (δ / 2) * (W (Fin.succAbove i k)).card :=
                by
                ...
                convert!
                  dense_minus_slack Gr (W (Fin.succAbove i k))
                    (commonRedNbhd Gr E (W (Fin.succAbove i k)))
                    ((S0 ∪ S1 ∪ U).filter (fun w => w ∉ E)) _ using
                  1;
                · convert! rfl;
                · grind +locals;
              ...
              exact_mod_cast
                (by
                  linarith [show (m (Fin.succ (Fin.succ k)) : ℝ) ≤ ∑ x : Fin (q' + 2 + 1), m x
                      from
                      mod_cast
                        Finset.single_le_sum (fun a _ => Nat.zero_le (m a))
                          (Finset.mem_univ _)] :
                  (m (Fin.succ (Fin.succ k)) : ℝ) ≤
                    Finset.card
                      (Finset.filter (fun v => ∀ w ∈ {w ∈ S0 ∪ S1 ∪ U | w ∉ E}, Gr.Adj v w)
                        (commonRedNbhd Gr E (W (Fin.succAbove i k)))));
            · intro v hv
              rcases Finset.mem_filter.mp hv with ⟨hvCommon, hvU⟩
              simp only [commonRedNbhd, Finset.mem_filter] at hvCommon
              have hadj : ∀ w ∈ S0 ∪ S1 ∪ U, Gr.Adj v w := by
                intro w hw
                by_cases hwE : w ∈ E
                · exact (hvCommon.2 w hwE).symm
                · exact hvU w (Finset.mem_filter.mpr ⟨hw, hwE⟩)
              exact Finset.mem_filter.mpr
                ⟨hvCommon.1,
                  fun w hw => hadj w (Finset.mem_union_left U (Finset.mem_union_left S1 hw)),
                  fun w hw => hadj w (Finset.mem_union_left U (Finset.mem_union_right S0 hw)),
                  fun w hw => hadj w (Finset.mem_union_right _ hw)⟩
      ...
      ∀ (i : Fin q) (E : Finset V), IsObstruction Gr (m 0) (m 1) (W i) X E →
        ∃ j : Fin q, j ≠ i ∧
          ((commonRedNbhd Gr E (W j)).card : ℝ) ≤ δ * (W j).card := by
              refine ⟨δ / (4 * (∑ i, (m i : ℝ)) + 4), ?_, ?_⟩
              · positivity
              · refine ⟨⌈2 * (∑ i, (m i : ℝ)) / δ⌉₊ + 1, ?_⟩;
                intro V _ _ Gr _ W X hdisjW hdisjX hWcard hblue hGr i E hE;
                by_cases h_contra :
                  ∀ j ≠ i, δ * (W j).card < ((commonRedNbhd Gr E (W j)).card : ℝ);
                · obtain ⟨S0, S1, hS0sub, hS1sub, hS0card, hS1card, hdisj, hHred⟩ :
                    ∃ S0 S1 : Finset V,
                      S0 ⊆ W i ∪ E ∧
                        S1 ⊆ W i ∪ E ∧
                          S0.card = m 0 ∧
                            S1.card = m 1 ∧ Disjoint S0 S1 ∧ ∀ u ∈ S0, ∀ v ∈ S1, Gr.Adj u v :=
                    by apply bip_sides_of_contained; exact hE.2.2.1;
                  obtain ⟨q', rfl⟩ : ∃ q', q = q' + 2 := by
                    exact ⟨q - 2, by rw [Nat.sub_add_cancel hq]⟩;
                  have :=
                    red_F_from_red_H_ordered Gr (q' + 1) m S0 S1 hS0card hS1card hdisj hHred
                      (fun k => W (i.succAbove k)) ?_ ?_ ?_ ?_;
                  · contradiction;
                  · exact fun a b hab => hdisjW _ _ <| by simpa [Fin.succAbove_ne] using! hab;
                  · intro j; specialize hdisjW i (i.succAbove j);
                    simp_all +decide only [Finset.disjoint_left, le_add_iff_nonneg_left,
                      zero_le, Order.add_one_le_iff, ne_eq, not_nonempty_iff, Fin.ne_succAbove,
                      not_false_eq_true, forall_const]; intro a ha; specialize hS0sub ha;
                    simp_all +decide only [subset_iff, mem_union];
                    cases hS0sub <;> [exact hdisjW ‹_›; exact fun h => hdisjX _ (hE.2.1 ‹_›) h];
                  · intro j; specialize hdisjW i (i.succAbove j);
                    simp_all +decide only [Finset.disjoint_left, le_add_iff_nonneg_left,
                      zero_le, Order.add_one_le_iff, ne_eq, not_nonempty_iff, Fin.ne_succAbove,
                      not_false_eq_true, forall_const]; intro a ha; specialize hS1sub ha;
                    simp_all +decide only [subset_iff, mem_union];
                    cases hS1sub <;> simp_all +decide only [IsObstruction, SetLike.coe_sort_coe,
                      not_nonempty_iff, not_false_eq_true];
                    exact fun h => hdisjX _ (hE.2.1 ‹_›) h;
                  · intro k U hUstruct hUcard;
                    convert!
                      obstruction_richness Gr q' m δ hδ W E i S0 S1 hS0sub hS1sub hS0card
                        hS1card hdisjW ?_ ?_ ?_ ?_ k U hUstruct hUcard using
                      1;
                    · grind;
                    · exact fun a =>
                        Finset.disjoint_left.mpr fun x hx hx' =>
                          Finset.disjoint_left.mp (hdisjX a) (hE.2.1 hx) hx';
                    · exact hWcard;
...
theorem red_profile_inequality (q : ℕ) (hq : 2 ≤ q) (δ : ℝ) (hδ : 0 < δ) :
  ...
  intro VT _ _ V _ _ T _ Gr _ W X hT hcardT hdisjW hdisjX hsize hmindeg hnoblueT x hx; contrapose!
      hnoblueT;
  ...
  · intro i;
    ...
    · exact
          mul_le_mul_of_nonneg_right (by linarith [min_le_left κ δ0, min_le_right κ δ0])
            (Nat.cast_nonneg _);
  · -- By definition of $redCount$ and $blueCount$, we have $redCount i + blueCount i = |W i|$.
    have h_red_blue :
        ∀ i,
          ((Gr.neighborFinset x ∩ W i).card : ℝ) + ((Grᶜ.neighborFinset x ∩ W i).card : ℝ) =
            (W i).card :=
        by
        intro i; norm_cast; rw [← Finset.card_union_of_disjoint];
        · congr 1
          ext y
          simp only [mem_union, mem_inter, mem_neighborFinset, compl_adj, ne_eq]
          constructor
          · exact fun h => h.elim And.right And.right
          · intro hy
            by_cases hxy : Gr.Adj x y
            · exact Or.inl ⟨hxy, hy⟩
            · refine Or.inr ⟨⟨?_, hxy⟩, hy⟩
              intro hxy
              subst y
              exact Finset.disjoint_left.mp (hdisjX i) hx hy
        · simp +contextual only [neighborFinset, Finset.disjoint_left, mem_inter,
            Set.mem_toFinset, mem_neighborSet, compl_adj, ne_eq, not_true_eq_false, and_false,
            and_true, not_false_eq_true, implies_true];
    -- By definition of $redCount$ and $blueCount$, we have $\sum_{i} \frac{redCount i}{|W i|} < q
    -- - 1 - \delta$.
    have h_sum_red : ∑ i, ((Gr.neighborFinset x ∩ W i).card : ℝ) / (W i).card < q - 1 - δ := by
      convert! hnoblueT using 1;
    -- By definition of $redCount$ and $blueCount$, we have $\sum_{i} \frac{blueCount i}{|W i|} >
    -- 1 + \delta$.
    have h_sum_blue : ∑ i, ((Grᶜ.neighborFinset x ∩ W i).card : ℝ) / (W i).card > 1 + δ :=
        by
        have h_sum_blue :
          ∑ i, ((Grᶜ.neighborFinset x ∩ W i).card : ℝ) / (W i).card =
            ∑ i, (1 - ((Gr.neighborFinset x ∩ W i).card : ℝ) / (W i).card) :=
          by
          refine Finset.sum_congr rfl fun i _ => ?_;
          rw [one_sub_div] <;> norm_num [h_red_blue i];
          · rw [← h_red_blue i, add_sub_cancel_left];
          · intro h; specialize hsize i; norm_num [h] at hsize;
            linarith [show (Fintype.card VT : ℝ) ≥ 2 by norm_cast];
        norm_num [h_sum_blue]; linarith;
    refine
        le_trans ?_
          (Finset.sum_le_sum fun i _ =>
            show
              (Grᶜ.neighborFinset x ∩ W i |> Finset.card : ℝ) ≥
                (Grᶜ.neighborFinset x ∩ W i |> Finset.card : ℝ) / (W i |> Finset.card : ℝ) *
                  (Fintype.card VT : ℝ)
              from ?_);
    · rw [← Finset.sum_mul _ _ _];
        exact mul_le_mul_of_nonneg_right h_sum_blue.le <| Nat.cast_nonneg _;
    · rw [div_mul_eq_mul_div, ge_iff_le, div_le_iff₀] <;> norm_cast at * <;> nlinarith [ hsize i ];
  · intro i v hv; specialize hmindeg i v hv;
      nlinarith [hsize i, show (0 : ℝ) ≤ min κ δ0 by positivity, min_le_left κ δ0,
        min_le_right κ δ0];
