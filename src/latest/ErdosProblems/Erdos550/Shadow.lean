...
lemma limit_productsum_bound (q : ℕ) (a : ℕ)
  ...
  have h_sum_zero :
      ∫⁻ ω, ∑' x : X, (if compatCount A ω x = q then 1 else 0) ∂(Measure.pi μ) ≤ ↑(a - 1) := by
    refine le_trans (MeasureTheory.lintegral_mono_ae
      (g := fun _ => (↑(a - 1) : ENNReal)) ?_) ?_
...
lemma limit_impurity_summable (q : ℕ) (hq : 2 ≤ q) (a : ℕ) (_ha : 1 ≤ a)
  ...
  refine Summable.of_nonneg_of_le
    (f := fun x => 2 ^ (q - 1) * (∏ i, (μ i (A i x)).toReal) +
      if (⨅ i, (μ i (A i x)).toReal) > 1 / 2 then 1 else 0) ?_ ?_ ?_
  · exact fun x => Real.iInf_nonneg fun i => ENNReal.toReal_nonneg;
  · intro x;
    split_ifs;
    · refine
        le_add_of_nonneg_of_le
          (mul_nonneg (pow_nonneg zero_le_two _)
            (Finset.prod_nonneg fun _ _ => ENNReal.toReal_nonneg))
          ?_;
      exact
          le_trans (ciInf_le (Finite.bddBelow_range _) ⟨0, by linarith⟩)
            (ENNReal.toReal_le_of_le_ofReal zero_le_one <| by
              exact
                le_trans (MeasureTheory.measure_mono <| Set.subset_univ _) <| by simp +decide);
    · -- Let $h x$ be a coordinate achieving the min, so $e x = (μ (h x) (A (h x) x)).toReal$.
      obtain ⟨i, hi⟩ : ∃ i, (μ i (A i x)).toReal = ⨅ i, (μ i (A i x)).toReal := by
        exact
              (IsCompact.sInf_mem (Set.finite_range _ |> Set.Finite.isCompact) <|
                Set.nonempty_of_mem <| Set.mem_range_self <| ⟨0, by linarith⟩);
      -- By (N1), $\sum_{j \neq i} (1 - \rho_j) \leq e x$ where $\rho_j = (\mu j (A j x)).toReal$.
      have h_sum :
          ∑ j ∈ Finset.univ.erase i, (1 - (μ j (A j x)).toReal) ≤ (μ i (A i x)).toReal :=
          by
          have h_sum :
            ∑ j ∈ Finset.univ.erase i, (μ j (A j x)).toReal ≥
              (q - 1 : ℝ) - (μ i (A i x)).toReal :=
            by
            have := hN1 x; rw [← ENNReal.toReal_sum] at *;
            · rw [← Finset.sum_erase_add _ _ (Finset.mem_univ i), add_comm] at this;
              rw [ge_iff_le, tsub_le_iff_right]; convert! ENNReal.toReal_mono _ this using 1;
              · rw [ENNReal.toReal_sub_of_le] <;> norm_num; linarith;
              · rw [add_comm, ENNReal.toReal_add] <;> norm_num;
              · exact
                  ne_of_lt
                    (ENNReal.add_lt_top.mpr
                      ⟨MeasureTheory.measure_lt_top _ _,
                        ENNReal.sum_lt_top.mpr fun j hj => MeasureTheory.measure_lt_top _ _⟩);
            · exact fun _ _ => MeasureTheory.measure_ne_top _ _;
          simp +zetaDelta only [sum_sub_distrib, sum_const, mem_univ, card_erase_of_mem,
            card_univ, Fintype.card_fin, nsmul_eq_mul, mul_one, sum_erase_eq_sub,
            tsub_le_iff_right, add_sub_cancel, ge_iff_le] at *;
          rw [Nat.cast_pred] <;> linarith;
      -- Since $\rho_j \geq 1 - e x$ for every $j \neq i$, we have $\prod_{j \neq i} \rho_j \geq
      -- (1 - e x)^{q-1}$.
      have h_prod :
          ∏ j ∈ Finset.univ.erase i, (μ j (A j x)).toReal ≥
            (1 - (μ i (A i x)).toReal) ^ (q - 1) :=
          by
          have h_prod :
            ∀ j ∈ Finset.univ.erase i, (μ j (A j x)).toReal ≥ 1 - (μ i (A i x)).toReal :=
            by
            intro j hj; contrapose! h_sum;
            refine
              lt_of_lt_of_le ?_
                (Finset.single_le_sum
                  (fun a _ => sub_nonneg.2 <| ENNReal.toReal_le_of_le_ofReal zero_le_one <| ?_)
                  hj);
            · linarith;
          ...
          exact fun _ _ =>
            hi.symm ▸
              le_trans (ciInf_le (Finite.bddBelow_range _) i)
        ...
        refine
            le_trans ?_
              (mul_le_mul_of_nonneg_left
                  (mul_le_mul_of_nonneg_left h_prod <| ENNReal.toReal_nonneg) <|
                by positivity);
  ...
  · refine Summable.add ?_ ?_;
    · have h_prod_summable : Summable (fun x => ∏ i, (μ i (A i x)).toReal) := by
        simpa only [ENNReal.toReal_prod] using
          (ENNReal.summable_toReal (f := fun x : X => ∏ i, μ i (A i x))
            (ne_of_lt (lt_of_le_of_lt (limit_productsum_bound q a μ A hA hN2)
              ENNReal.coe_lt_top)))
    ...
    · have h_finite : Set.Finite {x : X | (⨅ i, (μ i (A i x)).toReal) > 1 / 2} := by
            ...
            simpa only [ENNReal.toReal_prod] using
              (ENNReal.summable_toReal (f := fun x : X => ∏ i, μ i (A i x))
                (ne_of_lt (lt_of_le_of_lt (limit_productsum_bound q a μ A hA hN2)
                  ENNReal.coe_lt_top)))
          convert!
              h_finite.tendsto_cofinite_zero.eventually
                (gt_mem_nhds <| show 0 < (1 / 2 : ℝ) ^ q by positivity) using
              1 ; aesop;
        refine h_finite.subset fun x hx => ?_;
        refine
              le_trans ?_
                (Finset.prod_le_prod ?_ fun i _ =>
                  show (μ i (A i x) |> ENNReal.toReal) ≥ 1 / 2 from ?_) <;>
            norm_num;
      ...
      exact summable_of_ne_finset_zero (s := h_finite.toFinset)
        (fun x hx => if_neg <| by simpa only [Set.Finite.mem_toFinset, Set.mem_ofPred_eq] using hx)
...
lemma exists_pushforward_relabel (q t : ℕ) (s : ℕ → ℕ)
  ...
  · intro i S hS
    ...
    congr 1
    change ((ρ i).toMeasure.map F) {σ | ∀ x ∈ S, σ x = true}
        = (ρ i).toMeasure {σ | ∀ x ∈ S.image s, σ x = true}
    rw [Measure.map_apply hFmeas hmeas]
    congr 1
    ext σ
    simp only [Set.mem_preimage, Set.mem_ofPred_eq, hF, Finset.mem_image]
    constructor
  ...
  · intro i ℓ hℓ
    ...
    have hpre : F ⁻¹' {σ : ℕ → Bool | ∀ x ∈ ({ℓ} : Finset ℕ), σ x = true} = ∅ := by
      ext σ
      simp only [Set.mem_preimage, hset, Set.mem_ofPred_eq, Set.mem_empty_iff_false, iff_false]
      intro h
      rw [hF] at h
      simp only [if_neg (by omega : ¬ ℓ < t)] at h
      exact Bool.false_ne_true h
    change (((ρ i).toMeasure.map F) {σ | ∀ x ∈ ({ℓ} : Finset ℕ), σ x = true}).toReal = 0
...
lemma exists_cimp_eq (q : ℕ) (hq : 1 ≤ q)
  ...
  obtain ⟨i, _, hi⟩ := Finset.exists_min_image Finset.univ
    (fun i => cdens q ρ i {x}) ⟨⟨0, hq⟩, Finset.mem_univ _⟩
  refine ⟨i, le_antisymm (cimp_le q ρ i x) ?_⟩
  let : Nonempty (Fin q) := ⟨i⟩
...
lemma cdens_inter_drop (q : ℕ) (ρ : Fin q → ProbabilityMeasure (ℕ → Bool))
    ...
    cdens q ρ i P ≤ cdens q ρ i (P ∪ Q) + ∑ y ∈ Q, (1 - cdens q ρ i {y}) := by
        unfold cdens;
        have h_subset :
          (ρ i).toMeasure {σ : ℕ → Bool | ∀ x ∈ P, σ x = true} ≤
            (ρ i).toMeasure {σ : ℕ → Bool | ∀ x ∈ P ∪ Q, σ x = true} +
              ∑ y ∈ Q, (ρ i).toMeasure {σ : ℕ → Bool | σ y = false} :=
          by
          refine
            le_trans (MeasureTheory.measure_mono ?_) (MeasureTheory.measure_union_le _ _) |>
                le_trans <|
              add_le_add (le_refl _) (MeasureTheory.measure_biUnion_finset_le _ _);
        ...
        · rw [ENNReal.toReal_add, ENNReal.toReal_sum];
          · congr! 2;
            rw [show {σ : ℕ → Bool | σ _ = false} = (Set.univ \ {σ : ℕ → Bool | σ _ = true}) by
                  ext; aesop,
...
lemma home_complement_bound (q : ℕ) (ρ : Fin q → ProbabilityMeasure (ℕ → Bool))
  ...
  · fin_cases i₀;
  · have h_sum :
      ∑ i ∈ Finset.univ.erase i₀, (1 - cdens (q + 1) ρ i { x }) ≤ ε + cdens (q + 1) ρ i₀ { x } := by
      simp_all +decide [ Finset.sum_sub_distrib ];
      linarith;
    linarith [Finset.single_le_sum (fun i _ => sub_nonneg.mpr (cdens_le_one (q + 1) ρ i { x }))
          (Finset.mem_erase_of_ne_of_mem hj (Finset.mem_univ j))]
...
lemma ultrafilter_nat_dichotomy (U : Ultrafilter ℕ) (f : ℕ → ℕ) :
  ...
  obtain ⟨M, hM⟩ : ∃ M, {n | f n < M} ∈ U := by
    simp_all +decide only [tendsto_atTop, not_forall, not_eventually, not_le,
        Ultrafilter.frequently_iff_eventually];
    exact ⟨ h.2.choose, h.2.choose_spec ⟩;
  -- Since ${n | f n < M} \in U$, we can partition it into finitely many sets ${n | f n = v}$ for
  -- $v < M$.
  have h_partition : {n | f n < M} = ⋃ v ∈ Finset.range M, {n | f n = v} := by
    ext n; simp [Finset.mem_range];
  have h_finite_union :
      ∀ {S : Finset ℕ}, (⋃ v ∈ S, {n | f n = v}) ∈ U → ∃ v ∈ S, {n | f n = v} ∈ U := by
...
lemma moving_tail (q : ℕ) (ρ : ℕ → Fin q → ProbabilityMeasure (ℕ → Bool))
  ...
  filter_upwards [hyesc.eventually_ge_atTop L, hyrange,
      hconv L |> fun h => h.eventually (gt_mem_nhds hL)] with n hn₁ hn₂ hn₃ using by
      simpa [abs_of_nonneg (cimp_nonneg q (ρ n) _)] using!
        lt_of_le_of_lt (hmono n L (y n) hn₁ hn₂) hn₃;
        ...
        cimp q ρ (s ℓ₂) ≤ cimp q ρ (s ℓ₁)) := by
            obtain ⟨l, hl⟩ :
              ∃ l : List ℕ,
                l.length = V.card ∧
                  (∀ x ∈ l, x ∈ V) ∧
                    (∀ x ∈ V, x ∈ l) ∧ List.Pairwise (fun x y => cimp q ρ y ≤ cimp q ρ x) l :=
              by
              obtain ⟨l, hl⟩ :
                ∃ l : List ℕ,
                  l.Perm V.toList ∧ List.Pairwise (fun x y => cimp q ρ y ≤ cimp q ρ x) l :=
                by
                have h_merge_sort :
                  ∀ (l : List ℕ) (cmp : ℕ → ℕ → Bool),
                    (∀ x y, cmp x y = true ↔ cimp q ρ y ≤ cimp q ρ x) →
                      List.Pairwise (fun x y => cimp q ρ y ≤ cimp q ρ x)
                        (List.mergeSort l (fun x y => cmp x y)) :=
                  by
                  intros l cmp hcmp
                  have h_merge_sort :
                    List.Pairwise (fun x y => cmp x y = true)
                      (List.mergeSort l (fun x y => cmp x y)) :=
                    by
                    apply_rules [List.pairwise_mergeSort]; · grind;
                    · grind;
              ...
              exact
                ⟨l, by simpa using! hl.1.length_eq, fun x hx => by simpa using! hl.1.subset hx,
                  fun x hx => by simpa using! hl.1.symm.subset <| by simpa using! hx, hl.2⟩;
            refine ⟨fun ℓ => l[ℓ]?.getD 0, ?_, ?_, ?_, ?_⟩ <;>
              simp_all +decide only [Set.InjOn, Set.mem_Iio, getElem?_pos, Option.getD_some,
                List.getElem_mem, implies_true];
            · have h_unique : List.Nodup l :=
                by
                have h_card : l.toFinset.card = V.card := by
                  exact congr_arg Finset.card (Finset.ext fun x => by aesop);
                rw [List.nodup_iff_injective_get]; intro i j hij;
                have :=
                  Finset.card_image_iff.mp
                    (by aesop :
                      Finset.card (Finset.image l.get Finset.univ) = Finset.card Finset.univ);
                aesop;
            ...
            · intro ℓ₁ ℓ₂ h₁ h₂; have := List.pairwise_iff_get.mp hl.2.2.2;
              simp_all +decide only [List.get_eq_getElem, ge_iff_le];
              by_cases h₃ : ℓ₁ < l.length <;> by_cases h₄ : ℓ₂ < l.length <;>
                simp_all +decide only [getElem?_pos, Option.getD_some, not_true_eq_false,
                  not_lt, getElem?_neg, Option.getD_none];
              · cases lt_or_eq_of_le h₁ <;> [exact this ⟨ℓ₁, by linarith⟩ ⟨ℓ₂, by linarith⟩ ‹_›;
                  aesop];
              · linarith
...
lemma ae_noncanonical_finite (q : ℕ) (hq : 2 ≤ q) (a : ℕ) (ha : 1 ≤ a)
    ...
    ∀ᵐ ω ∂(Measure.pi μ),
      {x : X | ω (h x) ∈ A (h x) x ∨ ∃ j, j ≠ h x ∧ ω j ∉ A j x}.Finite := by
          have h_summable : Summable (fun x : X => (μ (h x) (A (h x) x)).toReal) :=
            by
            convert! limit_impurity_summable q hq a ha μ A hA hN1 hN2 using 1; ext x;
            refine le_antisymm ?_ ?_;
            · refine le_csInf ?_ ?_; · exact ⟨_, ⟨h x, rfl⟩⟩;
              · rintro _ ⟨i, rfl⟩;
                exact ENNReal.toReal_mono (MeasureTheory.measure_ne_top _ _) (hhome x i);
            · exact ciInf_le (Finite.bddBelow_range _) _;
          have h_summable_measure :
            ∑' x, (Measure.pi μ {ω | ω (h x) ∈ A (h x) x ∨ ∃ j, j ≠ h x ∧ ω j ∉ A j x}) ≠ ⊤ :=
            by
            have h_summable_measure :
              ∀ x,
                (Measure.pi μ {ω | ω (h x) ∈ A (h x) x ∨ ∃ j, j ≠ h x ∧ ω j ∉ A j x}) ≤
                  2 * ENNReal.ofReal ((μ (h x) (A (h x) x)).toReal) :=
              by
              intro x
              have hprob :
                (Measure.pi μ)
                    {ω : ∀ i, Ω i | ω (h x) ∈ A (h x) x ∨ ∃ j, j ≠ h x ∧ ω j ∉ A j x} ≤
                  (μ (h x)) (A (h x) x) + ∑ j ∈ Finset.univ.erase (h x), (1 - (μ j) (A j x)) :=
                by
                refine
                  le_trans
                    (MeasureTheory.measure_mono (t :=
                      {ω : ∀ i, Ω i | ω (h x) ∈ A (h x) x} ∪
                        ⋃ j ∈ Finset.univ.erase (h x), {ω : ∀ i, Ω i | ω j ∉ A j x})
                      ?_)
                    ?_
                ...
                · refine le_trans (MeasureTheory.measure_union_le _ _) (add_le_add ?_ ?_);
                  ...
                  · refine le_trans (MeasureTheory.measure_biUnion_finset_le _ _) ?_;
                    ...
                    have h_measure_compl :
                      (Measure.pi μ) {ω : ∀ i, Ω i | ω i ∉ A i x} = (μ i) (A i x)ᶜ := by
                      convert! pi_marginal μ i (A i x)ᶜ (hA i x |> MeasurableSet.compl) using 1;
                    rw [h_measure_compl, MeasureTheory.measure_compl] <;> aesop;
              have h_bound :
                ∑ j ∈ Finset.univ.erase (h x), (1 - (μ j) (A j x)) ≤
                  ENNReal.ofReal ((μ (h x) (A (h x) x)).toReal) :=
                by
                have h_bound :
                  ∑ j ∈ Finset.univ.erase (h x), (1 - (μ j) (A j x)) ≤
                    (q - 1 : ENNReal) - ∑ j ∈ Finset.univ.erase (h x), (μ j) (A j x) :=
                  by
                  rw [ENNReal.sub_eq_of_eq_add]; · simp +zetaDelta at *;
                  · rw [← Finset.sum_add_distrib,
                      Finset.sum_congr rfl fun _ _ =>
                        tsub_add_cancel_of_le <|
                          MeasureTheory.measure_mono (Set.subset_univ _) |> le_trans <| by
                            simp +decide];
                    simp +decide [Finset.card_erase_of_mem <| Finset.mem_univ <| h x];
                refine le_trans h_bound ?_;
                simp_all +decide only [ne_eq, measure_ne_top, not_false_eq_true,
                  ENNReal.ofReal_toReal, tsub_le_iff_right];
                convert! hN1 x using 1
                exact
                  congrArg (fun z : ENNReal => z + 1)
                    (Finset.add_sum_erase Finset.univ (fun j => μ j (A j x))
                      (Finset.mem_univ (h x)))
              refine le_trans hprob ?_;
              convert! add_le_add_left h_bound (μ (h x) (A (h x) x)) using 1
              · ring
          ...
          exact ae_finite_setOfPred_mem h_summable_measure
...
-- The measure estimates and finite exceptional sets are combined with the canonical condition.
lemma exists_good_canonical_outcome (q : ℕ) (hq : 2 ≤ q) (a : ℕ) (ha : 1 ≤ a)
    ...
    ∃ ω : ∀ i, Ω i,
      ...
      {x : X | ω (hh x) ∈ A (hh x) x ∨ ∃ j', j' ≠ hh x ∧ ω j' ∉ A j' x}.Finite := by
          have h_pos : 0 < (Measure.pi μ) {ω | Ucard A ω ≤ ((a : ℕ) - 1 : ℕ)} :=
            by
            have h_nonempty : (Measure.pi μ) {ω | Ucard A ω ≥ a} ≤ (a - 1 : ℝ≥0∞) / a :=
              by
              have := @MeasureTheory.meas_ge_le_lintegral_div;
              refine le_trans (this ?_ ?_ ?_) ?_;
              · refine Measurable.aemeasurable ?_; refine Measurable.tsum ?_; intro x;
                exact
                  Measurable.ite
              ...
              · gcongr; convert! Erdos550.lintegral_Ucard_le μ A hq hA hN1 hN2 using 1;
                cases a <;> aesop;
            have h_nonempty :
              (Measure.pi μ) {ω | Ucard A ω ≤ (a - 1 : ℕ)} ≥ 1 - (a - 1 : ℝ≥0∞) / a :=
              by
              refine le_trans (tsub_le_tsub_left h_nonempty _) ?_; rw [tsub_le_iff_right];
              refine le_trans ?_ (MeasureTheory.measure_union_le _ _);
              refine le_trans ?_ (MeasureTheory.measure_mono (s := Set.univ) ?_)
              · simp
              · grind +suggestions
            refine lt_of_lt_of_le ?_ h_nonempty; rcases a with (_ | _ | a) <;> norm_num at *;
            rw [ENNReal.div_lt_iff] <;> norm_cast <;> norm_num;
          have h_inter_pos :
            0 <
              (Measure.pi μ)
                ({ω | Ucard A ω ≤ ((a : ℕ) - 1 : ℕ)} ∩
                    {ω | ∀ e, ∃ x ∈ E e, ω (j e) ∉ A (j e) x} ∩
                  {ω | {x | ω (hh x) ∈ A (hh x) x ∨ ∃ j', j' ≠ hh x ∧ ω j' ∉ A j' x}.Finite}) :=
            by
            have h_inter_pos :
              (Measure.pi μ)
                  ({ω | Ucard A ω ≤ ((a : ℕ) - 1 : ℕ)} \
                    ({ω | ∀ e, ∃ x ∈ E e, ω (j e) ∉ A (j e) x} ∩
                      {ω |
                        {x |
                            ω (hh x) ∈ A (hh x) x ∨ ∃ j', j' ≠ hh x ∧ ω j' ∉ A j' x}.Finite})) =
                0 :=
              by
              refine
                MeasureTheory.measure_mono_null (t :=
                  {ω | ¬∀ e, ∃ x ∈ E e, ω (j e) ∉ A (j e) x} ∪
                    {ω | ¬{x | ω (hh x) ∈ A (hh x) x ∨ ∃ j', j' ≠ hh x ∧ ω j' ∉ A j' x}.Finite})
                  ?_ ?_
              · grind;
              · refine MeasureTheory.measure_union_null ?_ ?_;
                ...
                · exact
                    MeasureTheory.measure_mono_null (fun x hx => by aesop)
                      (ae_noncanonical_finite q hq a ha μ A hA hN1 hN2 hh hhome);
            rw [MeasureTheory.measure_eq_measure_of_null_sdiff (fun _ h => h.1.1) ?_]
            · exact h_pos
            · convert h_inter_pos using 2
              ext ω
          ...
          · have := hω.1.1; contrapose! this;
            simp +decide only [ENNReal.natCast_sub, Nat.cast_one, Set.mem_ofPred_eq, not_le];
            refine
              lt_of_lt_of_le ?_
                (ENNReal.tsum_le_tsum fun x =>
                  show
                    (if compatCount A ω x ≤ q - 2 then 1 else 0 : ENNReal) ≥
                      if x ∈ {x | compatCount A ω x ≤ q - 2} then 1 else 0
                    from ?_);
            · rw [ENNReal.tsum_eq_iSup_sum];
              refine
                lt_of_lt_of_le ?_
                  (le_ciSup ?_ (this.exists_subset_card_eq (a + 1) |> Classical.choose));
              · rw [Finset.sum_congr rfl fun x hx =>
                    if_pos <|
                      Classical.choose_spec (this.exists_subset_card_eq (a + 1)) |>.1 hx];
                simp +decide only [sum_const,
                  Classical.choose_spec (this.exists_subset_card_eq (a + 1)) |>.2, nsmul_eq_mul,
                  Nat.cast_add, Nat.cast_one, mul_one];
                exact_mod_cast Nat.lt_succ_of_le (Nat.pred_le _);
              · simp +decide [];
            · simp +decide [Set.mem_ofPred_eq];
          · by_cases h : Set.Finite {x : X | compatCount A ω x ≤ q - 2} <;>
              simp_all +decide only [tsub_le_iff_right, ENNReal.natCast_sub, Nat.cast_one,
                ne_eq, Set.mem_inter_iff, Set.mem_ofPred_eq, Set.ncard_def, Set.not_finite,
                Set.Infinite.encard_eq, ENat.toNat_top, zero_le];
            have h_card :
              Ucard A ω = ∑ x ∈ h.toFinset, if compatCount A ω x ≤ q - 2 then 1 else 0 := by
              rw [Ucard]; rw [tsum_eq_sum]; simp +contextual [h.mem_toFinset];
            simp_all +decide only [sum_boole, ge_iff_le]; convert! hω.1.1 using 1;
            rw [Set.Finite.encard_eq_coe_toFinset_card h]; norm_cast;
            rw [Finset.filter_true_of_mem fun x hx => by simpa using! h.mem_toFinset.mp hx];
            norm_cast
...
lemma exists_ultrafilter_eventually_eq {α : Type*} [Finite α] [Nonempty α]
  ...
  exact U.eventually_exists_iff.mp (Filter.Eventually.of_forall fun n => ⟨g n, rfl⟩)
...
lemma cimp_tendsto {q : ℕ} (ρ : ℕ → Fin q → ProbabilityMeasure (ℕ → Bool))
  ...
  rcases q with (_ | q) <;>
      simp_all +decide only [IsEmpty.forall_iff, cimp, Real.iInf_of_isEmpty,
  ...
  have h_cont_inf : Continuous (fun v : Fin (q + 1) → ℝ => ⨅ i, v i) := by
    ...
    exact ⟨ v 0, Finset.mem_image_of_mem _ ( Finset.mem_univ _ ) ⟩) := by
      ...
      · exact fun w hw => by
            rcases
              Finset.mem_image.mp
                (Finset.min'_mem (Finset.image v Finset.univ) (by solve_by_elim)) with
              ⟨i, _, hi⟩;
            exact ⟨i, hi ▸ hw⟩;
    simp +decide only [this, min', id_eq, inf'_image, Function.comp_apply];
    refine continuous_iff_continuousAt.mpr ?_;
    intro v; exact (by
    refine tendsto_order.2 ⟨ ?_, ?_ ⟩ <;> intro x hx <;> simp_all +decide only [inf'_lt_iff,
      mem_univ, true_and, lt_inf'_iff, forall_const, eventually_all];
...
lemma shadow_N1 (q : ℕ) (ρ' : ℕ → Fin q → ProbabilityMeasure (ℕ → Bool)) (t : ℕ → ℕ)
    ...
    (q : ℝ≥0∞) - 1 ≤ ∑ i, (L i).toMeasure {σ | σ (x : ℕ) = true} := by
        have h_persistent :
          ∀ i,
            (L i).toMeasure {σ : ℕ → Bool | σ x = true} = ENNReal.ofReal (cdens q L i {↑x}) :=
          by
          intro i
          simp [cdens];
        have h_persistent : (q : ℝ) - 1 ≤ ∑ i, cdens q L i {↑x} :=
          by
          have h_persistent :
            Filter.Tendsto (fun n => ∑ i, cdens q (ρ' n) i {↑x}) (↑U)
              (𝓝 (∑ i, cdens q L i {↑x})) :=
            by exact tendsto_finsetSum _ fun i _ => hcd i _;
          have h_persistent :
            ∀ᶠ n in (U : Filter ℕ), (q : ℝ) - 1 - ε n ≤ ∑ i, cdens q (ρ' n) i {↑x} := by
            exact Filter.mem_of_superset x.2 fun n hn => hA1' n _ hn;
          have :=
            le_of_tendsto_of_tendsto (Filter.Tendsto.sub tendsto_const_nhds hεU) ‹_›
              h_persistent;
          aesop;
        convert! ENNReal.ofReal_le_ofReal h_persistent using 1;
        rw [ENNReal.ofReal_sum_of_nonneg]; · aesop;
...
lemma shadow_N2 (q : ℕ) (a : ℕ) (ρ' : ℕ → Fin q → ProbabilityMeasure (ℕ → Bool))
  ...
  obtain ⟨i, hi⟩ : ∃ i,
      {n | cdens q (ρ' n) i (S.map (Function.Embedding.subtype (XsetU t U))) ≤ ε n} ∈ U := by
    have h_union :
        ⋃ i : Fin q,
            {n | cdens q (ρ' n) i (S.map (Function.Embedding.subtype (XsetU t U))) ≤ ε n} ∈
          U := by
      refine Filter.mem_of_superset (x := ⋂ x ∈ S, { n | ( x : ℕ ) < t n }) ?_ ?_
  ...
  -- By the ultrafilter property, we have that `cdens q L i (S.map (Function.Embedding.subtype
  -- (XsetU t U))) ≤ 0`.
...
lemma shadow_finish (q : ℕ) (hq : 2 ≤ q) (a : ℕ) (ha : 1 ≤ a) (rStar : ℕ)
  ...
  have hvXs : ∀ p ∈ R, v p ∈ Xs := by
    intro p hp
    rw [hXs, XsetU, Set.mem_ofPred_eq]
    ...
    have hzero : ∀ ℓ ∉ Xs, cimp q L ℓ = 0 := by
      ...
      have hUc : {n | t n ≤ ℓ} ∈ (U : Filter ℕ) := by
        have hnot : {n | ℓ < t n} ∉ U := by
          intro hmem
          rw [hXs, XsetU, Set.mem_ofPred_eq] at hℓ
        ...
        intro n hn; simp only [Set.mem_compl_iff, Set.mem_ofPred_eq, not_lt] at hn
        exact hn
