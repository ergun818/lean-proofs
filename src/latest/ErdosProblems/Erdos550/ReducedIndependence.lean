...
-/
lemma exists_reg_clique {ι : Type*} [Finite ι]
  ...
  have h_clique_free : (induce (↑I) Rg).CliqueFree (q + 1) := by
    intro s hs;
    contrapose! h_contra;
    use Finset.image Subtype.val s;
    simp_all +decide only [SetLike.coe_sort_coe, mem_image, Subtype.exists, exists_and_right,
      exists_eq_right, ne_eq, forall_exists_index] ;
    exact ⟨ Finset.image_subset_iff.mpr fun x hx => x.2, by rw [
      Finset.card_image_of_injective _ Subtype.coe_injective, hs.2], fun a ha ha' b hb hb'
            hab =>
          by simpa [hab] using! hs.1 ha' hb' <| by aesop ⟩;
  contrapose! h;
...
-/
lemma no_large_sparse_regular_family {W : Type} [Finite W] (F : SimpleGraph W)
  ...
  obtain ⟨ε₀, hε₀, m₀, h⟩ :=
      Erdos550.exists_red_multipartite_of_sparse F q hcol (1 - d) (by linarith);
  refine ⟨ ε₀, hε₀, m₀, ?_ ⟩;
  intro V _ _ G _ hG ι _ _ C Rg _ I hI hC hdisj hreg hsparse
  obtain ⟨s, hs⟩ := Erdos550.exists_reg_clique Rg I q hI
  obtain ⟨P, hP⟩ : ∃ P : Fin (q + 1) → ι,
      (∀ i, P i ∈ s) ∧ (∀ i j, i ≠ j → P i ≠ P j) ∧ (∀ i j, i ≠ j → Rg.Adj (P i) (P j)) := by
    obtain ⟨e, he⟩ : ∃ e : Fin (q + 1) ≃ s, True := by
      exact ⟨ Fintype.equivOfCardEq ( by simp +decide [ hs.2.1 ] ), trivial ⟩;
    exact
        ⟨fun i => e i |>.1, fun i => e i |>.2, fun i j hij => fun h =>
          hij <| e.injective <| Subtype.ext h, fun i j hij =>
          hs.2.2 _ (e i |>.2) _ (e j |>.2) <| fun h => hij <| e.injective <| Subtype.ext h⟩;
  exact
      hG <|
        h G (fun i => C (P i)) (fun i => hC _ <| hs.1 <| hP.1 i)
          (fun i j hij => hdisj _ (hs.1 <| hP.1 i) _ (hs.1 <| hP.1 j) <| hP.2.1 i j hij)
          (fun i j hij => hreg _ _ <| hP.2.2 i j hij)
          (fun i j hij => by
            linarith [hsparse _ (hs.1 <| hP.1 i) _ (hs.1 <| hP.1 j) <| hP.2.2 i j hij])
...
-/
lemma turan_lt_induce_of_few_irregular {ι : Type*} [Finite ι] [DecidableEq ι]
  ...
  let := Fintype.ofFinite ι
  classical
  contrapose! hthr; have := Fintype.card_coe 𝒜; simp_all +decide only [SetLike.coe_sort_coe,
    Fintype.card_coe, Nat.choose_two_right, ge_iff_le] ;
  have h_edge_count :
      ((induce (↑𝒜) Rg).edgeFinset.card + (induce (↑𝒜) Rg)ᶜ.edgeFinset.card) =
        (Nat.choose (Finset.card 𝒜) 2) := by
    rw [ ← Finset.card_union_of_disjoint ];
    · have h_card_edges :
        (induce (𝒜 : Set ι) Rg).edgeFinset ∪ (induce (𝒜 : Set ι) Rg)ᶜ.edgeFinset =
          (⊤ : SimpleGraph { x // x ∈ 𝒜 }).edgeFinset := by
        ext ⟨x, y⟩;
        simp only [SetLike.coe_sort_coe, mem_union, mem_edgeFinset, mem_edgeSet, comap_adj,
          compl_adj, ne_eq, edgeFinset_top, Set.toFinset_compl, mem_compl, Set.mem_toFinset,
          Sym2.mem_diagSet, Sym2.mk_isDiag_iff];
        by_cases h : x = y <;> simp +decide only [h, SimpleGraph.irrefl, not_true_eq_false,
          not_false_eq_true, and_true, or_self, true_and, iff_true];
        exact em _;
      rw [ h_card_edges, SimpleGraph.card_edgeFinset_top_eq_card_choose_two ];
      rw [ Fintype.card_coe ];
    · exact disjoint_edgeFinset.mpr disjoint_compl_right
  cases k : Finset.card 𝒜 <;> simp_all +decide only [SetLike.coe_sort_coe, Nat.choose_two_right,
    zero_tsub, mul_zero, Nat.zero_div, Nat.add_eq_zero_iff, card_eq_zero, edgeFinset_eq_empty,
    compl_eq_bot, zero_le, add_tsub_cancel_right, ge_iff_le] ; omega
...
lemma exists_dense_regular_pair_in_family {W : Type} [Finite W] (F : SimpleGraph W)
    (q : ℕ) (hcol : F.Colorable (q + 1)) (d : ℝ) (hd1 : d < 1) :
    ∃ ε₀ : ℝ, 0 < ε₀ ∧ ∃ m₀ : ℕ, ∀ {V : Type} [Fintype V] [DecidableEq V]
      (G : SimpleGraph V) [DecidableRel G.Adj], ¬ (F ⊑ Gᶜ) →
      ∀ {ι : Type} [Fintype ι] [DecidableEq ι] (C : ι → Finset V)
