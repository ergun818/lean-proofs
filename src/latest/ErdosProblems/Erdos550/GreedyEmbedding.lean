  ...
  have h_hom :
      ∃ f : (i : Fin k) × Fin (m i) → V,
        Function.Injective f ∧
          ∀ i j, i ≠ j → ∀ x ∈ Finset.univ, ∀ y ∈ Finset.univ, Gr.Adj (f ⟨i, x⟩) (f ⟨j, y⟩) := by
    -- By definition of `S`, we can construct the desired homomorphism.
    obtain ⟨f, hf⟩ : ∃ (f : (i : Fin k) → Fin (m i) → V),
        (∀ i, Function.Injective (f i)) ∧ (∀ i, ∀ x ∈ Finset.univ, f i x ∈ S i) := by
      have h_equiv : ∀ i, Nonempty (Fin (m i) ≃ S i) := by
        exact fun i => ⟨ Fintype.equivOfCardEq <| by simp +decide [ hcard i ] ⟩;
      exact
          ⟨fun i x => (h_equiv i).some x |>.1, fun i => fun x y hxy => by
            simpa using! (h_equiv i).some.injective (Subtype.ext hxy), fun i x _ =>
            (h_equiv i).some x |>.2⟩;
    refine ⟨ fun ⟨ i, x ⟩ => f i x, ?_, ?_ ⟩ <;> simp_all +decide only [ne_eq, mem_univ,
      forall_const];
...
-/
theorem exists_cross_complete_sets (Gr : SimpleGraph V)
  ...
  | succ k ih =>
    obtain ⟨S', hS'⟩ :=
        ih (fun i ↦ m i.castSucc) (fun i ↦ C i.castSucc)
          (fun i j hij ↦ hdisj _ _ (by simpa [Fin.ext_iff] using! hij))
          (fun j U hU ↦
            le_trans
                (hrich _ _ <| by
                  simpa [Fin.sum_univ_castSucc] using! hU.trans (by simp +decide)) <|
              by simp +decide);
    -- Let `U = (Finset.univ : Finset (Fin k)).biUnion S'`, the union of all chosen sets.
    set U := Finset.biUnion Finset.univ S' with hU_def;
    -- Apply `hrich` to get a set `F = filter (fun v => ∀ u ∈ U, Gr.Adj v u) (C (Fin.last k))` of
    -- card `≥ m (Fin.last k)`.
    obtain ⟨F, hF⟩ : ∃ F : Finset V,
        F ⊆ C (Fin.last k) ∧ F.card = m (Fin.last k) ∧ ∀ v ∈ F, ∀ u ∈ U, Gr.Adj v u := by
      have hF :
          m (Fin.last k) ≤
            (Finset.filter (fun v => ∀ u ∈ U, Gr.Adj v u) (C (Fin.last k))).card := by
        apply hrich;
        refine le_trans ( Finset.card_biUnion_le ) ?_;
        rw [ Fin.sum_univ_castSucc ] ; simp +decide [ hS'.1 ];
      obtain ⟨ F, hF ⟩ := Finset.exists_subset_card_eq hF;
      exact
          ⟨F, Finset.Subset.trans hF.1 (Finset.filter_subset _ _), hF.2, fun v hv u hu =>
            Finset.mem_filter.mp (hF.1 hv) |>.2 u hu⟩;
    ...
    · intro i j hij
      cases i using Fin.lastCases <;> cases j using Fin.lastCases <;>
        simp +decide only [Fin.snoc_castSucc, Fin.snoc_last] at hij ⊢;
      · exact (hij rfl).elim;
    ...
    · intro i j hij x hx y hy;
      by_cases hi : i.val < k <;> by_cases hj : j.val < k <;> simp +decide only [Fin.snoc,
        ↓reduceDIte, Fin.castSucc_castLT, cast_eq, hi, hj] at hx hy ⊢;
      · exact hS'.2.2.2 _ _ ( by simpa [ Fin.ext_iff ] using! hij ) _ hx _ hy;
      · exact
            hF.2.2 _ hy _ (Finset.mem_biUnion.mpr ⟨_, Finset.mem_univ _, hx⟩) |>
...
theorem greedy_multipartite_embedding (Gr : SimpleGraph V)
    [DecidableRel Gr.Adj] (k : ℕ) (m : Fin k → ℕ) (C : Fin k → Finset V)
    (hdisj : ∀ i j, i ≠ j → Disjoint (C i) (C j))
    (hrich : ∀ (j : Fin k) (U : Finset V), U.card ≤ ∑ i, m i →
        m j ≤ ((C j).filter (fun v => ∀ u ∈ U, Gr.Adj v u)).card) :
...
-/
theorem exists_cross_complete_sets_ordered (Gr : SimpleGraph V)
    [DecidableRel Gr.Adj] (k : ℕ) (m : Fin k → ℕ) (C : Fin k → Finset V)
    (hdisj : ∀ i j, i ≠ j → Disjoint (C i) (C j))
  ...
  | succ k ih =>
    obtain ⟨S', hS'⟩ :=
        ih (fun i => m i.castSucc) (fun i => C i.castSucc)
          ...
          (fun j U hU hU' =>
            hrich _ _
                (fun u hu => by
                  obtain ⟨i, hi, hi'⟩ := hU u hu
                  exact ⟨i.castSucc, by simpa [Fin.castSucc_lt_last] using! hi, hi'⟩) <|
              hU'.trans <| by simp +decide [Fin.sum_univ_castSucc]);
    -- Let $U = \bigcup_{i : Fin k} S' i$. Then $|U| \leq \sum_{i : Fin k} |S' i| = \sum_{i : Fin
    -- k} m (Fin.castSucc i) \leq \sum_{i : Fin (k + 1)} m i$.
    ...
    -- By the richness hypothesis, there exists a subset $F \subseteq C (Fin.last k)$ of size $m
    -- (Fin.last k)$ such that every vertex in $F$ is adjacent to every vertex in $U$.
    obtain ⟨F, hF⟩ : ∃ F : Finset V,
        F ⊆ C (Fin.last k) ∧ F.card = m (Fin.last k) ∧ ∀ v ∈ F, ∀ u ∈ U, Gr.Adj v u := by
      ...
      · obtain ⟨ F, hF ⟩ := Finset.exists_subset_card_eq this;
        exact
            ⟨F, Finset.Subset.trans hF.1 (Finset.filter_subset _ _), hF.2, fun v hv u hu =>
              Finset.mem_filter.mp (hF.1 hv) |>.2 u hu⟩;
      · simp +zetaDelta only [ne_eq, mem_biUnion, mem_univ, true_and, forall_exists_index] at *;
    ...
    · intro i j hij;
      simp +zetaDelta only [ne_eq, mem_biUnion, mem_univ, true_and, forall_exists_index] at *;
      cases i using Fin.lastCases <;> cases j using Fin.lastCases <;>
        simp +decide only [not_true_eq_false, Fin.snoc_last, Fin.snoc_castSucc,
          Fin.castSucc_ne_last, not_false_eq_true, Fin.castSucc_inj] at hij ⊢;
...
`K_{m₀,…,m_{k-1}}`. -/
theorem greedy_multipartite_embedding_ordered (Gr : SimpleGraph V)
    [DecidableRel Gr.Adj] (k : ℕ) (m : Fin k → ℕ) (C : Fin k → Finset V)
    (hdisj : ∀ i j, i ≠ j → Disjoint (C i) (C j))
    (hrich : ∀ (j : Fin k) (U : Finset V),
        (∀ u ∈ U, ∃ i, i < j ∧ u ∈ C i) → U.card ≤ ∑ i, m i →
