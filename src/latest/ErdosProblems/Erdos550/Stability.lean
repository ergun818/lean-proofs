...
theorem sum_pairs_identity {q : ℕ} (s : Fin q → ℝ) :
  ...
  | zero => simp
  | succ q ih =>
    simp +decide only [Fin.sum_univ_succ];
        convert!
            congr_arg (fun x : ℝ => x + 2 * s 0 * ∑ i : Fin q, s (Fin.succ i))
              (ih fun i => s (Fin.succ i)) using
            1 <;>
          ring_nf;
...
theorem deviation_identity {q : ℕ} (hq : 0 < q) (s : Fin q → ℝ) :
    ...
    (N ^ 2 * (q - 1) / q) / 2
      ...
      = (∑ i, (s i - N / q) ^ 2) / 2 := by
  have := sum_pairs_identity s; norm_num [sub_sq, Finset.sum_add_distrib, Finset.mul_sum _ _ _,
    Finset.sum_mul] at this ⊢; ring_nf at this ⊢;
...
-- The cardinality calculation expands residue classes and ordered pairs in the Turán graph.
theorem turanEdges_partite_step (q d N : ℕ) (hq : 1 ≤ q) (hd : d ≤ N) :
    turanEdges q d + d * (N - d) ≤ turanEdges (q + 1) N := by
  -- Let `G := SimpleGraph.comap part (⊤ : SimpleGraph (Fin (q+1)))`, so `G.Adj x y ↔ part x ≠
  -- part y`.
  set G : SimpleGraph (Fin N) :=
      SimpleGraph.comap
        (fun x =>
          if x.val < d then Fin.castSucc ⟨x.val % q, Nat.mod_lt _ (by omega)⟩ else Fin.last q)
        (⊤ : SimpleGraph (Fin (q + 1)));
  ...
  have h_non_edges :
      (Finset.univ.filter (fun p : Fin N × Fin N => p.1 < p.2 ∧ ¬(G.Adj p.1 p.2))).card =
        ∑ c : Fin (q + 1),
          (Finset.filter
                (fun x : Fin N =>
                  (if x.val < d then Fin.castSucc ⟨x.val % q, Nat.mod_lt _ (by omega)⟩
                    else Fin.last q) =
                    c)
                Finset.univ).card.choose
            2 :=
      by
      have h_non_edges :
        (Finset.univ.filter (fun p : Fin N × Fin N => p.1 < p.2 ∧ ¬(G.Adj p.1 p.2))).card =
          ∑ c : Fin (q + 1),
            (Finset.filter
                (fun p : Fin N × Fin N =>
                  p.1 < p.2 ∧
                    (if p.1.val < d then Fin.castSucc ⟨p.1.val % q, Nat.mod_lt _ (by omega)⟩
                        else Fin.last q) =
                        c ∧
                      (if p.2.val < d then Fin.castSucc ⟨p.2.val % q, Nat.mod_lt _ (by omega)⟩
                        else Fin.last q) =
                        c)
                Finset.univ).card :=
        by
        rw [← Finset.card_biUnion]; · congr with p; aesop;
        · exact fun x _ y _ hxy => Finset.disjoint_left.mpr fun p hp hp' => hxy <| by aesop;
      rw [h_non_edges]; refine Finset.sum_congr rfl fun c _ => ?_;
      rw [← Finset.card_powersetCard];
      refine Finset.card_bij (fun p hp => { p.1, p.2 }) ?_ ?_ ?_; · grind;
      · simp +contextual [Finset.Subset.antisymm_iff, Finset.subset_iff]; grind;
      · simp +decide only [Fin.castSucc_mk, mem_powersetCard, mem_filter, mem_univ, true_and,
          exists_prop, Prod.exists, and_imp];
        intro b hb hcard; obtain ⟨x, y, hxy, rfl⟩ := Finset.card_eq_two.mp hcard;
        cases lt_or_gt_of_ne hxy <;>
          [exact
            ⟨x, y,
              ⟨‹_›, by simpa using! hb (by simp +decide), by simpa using! hb (by simp +decide)⟩,
              rfl⟩;
          exact
            ⟨y, x,
              ⟨‹_›, by simpa using! hb (by simp +decide), by simpa using! hb (by simp +decide)⟩,
              by simp +decide [*, Finset.pair_comm]⟩];
  -- The fibre of `Fin.last q` has size `N - d`; the fibres of `Fin.castSucc i` (`i : Fin q`)
  -- partition `[0,d)` by residue mod `q`, matching exactly the colour classes of `turanGraph d
  -- q`.
  have h_fibres :
      ∑ c : Fin (q + 1),
          (Finset.filter
        ...
        (N - d).choose 2 +
          (Finset.univ.filter
              (fun p : Fin d × Fin d => p.1 < p.2 ∧ ¬(turanGraph d q).Adj p.1 p.2)).card :=
      by
      have h_fibres :
        ∀ i : Fin q,
          (Finset.filter
                (fun x : Fin N =>
                  (if x.val < d then Fin.castSucc ⟨x.val % q, Nat.mod_lt _ (by omega)⟩
                    ...
                    Fin.castSucc i)
                Finset.univ).card =
            (Finset.filter (fun x : Fin d => x.val % q = i.val) Finset.univ).card :=
        by
        intro i
        have hpred :
          ∀ x : Fin N,
            (if x.val < d then Fin.castSucc ⟨x.val % q, Nat.mod_lt _ (by omega)⟩
                ...
                Fin.castSucc i ↔
              x.val < d ∧ x.val % q = i.val :=
          by
          intro x
          by_cases hx : x.val < d <;> simp [hx, Fin.ext_iff]; omega
        simp_rw [hpred]
        refine
          Finset.card_bij (fun x hx => (⟨x.val, (Finset.mem_filter.mp hx).2.1⟩ : Fin d)) ?_ ?_
            ?_
        · intro x hx
          exact Finset.mem_filter.mpr ⟨Finset.mem_univ _, (Finset.mem_filter.mp hx).2.2⟩
        · intro a ha b hb hab
          exact Fin.ext (congrArg (fun x : Fin d => x.val) hab)
        · intro b hb
          refine ⟨⟨b.val, b.isLt.trans_le hd⟩, ?_, rfl⟩
          exact Finset.mem_filter.mpr ⟨Finset.mem_univ _, b.isLt, (Finset.mem_filter.mp hb).2⟩
      have h_fibres_sum :
        ∑ i : Fin q,
            (Finset.filter (fun x : Fin d => x.val % q = i.val) Finset.univ).card.choose 2 =
          (Finset.univ.filter
              (fun p : Fin d × Fin d => p.1 < p.2 ∧ ¬(turanGraph d q).Adj p.1 p.2)).card :=
        by
        have h_fibres_sum :
          ∀ i : Fin q,
            (Finset.filter (fun x : Fin d => x.val % q = i.val) Finset.univ).card.choose 2 =
              (Finset.univ.filter
                  (fun p : Fin d × Fin d =>
                    p.1 < p.2 ∧ p.1.val % q = i.val ∧ p.2.val % q = i.val)).card :=
          by
          intro i
          have h_fibres_sum :
            (Finset.filter (fun x : Fin d => x.val % q = i.val) Finset.univ).card.choose 2 =
              (Finset.powersetCard 2
                  (Finset.filter (fun x : Fin d => x.val % q = i.val) Finset.univ)).card :=
            by rw [Finset.card_powersetCard];
          convert! h_fibres_sum using 1;
          refine Finset.card_bij (fun p hp => { p.1, p.2 }) ?_ ?_ ?_ <;>
            simp +decide only [mem_powersetCard, mem_filter, mem_univ, true_and, exists_prop,
              Prod.exists, and_imp];
          · grind;
          · simp +contextual [Finset.Subset.antisymm_iff, Finset.subset_iff]; grind;
          · intro b hb hb'; rw [Finset.card_eq_two] at hb'; obtain ⟨a, b, hab, rfl⟩ := hb';
            simp_all +decide only [Fin.castSucc_mk, card_powersetCard, ne_eq, subset_iff,
              mem_insert, mem_singleton, mem_filter, mem_univ, true_and, forall_eq_or_imp,
              forall_eq];
            cases lt_or_gt_of_ne hab <;> [exact ⟨a, b, ⟨‹_›, hb⟩, rfl⟩;
              exact ⟨b, a, ⟨‹_›, hb.2, hb.1⟩, by rw [Finset.pair_comm]⟩];
        simp_all +decide only [Fin.castSucc_mk, turanGraph, ne_eq, Decidable.not_not]
        rw [← Finset.card_biUnion]
        · congr with p; simp +decide only [mem_biUnion, mem_univ, mem_filter, true_and,
            exists_and_left, and_congr_right_iff];
          exact fun _ =>
            ⟨fun ⟨x, hx₁, hx₂⟩ => hx₁.trans hx₂.symm, fun hx =>
              ⟨⟨p.1 % q, Nat.mod_lt _ hq⟩, rfl, hx.symm⟩⟩;
        · exact fun i _ j _ hij =>
            Finset.disjoint_left.mpr fun x hx₁ hx₂ => hij <| Fin.ext <| by aesop;
      ...
      rw [show
          (Finset.univ.filter fun x : Fin N =>
              (if (x : ℕ) < d then Fin.castSucc ⟨(x : ℕ) % q, Nat.mod_lt _ (by linarith)⟩
                else Fin.last q) =
                Fin.last q) =
            Finset.univ \ Finset.univ.filter fun x : Fin N => (x : ℕ) < d
          from ?_];
      · have hcard : (Finset.univ.filter (fun x : Fin N => x.val < d)).card = d := by
          calc
            _ = (Finset.univ : Finset (Fin d)).card :=
              by
              refine
                Finset.card_bij (fun x hx => (⟨x.val, (Finset.mem_filter.mp hx).2⟩ : Fin d)) ?_
                  ?_ ?_
              · exact fun _ _ => Finset.mem_univ _
              · exact fun a _ b _ hab => Fin.ext (congrArg (fun x : Fin d => x.val) hab)
              · intro b hb
                exact
                  ⟨⟨b.val, b.isLt.trans_le hd⟩,
                    Finset.mem_filter.mpr ⟨Finset.mem_univ _, b.isLt⟩, rfl⟩
            _ = d := by simp
        rw [Finset.card_sdiff_of_subset (Finset.subset_univ _), Finset.card_univ,
          Fintype.card_fin, hcard]
      · grind
  -- Therefore, the number of edges of `G` is `N.choose 2 - (N - d).choose 2 - (Finset.univ.filter
  -- (fun p : Fin d × Fin d => p.1 < p.2 ∧ ¬(turanGraph d q).Adj p.1 p.2)).card`.
  have h_edges_G :
      G.edgeFinset.card =
        (N.choose 2) -
      ...
      have h_edges_G :
        ...
        have h_edges_G :
          Finset.filter (fun p : Fin N × Fin N => p.1 < p.2) Finset.univ =
            Finset.filter (fun p : Fin N × Fin N => p.1 < p.2 ∧ G.Adj p.1 p.2) Finset.univ ∪
              Finset.filter (fun p : Fin N × Fin N => p.1 < p.2 ∧ ¬G.Adj p.1 p.2) Finset.univ :=
          by grind;
        rw [h_edges_G, Finset.card_union_of_disjoint];
        · rw [show
              G.edgeFinset =
                Finset.image (fun p : Fin N × Fin N => s(p.1, p.2))
                  (Finset.filter (fun p : Fin N × Fin N => p.1 < p.2 ∧ G.Adj p.1 p.2)
                    Finset.univ)
              from ?_]; · rw [Finset.card_image_of_injOn]; simp +decide [Set.InjOn]; grind;
          · ext ⟨x, y⟩; simp [G]; grind;
      ...
      exact
        congrArg₂ _
          (Nat.recOn N (by norm_num)
            (fun n ih =>
              by
              cases n <;> simp +decide [Nat.choose, Fin.sum_univ_succ] at *
              all_goals linarith))
          rfl
  -- By definition of `turanEdges`, we know that `turanEdges q d = (d.choose 2) -
  -- (Finset.univ.filter (fun p : Fin d × Fin d => p.1 < p.2 ∧ ¬(turanGraph d q).Adj p.1
  -- p.2)).card`.
  have h_turanEdges_d :
      turanEdges q d =
        (d.choose 2) -
      ...
      have h_turanEdges_d :
        (Finset.univ.filter (fun p : Fin d × Fin d => p.1 < p.2)).card =
          (turanGraph d q).edgeFinset.card +
            (Finset.univ.filter
                (fun p : Fin d × Fin d => p.1 < p.2 ∧ ¬(turanGraph d q).Adj p.1 p.2)).card :=
        by
        rw [show
            (Finset.univ.filter fun p : Fin d × Fin d => p.1 < p.2) =
              (Finset.univ.filter fun p : Fin d × Fin d =>
                  p.1 < p.2 ∧ (turanGraph d q).Adj p.1 p.2) ∪
                (Finset.univ.filter fun p : Fin d × Fin d =>
                  p.1 < p.2 ∧ ¬(turanGraph d q).Adj p.1 p.2)
            from ?_,
        ...
        · refine congr_arg₂ (· + ·) ?_ rfl
          ...
          · intro p hp
            exact SimpleGraph.mem_edgeFinset.mpr (Finset.mem_filter.mp hp).2.2
          · intro p hp r hr heq
            ...
            · exact False.elim (by rcases p with ⟨a, b⟩; rcases r with ⟨c, e⟩; simp_all; omega)
          · intro e he
            induction e using Sym2.inductionOn with
            | _ u
              v =>
              have huv : (turanGraph d q).Adj u v := SimpleGraph.mem_edgeFinset.mp he
              rcases lt_or_gt_of_ne huv.ne with hlt | hgt
              · exact ⟨(u, v), Finset.mem_filter.mpr ⟨Finset.mem_univ _, hlt, huv⟩, rfl⟩
              · exact
                  ⟨(v, u), Finset.mem_filter.mpr ⟨Finset.mem_univ _, hgt, huv.symm⟩,
                    Sym2.eq_swap⟩
        · exact Finset.disjoint_filter.mpr fun _ _ _ _ => by tauto;
        · grind;
      convert! h_turanEdges_d using 1; rw [Nat.choose_two_right]; rw [Finset.card_filter];
      erw [Finset.sum_product]; norm_num [Finset.sum_ite, Finset.filter_lt_eq_Ioi];
      rw [← Finset.sum_range_id]; rw [← Finset.sum_range_reflect, Finset.sum_range];
  -- Therefore, the number of edges of `G` is `turanEdges q d + d * (N - d)`.
  have h_edges_G_simplified : G.edgeFinset.card = turanEdges q d + d * (N - d) :=
      by
      ...
      · rw [tsub_eq_of_eq_add];
        linarith [Nat.sub_add_cancel
            (show
              Finset.card
                  (Finset.filter
                    (fun p : Fin d × Fin d => p.1 < p.2 ∧ ¬(turanGraph d q).Adj p.1 p.2)
                    Finset.univ) ≤
                d.choose 2
              from
              by
              refine
                le_trans
                  (Finset.card_le_card (t :=
                    Finset.univ.filter (fun p : Fin d × Fin d => p.1 < p.2)) ?_)
                  ?_
              · grind;
              · rw [Nat.choose_two_right]; rw [Finset.card_filter];
                convert! Finset.sum_range_id d |> le_of_eq using 1; erw [Finset.sum_product];
                norm_num [Finset.sum_ite]; simp +decide only [filter_lt_eq_Ioi, Fin.card_Ioi];
                rw [← Finset.sum_range_reflect, Finset.sum_range])];
      · rw [← Nat.add_sub_of_le hd];
        exact
          Nat.recOn (N - d) (by norm_num) fun n ih => by
            simp +decide [Nat.choose] at *
            linarith
  -- By Turán's theorem, `G` is `CliqueFree (q+2)`.
  have h_clique_free : G.CliqueFree (q + 2) :=
      by
      intro s hs;
      have :=
        Finset.card_le_univ
          (Finset.image
            (fun x : Fin N =>
              if x.val < d then Fin.castSucc ⟨x.val % q, Nat.mod_lt _ (by linarith)⟩
              else Fin.last q)
            s);
      simp_all +decide only [Fin.castSucc_mk, Fintype.card_fin]
...
-/
theorem induce_neighborFinset_cliqueFree {V : Type} [Fintype V]
    (J : SimpleGraph V) [DecidableRel J.Adj] (v : V) (n : ℕ)
    (h : J.CliqueFree (n + 1)) :
    (J.induce (↑(J.neighborFinset v) : Set V)).CliqueFree n := by
  classical
  intro t ht
  have himg : J.IsNClique n (t.image Subtype.val) := by
    refine ⟨?_, (Finset.card_image_of_injective _ Subtype.val_injective).trans ht.2⟩
    intro a ha b hb hab
    obtain ⟨x, hx, rfl⟩ := Finset.mem_image.mp ha
    obtain ⟨y, hy, rfl⟩ := Finset.mem_image.mp hb
    exact ht.1 hx hy (fun hxy => hab (congrArg Subtype.val hxy))
  apply h _ (himg.insert (a := v) ?_)
  intro b hb
  obtain ⟨x, hx, rfl⟩ := Finset.mem_image.mp hb
  exact (J.mem_neighborFinset v x.val).mp x.property
...
theorem induce_inter_neighbor_cliqueFree {V : Type} [Fintype V] [DecidableEq V]
  ...
  intro t ht
  let f : {x // x ∈ J.neighborFinset v ∩ S} → {x // x ∈ S} :=
    fun x => ⟨x.val, (Finset.mem_inter.mp x.property).2⟩
  have hf : Function.Injective f := by
    intro a b hab
    exact Subtype.ext (congrArg (fun x : {x // x ∈ S} => x.val) hab)
  have himg : (J.induce (↑S : Set V)).IsNClique n (t.image f) := by
    refine ⟨?_, (Finset.card_image_of_injective _ hf).trans ht.2⟩
    intro a ha b hb hab
    obtain ⟨x, hx, rfl⟩ := Finset.mem_image.mp ha
    obtain ⟨y, hy, rfl⟩ := Finset.mem_image.mp hb
    exact ht.1 hx hy (fun hxy => hab (congrArg f hxy))
  apply h _ (himg.insert (a := ⟨v, hv⟩) ?_)
  intro b hb
  obtain ⟨x, hx, rfl⟩ := Finset.mem_image.mp hb
  exact (J.mem_neighborFinset v x.val).mp (Finset.mem_inter.mp x.property).1
...
theorem edgesIn_union_disjoint {V : Type} [Fintype V] [DecidableEq V]
    (J : SimpleGraph V) {A B : Finset V} (hAB : Disjoint A B) :
  ...
  · apply congrArg Finset.card
    ...
    induction e using Sym2.inductionOn with
    | _ u v =>
      simp only [Finset.mem_filter, Finset.mem_union, Sym2.forall_mem_pair,
        SimpleGraph.mem_edgeFinset]
      constructor
      · rintro ⟨he, hu, hv⟩
        rcases hu with hu | hu <;> rcases hv with hv | hv
        · exact Or.inl (Or.inl ⟨he, hu, hv⟩)
        · exact Or.inr ⟨he, u, v, rfl, hu, hv⟩
        · exact Or.inr ⟨he, v, u, Sym2.eq_swap, hv, hu⟩
        · exact Or.inl (Or.inr ⟨he, hu, hv⟩)
      · rintro ((⟨he, hu, hv⟩ | ⟨he, hu, hv⟩) | ⟨he, a, b, heq, ha, hb⟩)
        · exact ⟨he, Or.inl hu, Or.inl hv⟩
        · exact ⟨he, Or.inr hu, Or.inr hv⟩
        · rcases Sym2.eq_iff.mp heq with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩
          · exact ⟨he, Or.inl ha, Or.inr hb⟩
          · exact ⟨he, Or.inr hb, Or.inl ha⟩
  all_goals
    apply Finset.disjoint_left.mpr
    rintro ⟨u, v⟩ he hf
    simp only [Finset.mem_filter, Finset.mem_union, Sym2.forall_mem_pair,
      Sym2.eq_iff] at he hf
    have hd : ∀ x, x ∈ A → x ∈ B → False :=
      fun _ hxA hxB => Finset.disjoint_left.mp hAB hxA hxB
    aesop
...
theorem sum_neighbor_inter_self {V : Type} [Fintype V] [DecidableEq V]
    ...
    ∑ u ∈ B, (J.neighborFinset u ∩ B).card = 2 * edgesIn J B := by
        ...
        · simp +decide only [SetLike.coe_sort_coe, univ_eq_attach];
          ...
          intro a ha; rw [← SimpleGraph.card_neighborFinset_eq_degree];
          rw [← Finset.card_image_of_injective _ Subtype.coe_injective]; congr; ext; aesop;
        · convert! rfl;
          refine Finset.card_bij (fun e he => Sym2.map (fun x => x.val) e) ?_ ?_ ?_ <;>
            simp +decide only [SetLike.coe_sort_coe, mem_edgeFinset, mem_filter, Sym2.mem_map,
              Subtype.exists, exists_and_right, exists_eq_right, forall_exists_index,
              exists_prop, and_imp];
          · rintro ⟨u, v⟩; aesop;
          · rintro ⟨x, y⟩ hxy ⟨u, v⟩ huv h; simp_all +decide [];
          · rintro ⟨u, v⟩ huv hu;
            use Sym2.mk (⟨u, hu u (by simp +decide)⟩ : B) (⟨v, hu v (by simp +decide)⟩ : B);
            aesop;
...
theorem sum_neighbor_inter_cross {V : Type} [Fintype V] [DecidableEq V]
  ...
  simp +decide only [neighborFinset];
  simp +decide only [card_eq_sum_ones];
  rw [ Finset.sum_sigma' ];
  refine Finset.sum_bij ( fun x hx => s(x.2, x.1) ) ?_ ?_ ?_ ?_ <;> simp +decide only [mem_sigma,
    mem_inter, Set.mem_toFinset, mem_neighborSet, mem_filter, mem_edgeFinset, edgeSet,
    mem_edgeSet, Sym2.eq, Sym2.rel_iff', Prod.mk.injEq, Prod.swap_prod_mk, and_imp, exists_prop,
    Sigma.exists, forall_exists_index, implies_true];
...
-/
set_option maxHeartbeats 1600000 in
-- The induction tracks internal and crossing edge counts through each maximum degree split.
theorem furedi_rel (q : ℕ) (hq : 1 ≤ q) {V : Type} [Fintype V]
  ...
  classical
  induction hq generalizing S with
  | refl =>
      use fun _ => 0; simp +decide only [monoIn, Fin.isValue, and_true, turanEdges];
      rw [Finset.card_eq_zero.mpr] <;> norm_num [edgesIn];
      · convert! Nat.zero_le _ using 2; simp +decide only [edgeFinset, card_eq_zero,
          filter_eq_empty_iff, Set.mem_toFinset, not_forall]; contrapose! hcf;
        simp_all +decide only [CliqueFree, SetLike.coe_sort_coe, Nat.reduceAdd, not_forall,
          Decidable.not_not]; obtain ⟨e, he₁, he₂⟩ := hcf;
        rcases e with ⟨u, v⟩; use {⟨u, he₂ u (by simp +decide)⟩, ⟨v, he₂ v (by simp +decide)⟩};
        simp_all +decide only [mem_edgeSet, isNClique_iff, coe_insert, coe_singleton,
          isClique_insert, Set.pairwise_singleton, Set.mem_singleton_iff, ne_eq,
          Subtype.mk.injEq, comap_adj, SetLike.coe_sort_coe, forall_eq, true_and];
        exact ⟨fun _ => he₁, by rw [Finset.card_pair]; aesop⟩;
      · contrapose! hcf; simp_all +decide only [↓existsAndEq, mem_edgeSet, Sym2.mem_iff,
          forall_eq_or_imp, forall_eq, and_true, CliqueFree, SetLike.coe_sort_coe,
          Nat.reduceAdd, not_forall, Decidable.not_not];
        obtain ⟨x, y, hxy, hx, hy⟩ := hcf; use {⟨x, hx⟩, ⟨y, hy⟩};
        simp_all +decide only [isNClique_iff, coe_insert, coe_singleton, isClique_insert,
          Set.pairwise_singleton, Set.mem_singleton_iff, ne_eq, Subtype.mk.injEq, comap_adj,
          SetLike.coe_sort_coe, forall_eq, true_and];
  ...
  | @step q hq ih =>
    ...
    · obtain ⟨ v, hv ⟩ := Finset.exists_max_image S ( fun u => ( J.neighborFinset u ∩ S ).card ) hS;
      ...
      set B := S \ A;
      -- By the induction hypothesis, there exists a coloring $c'$ of $A$ such that $monoIn J c' A
      -- \leq turanEdges q A.card - edgesIn J A$.
      obtain ⟨ c', hc' ⟩ := ih A (by
      ...
      have h_mono :
          monoIn J (fun x => if x ∈ A then Fin.castSucc (c' x) else Fin.last q) S =
            monoIn J c' A + edgesIn J B :=
          by
          unfold monoIn edgesIn; rw [← Finset.card_union_of_disjoint];
          · congr with e; simp +decide [Fin.ext_iff]; rcases e with ⟨u, w⟩; simp_all +decide [];
            grind;
          · simp +contextual [Finset.disjoint_left]; grind;
      -- By the properties of the Turán graph, we have $2 * edgesIn J B + crossCount J A B \leq
      -- A.card * (S.card - A.card)$.
      have h_turan : 2 * edgesIn J B + crossCount J A B ≤ A.card * (S.card - A.card) :=
          by
          have h_turan :
            ∑ u ∈ B, (J.neighborFinset u ∩ S).card = 2 * edgesIn J B + crossCount J A B :=
            by
            have h_turan :
              ∑ u ∈ B, (J.neighborFinset u ∩ S).card =
                ∑ u ∈ B, (J.neighborFinset u ∩ B).card +
                  ∑ u ∈ B, (J.neighborFinset u ∩ A).card :=
              by
              rw [← Finset.sum_add_distrib]; refine Finset.sum_congr rfl fun x hx => ?_;
              rw [← Finset.card_union_of_disjoint];
              · congr with y; by_cases hy : y ∈ A <;> aesop;
              · exact
                  Finset.disjoint_left.mpr fun y hy₁ hy₂ =>
                    Finset.mem_sdiff.mp (Finset.mem_inter.mp hy₁ |>.2) |>.2
                      (Finset.mem_inter.mp hy₂ |>.2);
            rw [h_turan, sum_neighbor_inter_self, sum_neighbor_inter_cross];
            exact Finset.disjoint_sdiff;
          rw [← h_turan, mul_comm];
          refine
            le_trans (Finset.sum_le_sum fun x hx => hv.2 x <| Finset.mem_sdiff.mp hx |>.1) ?_;
          simp +decide [*]; grind;
      -- By the properties of the Turán graph, we have $edgesIn J S = edgesIn J A + edgesIn J B +
      -- crossCount J A B$.
      ...
      have :=
          turanEdges_partite_step q A.card S.card hq
            (show A.card ≤ S.card from Finset.card_le_card fun x hx => by aesop)
      norm_cast at *
      simp_all +decide only [Nat.succ_eq_add_one, ge_iff_le]
      rw [ Int.subNatNat_eq_coe ] at * ; omega;
    · have hS0 : S = ∅ := Finset.not_nonempty_iff_eq_empty.mp hS
      subst S
      refine ⟨fun _ => 0, ?_⟩
      have hnone (e : Sym2 V) : ¬(∀ x ∈ e, x ∈ (∅ : Finset V)) := by
        rcases e with ⟨u, v⟩
        intro h
        exact Finset.notMem_empty u (h u (by simp))
      simp only [monoIn, edgesIn, hnone, false_and, Finset.filter_false,
        Finset.card_empty, Nat.cast_zero, sub_zero]
      positivity
...
theorem furedi_partition (q : ℕ) (hq : 1 ≤ q) {V : Type} [Fintype V] [DecidableEq V]
  ...
  -- By definition of $turanEdges$, we know that $turanEdges q (Fintype.card V)$ is the maximum
  -- number of edges in a $q$-partite graph on $Fintype.card V$ vertices.
  have h_turan :
      ∀ (V : Type) [Fintype V] [DecidableEq V] (J : SimpleGraph V) [DecidableRel J.Adj],
        J.CliqueFree (q + 1) →
          ∃ c : V → Fin q,
            (monoIn J c Finset.univ : ℝ) ≤
              (turanEdges q (Fintype.card V) : ℝ) - edgesIn J Finset.univ :=
      by
      intros V _ _ J _ hKfree
      apply furedi_rel q hq J Finset.univ
      intro t ht
      apply hKfree (t.image Subtype.val)
      refine ⟨?_, (Finset.card_image_of_injective _ Subtype.val_injective).trans ht.2⟩
      intro a ha b hb hab
      obtain ⟨x, hx, rfl⟩ := Finset.mem_image.mp ha
      obtain ⟨y, hy, rfl⟩ := Finset.mem_image.mp hb
...
theorem erdos_simonovits_stability
  ...
  · intro V _ _ J _ hN hF hJ
    ...
    have hD₄ :
        (J.edgeFinset.card : ℝ) - D.card ≥
          (turanEdges q (Fintype.card V) : ℝ) - δ₁ * (Fintype.card V) ^ 2 :=
        by linarith;
    ...
    · refine mod_cast le_trans (Finset.card_le_card
        (t := (J.deleteEdges D).edgeFinset.filter
          (fun e => ∃ u v, e = s(u, v) ∧ c u = c v) ∪ D) ?_) ?_
