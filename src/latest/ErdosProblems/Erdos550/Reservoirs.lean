...
theorem markov_count {V : Type*} [Fintype V] (f : V → ℕ) (d : ℕ) :
  ...
  exact
        le_trans (by rw [Finset.sum_const, smul_eq_mul, mul_comm])
...
-/
theorem red_F_from_red_H {V : Type*} (Gr : SimpleGraph V)
    ...
    Kmult (p + 2) m ⊑ Gr := by
  classical
  apply greedy_multipartite_embedding_ordered Gr (p + 2) m (Fin.cons S0 (Fin.cons S1 W))
  · intro i j
    ...
    · rename_i j
      ...
      · exact fun _ => hHdisj
      · exact fun _ => hdisjS0W _
    · rename_i i
      induction i using Fin.cases
      · exact fun _ => hHdisj.symm
      · exact fun _ => (hdisjS0W _).symm
    · rename_i i j
      induction i using Fin.cases <;> induction j using Fin.cases
      · exact fun h => (h rfl).elim
      · exact fun _ => hdisjS1W _
      · exact fun _ => (hdisjS1W _).symm
      · exact fun h => hdisjW _ _ (fun e => h (congrArg (fun k => k.succ.succ) e))
  · intro j U hU hUcard
    ...
    | zero =>
      have hU0 : U = ∅ := by
        ...
        intro u hu
        obtain ⟨i, hi, _⟩ := hU u hu
        exact (Fin.not_lt_zero i) hi
      simpa only [hU0, Fin.cons_zero, notMem_empty, IsEmpty.forall_iff, implies_true,
        filter_true] using hS0.ge
    | succ j =>
      induction j using Fin.cases with
      | zero =>
        change m 1 ≤ (S1.filter (fun v => ∀ u ∈ U, Gr.Adj v u)).card
        refine hS1.ge.trans (Finset.card_le_card ?_)
        intro v hv
        refine Finset.mem_filter.mpr ⟨hv, ?_⟩
        intro u hu
        obtain ⟨i, hi, hmem⟩ := hU u hu
        have hi0 : i = 0 := by
          apply Fin.ext
          have hi' : i.val < 1 := by
            simpa only [Fin.lt_def, Fin.val_succ, Fin.val_zero, Nat.zero_add] using hi
          exact Nat.lt_one_iff.mp hi'
        subst i
        exact (hHred u hmem v hv).symm
      | succ j =>
        refine (hrich j U hUcard).trans (Finset.card_le_card ?_)
        intro v hv
        exact Finset.mem_filter.mpr ⟨(Finset.mem_filter.mp hv).1, (Finset.mem_filter.mp hv).2.2.2⟩
...
-/
theorem red_F_from_red_H_ordered {V : Type*} (Gr : SimpleGraph V)
    [DecidableRel Gr.Adj] (p : ℕ) (m : Fin (p + 2) → ℕ)
    (S0 S1 : Finset V) (hS0 : S0.card = m 0) (hS1 : S1.card = m 1)
    (hHdisj : Disjoint S0 S1)
    (hHred : ∀ u ∈ S0, ∀ v ∈ S1, Gr.Adj u v)
    ...
    Kmult (p + 2) m ⊑ Gr := by
  classical
  apply greedy_multipartite_embedding_ordered Gr (p + 2) m (Fin.cons S0 (Fin.cons S1 R))
  · intro i j
    induction i using Fin.cases <;> induction j using Fin.cases
    · exact fun h => (h rfl).elim
    · rename_i j
      induction j using Fin.cases
      · exact fun _ => hHdisj
      · exact fun _ => hdisjS0R _
    · rename_i i
      induction i using Fin.cases
      · exact fun _ => hHdisj.symm
      · exact fun _ => (hdisjS0R _).symm
    · rename_i i j
      induction i using Fin.cases <;> induction j using Fin.cases
      · exact fun h => (h rfl).elim
      · exact fun _ => hdisjS1R _
      · exact fun _ => (hdisjS1R _).symm
      · exact fun h => hdisjR _ _ (fun e => h (congrArg (fun k => k.succ.succ) e))
  · intro j U hU hUcard
    induction j using Fin.cases with
    | zero =>
      have hU0 : U = ∅ := by
    ...
    | succ j =>
      ...
      | zero =>
        ...
        exact (hHred u hmem v hv).symm
      | succ j =>
        have hprev : ∀ u ∈ U, u ∈ S0 ∨ u ∈ S1 ∨ ∃ k', k' < j ∧ u ∈ R k' := by
          intro u hu
          obtain ⟨i, hi, hmem⟩ := hU u hu
          induction i using Fin.cases with
          | zero => exact Or.inl hmem
          | succ i =>
            induction i using Fin.cases with
            | zero => exact Or.inr (Or.inl hmem)
            | succ i =>
              exact Or.inr (Or.inr ⟨i, by simpa only [Fin.succ_lt_succ_iff] using hi, hmem⟩)
        refine (hrich j U hprev hUcard).trans (Finset.card_le_card ?_)
        intro v hv
        exact Finset.mem_filter.mpr ⟨(Finset.mem_filter.mp hv).1, (Finset.mem_filter.mp hv).2.2.2⟩
...
lemma turanEdges_ge (q N : ℕ) (hq : 1 ≤ q) :
  ...
  · rw [ div_le_iff₀ ( by positivity ) ];
    have h_turan_edges :
        (turanEdges (q + 2) N : ℕ) ≥ (N ^ 2 - (N % (q + 2)) ^ 2) * (q + 1) / (2 * (q + 2)) := by
      unfold turanEdges;
    ...
    nlinarith [Nat.div_add_mod ((N ^ 2 - (N % (q + 2)) ^ 2) * (q + 1)) (2 * (q + 2)),
        Nat.mod_lt ((N ^ 2 - (N % (q + 2)) ^ 2) * (q + 1)) (by positivity : 0 < 2 * (q + 2))]
...
lemma chromaticNumber_Kmult (k : ℕ) (m : Fin k → ℕ) (hpos : ∀ i, 1 ≤ m i) :
  ...
  · refine le_ciInf fun n => ?_;
    by_cases hn : n < k
    · simp only [Set.mem_ofPred_eq, le_iInf_iff, Nat.cast_le]
      rintro ⟨ f, hf ⟩;
      have h_inj : Function.Injective (fun i : Fin k => f ⟨i, ⟨0, hpos i⟩⟩) := by
        intro i j hij
        specialize @hf ⟨i, ⟨0, hpos i⟩⟩ ⟨j, ⟨0, hpos j⟩⟩
        simp_all +decide only [Kmult,
          comap_adj, top_adj, ne_eq, completeGraph_eq_top, SimpleGraph.irrefl, imp_false,
          Decidable.not_not];
      exact absurd ( Fintype.card_le_of_injective _ h_inj ) ( by simpa )
    · simp only [Set.mem_ofPred_eq, le_iInf_iff, Nat.cast_le]
      exact fun _ => Nat.le_of_not_gt hn
...
lemma edges_le_mono_add_pairs {V : Type*} [Fintype V] [DecidableEq V]
  ...
  have hnat : Gr.edgeFinset.card ≤
      (Gr.edgeFinset.filter (fun e => ∃ u v, e = s(u, v) ∧ c u = c v)).card +
        ∑ p ∈ Finset.univ.filter (fun p : Fin q × Fin q => p.1 < p.2),
          (Finset.univ.filter (fun v => c v = p.1)).card *
            (Finset.univ.filter (fun v => c v = p.2)).card := by
    refine
        le_trans
          (Finset.card_le_card (t :=
            Finset.image (fun e => s(e.1, e.2))
                (Finset.filter (fun e => Gr.Adj e.1 e.2 ∧ c e.1 = c e.2)
                  (Finset.univ : Finset (V × V))) ∪
              Finset.biUnion (Finset.univ.filter (fun p : Fin q × Fin q => p.1 < p.2))
                (fun p =>
                  Finset.image (fun e => s(e.1, e.2))
                    (Finset.filter (fun v => c v = p.1) Finset.univ ×ˢ
                      Finset.filter (fun v => c v = p.2) Finset.univ)))
            ?_)
          ?_
    · intro e he; simp_all +decide only [mem_edgeFinset, mem_union, mem_image, mem_filter,
        mem_univ, true_and, Prod.exists, mem_biUnion, mem_product, ↓existsAndEq, and_true] ;
      rcases e with ⟨ a, b ⟩ ; cases lt_trichotomy ( c a ) ( c b ) <;> aesop;
    · refine le_trans ( Finset.card_union_le _ _ ) ( add_le_add ?_ ?_ );
      · refine Finset.card_le_card ?_;
        simp +decide only [subset_iff, mem_image, mem_filter, mem_univ, true_and, Prod.exists,
          mem_edgeFinset, forall_exists_index, and_imp];
...
lemma bip_sides_res {V : Type*} (Gr : SimpleGraph V)
    (a b : ℕ) (Y : Finset V)
  ...
  classical
          obtain ⟨f, hf⟩ := h;
          refine
              ⟨Finset.image (fun k : Fin a => (f (Sum.inl k)).val) Finset.univ,
                Finset.image (fun k : Fin b => (f (Sum.inr k)).val) Finset.univ, ?_, ?_, ?_, ?_,
                ?_, ?_⟩ <;>
            simp +decide only [SetLike.coe_sort_coe, mem_image, mem_univ, true_and,
              forall_exists_index, forall_apply_eq_imp_iff];
          all_goals norm_num [Finset.subset_iff, Finset.disjoint_left];
          · rw [Finset.card_image_of_injective _ fun x y hxy => by
                simpa using! hf <| Subtype.ext hxy,
              Finset.card_fin];
          ...
          · intro i j;
            have :=
              f.map_rel
                (show (Kbip a b).Adj (Sum.inl i) (Sum.inr j) from by simp +decide [Kbip]);
            aesop;
...
-/
lemma reservoir_H_free {V : Type*} [Finite V] (Gr : SimpleGraph V)
  ...
  intro i hi
  obtain ⟨S0, S1, hS0, hS1, hHdisj, hHred⟩ : ∃ S0 S1 : Finset V,
      S0 ⊆ W i ∧
        S1 ⊆ W i ∧
          S0.card = m 0 ∧ S1.card = m 1 ∧ Disjoint S0 S1 ∧ ∀ u ∈ S0, ∀ v ∈ S1, Gr.Adj u v := by
  ...
  apply red_F_from_red_H_ordered Gr p m S0 S1 hHdisj
    hHred.left hHred.right.left hHred.right.right R
  ...
  · intro k U hU hUcard
    ...
    have h_filter_card :
        (R k \
              Finset.biUnion (S0 ∪ S1 ∪ U)
                (fun w => Finset.filter (fun v => ¬Gr.Adj w v) (R k))).card ≥
          m (Fin.succ (Fin.succ k)) := by
      have h_filter_card :
          (Finset.biUnion (S0 ∪ S1 ∪ U)
                (fun w => Finset.filter (fun v => ¬Gr.Adj w v) (R k))).card ≤
            (m 0 + m 1 + ∑ x, m x) * ζ := by
        refine le_trans ( Finset.card_biUnion_le ) ?_;
        refine
            le_trans (Finset.sum_le_sum fun x hx => show #({v ∈ R k | ¬Gr.Adj x v}) ≤ ζ from ?_)
              ?_;
        · rcases Finset.mem_union.mp hx with hx | hx
          · rcases Finset.mem_union.mp hx with hx | hx
            · exact hcross i (i.succAbove k) (Fin.ne_succAbove i k) x (hS0 hx)
            · exact hcross i (i.succAbove k) (Fin.ne_succAbove i k) x (hS1 hx)
          · rcases hU x hx with hx | hx | ⟨k', hk', hx⟩
            · exact hcross i (i.succAbove k) (Fin.ne_succAbove i k) x (hS0 hx)
            · exact hcross i (i.succAbove k) (Fin.ne_succAbove i k) x (hS1 hx)
            · exact hcross (i.succAbove k') (i.succAbove k)
                (fun h => (ne_of_lt hk') (Fin.succAbove_right_injective h)) x hx
      ...
      · exact
            le_tsub_of_add_le_left
              (by
                linarith [hWcard (Fin.succAbove i k),
                  Finset.single_le_sum (fun a _ => Nat.zero_le (m a))
                    (Finset.mem_univ (Fin.succ (Fin.succ k)))]);
      · exact Finset.biUnion_subset.mpr fun x hx => Finset.filter_subset _ _;
    refine le_trans h_filter_card ( Finset.card_mono ?_ );
    intro v hv; by_cases h : ∃ a, (a ∈ S0 ∨ a ∈ S1 ∨ a ∈ U) ∧ ¬Gr.Adj a v <;>
        simp_all +decide only [Nat.reduceLeDiff, ne_eq, not_nonempty_iff, SetLike.coe_sort_coe,
          union_assoc, ge_iff_le, mem_sdiff, mem_biUnion, mem_union, mem_filter, adj_comm,
          not_exists, not_and, Decidable.not_not, true_or, implies_true, or_true, and_self,
          not_true_eq_false, not_false_eq_true] ;

/-
**Finset Markov inequality.**  `d · |{v ∈ A : d ≤ f v}| ≤ ∑_{v∈A} f v`.
-/
lemma markov_finset {V : Type*} (A : Finset V) (f : V → ℕ) (d : ℕ) :
...
lemma sum_monoDeg_eq {V : Type*} [Fintype V] [DecidableEq V] (Gr : SimpleGraph V)
  ...
  convert!
      congr_arg ((↑) : ℕ → ℝ)
        (SimpleGraph.sum_degrees_eq_twice_card_edges
          (Gr ⊓ SimpleGraph.fromRel (fun u v => c u = c v))) using
      1;
  · simp only [Nat.cast_sum]
    ...
    congr 1
    ext u
    simp only [mem_filter, mem_univ, true_and, mem_neighborFinset, inf_adj, fromRel_adj]
    constructor
    · intro h
      exact ⟨h.2, h.2.ne, Or.inr h.1⟩
    · intro h
      exact ⟨h.2.2.elim Eq.symm id, h.1⟩
  · -- The intersection retains exactly the edges satisfying the colour relation.
    have h_edgeFinset :
        (Gr ⊓ SimpleGraph.fromRel (fun u v => c u = c v)).edgeFinset =
          {e ∈ Gr.edgeFinset | ∃ u v, e = s(u, v) ∧ c u = c v} := by
      ext e; simp only [edgeFinset_inf, mem_inter, mem_edgeFinset, edgeSet, mem_filter,
        and_congr_right_iff];
      cases e ; aesop;
    simp +decide only [edgeFinset_inf, Nat.cast_mul, Nat.cast_ofNat, mul_eq_mul_left_iff,
      Nat.cast_inj, OfNat.ofNat_ne_zero, or_false];
    grind +suggestions
...
-/
lemma sum_crossAll_eq {V : Type*} [Fintype V] (q : ℕ) (c : V → Fin q) :
  ...
  have h_sum_sq :
    ...
    have h_sum_sq :
        ∑ v, ((Finset.univ.filter fun u => c u = c v).card : ℝ) =
          ∑ i,
            ∑ v ∈ Finset.univ.filter fun v => c v = i,
              ((Finset.univ.filter fun u => c u = i).card : ℝ) := by
      simp +decide only [sum_filter];
      rw [ Finset.sum_comm, Finset.sum_congr rfl ] ; aesop;
    simp_all +decide only [sum_const, nsmul_eq_mul, sq];
  have hcard (v : V) :
      ((Finset.univ.filter (fun u => c u ≠ c v)).card : ℝ) =
        Fintype.card V - (Finset.univ.filter (fun u => c u = c v)).card := by
    have h := Finset.card_filter_add_card_filter_not
      (s := (Finset.univ : Finset V)) (fun u => c u = c v)
    rw [Finset.card_univ] at h
    have hreal := congrArg (fun n : ℕ => (n : ℝ)) h
    push_cast at hreal
    linarith
  simp_rw [hcard]
  rw [Finset.sum_sub_distrib, h_sum_sq]
  simp [sq]
...
lemma sum_crossBlue_eq {V : Type*} [Fintype V] [DecidableEq V] (Gr : SimpleGraph V)
    ...
    (∑ v, ((Finset.univ.filter (fun u => c u ≠ c v ∧ ¬ Gr.Adj v u)).card : ℝ))
      = (Fintype.card V : ℝ) ^ 2
        ...
        + 2 * ((Gr.edgeFinset.filter (fun e => ∃ u v, e = s(u, v) ∧ c u = c v)).card : ℝ) := by
  have h_crossBlue_split :
      ∀ v,
        ((Finset.univ.filter (fun u => c u ≠ c v ∧ ¬Gr.Adj v u)).card : ℝ) =
          ((Finset.univ.filter (fun u => c u ≠ c v)).card : ℝ) -
            ((Finset.univ.filter (fun u => c u ≠ c v ∧ Gr.Adj v u)).card : ℝ) := by
    intro v
    ...
    · congr 1
      ext u
      by_cases hu : Gr.Adj v u <;> simp [hu]
    · exact Finset.disjoint_filter.mpr (by aesop)
  have h_crossRed_split :
      ∀ v,
        ((Finset.univ.filter (fun u => c u ≠ c v ∧ Gr.Adj v u)).card : ℝ) =
          (Gr.degree v : ℝ) -
            ((Finset.univ.filter (fun u => c u = c v ∧ Gr.Adj v u)).card : ℝ) :=
      ...
      rw [eq_sub_iff_add_eq]
      norm_cast
      change _ + _ = (Gr.neighborFinset v).card
      rw [← Finset.card_union_of_disjoint]
      · congr 1
        ext u
        by_cases h : c u = c v <;> simp [h]
      · exact Finset.disjoint_filter.mpr (by aesop)
  have h_crossRed_split :
      (∑ v, ((Finset.univ.filter (fun u => c u ≠ c v ∧ Gr.Adj v u)).card : ℝ)) =
        2 * (Gr.edgeFinset.card : ℝ) -
          (∑ v, ((Finset.univ.filter (fun u => c u = c v ∧ Gr.Adj v u)).card : ℝ)) := by
    convert! Finset.sum_congr rfl fun v _ => h_crossRed_split v using 1;
    simp +decide only [← Nat.cast_sum, sum_sub_distrib, sum_degrees_eq_twice_card_edges,
      Nat.cast_mul, Nat.cast_ofNat];
  have := sum_crossAll_eq q c; have := sum_monoDeg_eq Gr q c; simp_all +decide only [ne_eq,
    sum_sub_distrib, sub_left_inj] ; ring;
...
-/
lemma reservoir_cleaning {V : Type*} [Fintype V] (Gr : SimpleGraph V)
  ...
  use fun i =>
      Finset.filter
        (fun v =>
          (Finset.univ.filter fun u => c u = i ∧ Gr.Adj u v).card < τ₁ ∧
            (Finset.univ.filter fun u => c u ≠ i ∧ ¬Gr.Adj u v).card < τ₂)
        (Finset.univ.filter fun v => c v = i);
  refine ⟨ ?_, ?_, ?_, ?_, ?_ ⟩;
  · exact fun i => Finset.filter_subset _ _;
  · simp +contextual only [ne_eq, Finset.disjoint_left, mem_filter, mem_univ, true_and,
      and_false, false_and, not_false_eq_true, implies_true];
  · simp +contextual only [ne_eq, mem_filter, mem_univ, true_and, and_imp];
    intro i w hi h₁ h₂;
    refine lt_of_le_of_lt ( Finset.card_le_card ?_ ) h₁;
    simp +contextual only [adj_comm, subset_iff, mem_filter, mem_univ, true_and, and_self,
      implies_true];
  · simp +contextual only [ne_eq, mem_filter, mem_univ, true_and, and_imp];
    intro i j hij w hw₁ hw₂ hw₃
    refine lt_of_le_of_lt ?_ hw₃
    simp +decide only [filter_filter]
    refine Finset.card_mono ?_ ; intro u hu ; simp_all +decide only [adj_comm, mem_filter,
      mem_univ, true_and, not_false_eq_true, and_true] ; aesop;
  · intro i
    have h_card_diff :
        (Finset.univ.filter (fun v => c v = i)).card -
            ((Finset.univ.filter (fun v => c v = i)).filter
                (fun v =>
                  (Finset.univ.filter (fun u => c u = i ∧ Gr.Adj u v)).card < τ₁ ∧
                    (Finset.univ.filter (fun u => c u ≠ i ∧ ¬Gr.Adj u v)).card < τ₂)).card ≤
          ((Finset.univ.filter (fun v => c v = i)).filter
                (fun v =>
                  τ₁ ≤ (Finset.univ.filter (fun u => c u = i ∧ Gr.Adj u v)).card)).card +
            ((Finset.univ.filter (fun v => c v = i)).filter
                (fun v =>
                  τ₂ ≤ (Finset.univ.filter (fun u => c u ≠ i ∧ ¬Gr.Adj u v)).card)).card := by
      ...
      exact
          Finset.card_le_card fun x hx => by
            by_cases h₁ :
                  τ₁ ≤
                    Finset.card (Finset.filter (fun u => c u = i ∧ Gr.Adj u x) Finset.univ) <;>
                by_cases h₂ :
                  τ₂ ≤
                    Finset.card (Finset.filter (fun u => c u ≠ i ∧ ¬Gr.Adj u x) Finset.univ) <;>
              aesop;
    have hglobal :
        (Finset.univ.filter (fun v => c v = i)).card -
            ((Finset.univ.filter (fun v => c v = i)).filter
              (fun v => (Finset.univ.filter (fun u => c u = i ∧ Gr.Adj u v)).card < τ₁ ∧
                (Finset.univ.filter (fun u => c u ≠ i ∧ ¬Gr.Adj u v)).card < τ₂)).card ≤
          (Finset.univ.filter (fun v =>
            τ₁ ≤ (Finset.univ.filter (fun u => c u = c v ∧ Gr.Adj v u)).card)).card +
          (Finset.univ.filter (fun v =>
            τ₂ ≤ (Finset.univ.filter (fun u => c u ≠ c v ∧ ¬Gr.Adj v u)).card)).card := by
      refine h_card_diff.trans (add_le_add (Finset.card_le_card ?_) (Finset.card_le_card ?_))
      · intro v hv
        rcases Finset.mem_filter.mp hv with ⟨hv, hbad⟩
        have hcv := (Finset.mem_filter.mp hv).2
        exact Finset.mem_filter.mpr ⟨Finset.mem_univ _, by
          simpa only [hcv, SimpleGraph.adj_comm] using hbad⟩
      · intro v hv
        rcases Finset.mem_filter.mp hv with ⟨hv, hbad⟩
        have hcv := (Finset.mem_filter.mp hv).2
        exact Finset.mem_filter.mpr ⟨Finset.mem_univ _, by
          simpa only [hcv, SimpleGraph.adj_comm] using hbad⟩
    have hreal := (Nat.cast_le (α := ℝ)).mpr hglobal
    rw [Nat.cast_sub (Finset.card_le_card (Finset.filter_subset _ _)), Nat.cast_add] at hreal
    exact hreal.trans (add_le_add (markov_count_div _ _ hτ₁) (markov_count_div _ _ hτ₂))
...
lemma colour_balance {V : Type*} [Fintype V] [DecidableEq V] (Gr : SimpleGraph V)
  ...
  have h_sum_sq_diff_bound :
      ∑ i : Fin q,
          ((Finset.univ.filter (fun v => c v = i)).card - (Fintype.card V : ℝ) / q) ^ 2 ≤
        2 * (q ^ 2 + (δ + β) * (Fintype.card V : ℝ) ^ 2) := by
    -- By deviation_identity, we have (N^2*(q-1)/q)/2 - ∑ p∈filter(p.1<p.2), s p.1 * s p.2 = (∑
    -- i,(s i - (∑ s)/q)^2)/2.
    have h_deviation :
        ((Fintype.card V : ℝ) ^ 2 * (q - 1) / q) / 2 -
            (∑ p ∈ Finset.univ.filter (fun p : Fin q × Fin q => p.1 < p.2),
              ((Finset.univ.filter (fun v => c v = p.1)).card : ℝ) *
                ((Finset.univ.filter (fun v => c v = p.2)).card : ℝ)) =
          (∑ i : Fin q,
              ((Finset.univ.filter (fun v => c v = i)).card - (Fintype.card V : ℝ) / q) ^ 2) /
            2 := by
      convert!
          deviation_identity hq
            (fun i => (Finset.card (Finset.filter (fun v => c v = i) Finset.univ) : ℝ)) using
          1;
      · rw [show
            (∑ i : Fin q, (Finset.card (Finset.filter (fun v => c v = i) Finset.univ) : ℝ)) =
              Fintype.card V
            from ?_];
    ...
    have h_edges_le_mono_add_pairs :
        (Gr.edgeFinset.card : ℝ) ≤
          ((Gr.edgeFinset.filter (fun e => ∃ u v, e = s(u, v) ∧ c u = c v)).card : ℝ) +
            (∑ p ∈ Finset.univ.filter (fun p : Fin q × Fin q => p.1 < p.2),
              ((Finset.univ.filter (fun v => c v = p.1)).card : ℝ) *
    ...
    have h_turanEdges_ge :
        ((Fintype.card V : ℝ) ^ 2 * (q - 1) / q) / 2 - q ^ 2 ≤
          (turanEdges q (Fintype.card V) : ℝ) := by
  ...
  exact fun i =>
      le_trans
        (Finset.single_le_sum
          (fun i _ =>
            sq_nonneg
              ((Finset.card (Finset.filter (fun v => c v = i) Finset.univ) : ℝ) -
                Fintype.card V / q))
          (Finset.mem_univ i))
        h_sum_sq_diff_bound
  ...
  have h_identity :
      (∑ v, ((Finset.univ.filter (fun u => c u ≠ c v ∧ ¬Gr.Adj v u)).card : ℝ)) =
        (Fintype.card V : ℝ) ^ 2 -
              (∑ i, ((Finset.univ.filter (fun v => c v = i)).card : ℝ) ^ 2) -
            2 * (Gr.edgeFinset.card : ℝ) +
          2 * ((Gr.edgeFinset.filter (fun e => ∃ u v, e = s(u, v) ∧ c u = c v)).card : ℝ) :=
      by convert! sum_crossBlue_eq Gr q c using 1;
  have h_identity :
      (∑ i, ((Finset.univ.filter (fun v => c v = i)).card : ℝ) ^ 2) ≥
        (Fintype.card V : ℝ) ^ 2 / q := by
    have h_identity :
        (∑ i, ((Finset.univ.filter (fun v => c v = i)).card : ℝ)) = (Fintype.card V : ℝ) := by
      rw_mod_cast [← Finset.card_biUnion]
      · congr 1
        ext i
        aesop
    ...
    have h_identity :
        (∑ i : Fin q, ((Finset.univ.filter (fun v => c v = i)).card : ℝ) ^ 2) * q ≥
          (∑ i : Fin q, ((Finset.univ.filter (fun v => c v = i)).card : ℝ)) ^ 2 := by
      have h_cauchy_schwarz :
          ∀ (u v : Fin q → ℝ), (∑ i, u i * v i) ^ 2 ≤ (∑ i, u i ^ 2) * (∑ i, v i ^ 2) := by
        exact fun u v => Finset.sum_mul_sq_le_sq_mul_sq univ u v;
      simpa [mul_comm] using!
          h_cauchy_schwarz
            (fun i => (Finset.card (Finset.filter (fun v => c v = i) Finset.univ) : ℝ))
            (fun _ => 1);
    rw [ ge_iff_le, div_le_iff₀ ] <;> first | positivity | aesop;
  have := turanEdges_ge q ( Fintype.card V ) hq;
  rw [div_sub', div_le_iff₀] at this <;>
      nlinarith [show (q : ℝ) ≥ 1 by norm_cast,
        mul_div_cancel₀ ((Fintype.card V : ℝ) ^ 2) (by positivity : (q : ℝ) ≠ 0)]
...
-- The reservoir construction checks all size, density, and exceptional set estimates together.
lemma clean_reservoir_at {V : Type*} [Fintype V] [DecidableEq V] (Gr : SimpleGraph V)
        ...
        (∀ i, W i ⊆ Finset.univ.filter (fun v => c v = i)) ∧
          (∀ i j, i ≠ j → Disjoint (W i) (W j)) ∧
            (∀ i, ∀ w ∈ W i, ((W i).filter (fun v => Gr.Adj w v)).card < τ₁) ∧
              (∀ i j, i ≠ j → ∀ w ∈ W i, ((W j).filter (fun v => ¬Gr.Adj w v)).card < τ₂) ∧
                (∀ i,
                  ((Finset.univ.filter (fun v => c v = i)).card : ℝ) - ((W i).card : ℝ) ≤
                    (2 * β * (Fintype.card V) ^ 2 / τ₁ +
                      (2 * q ^ 2 + 2 * (δ + β) * (Fintype.card V) ^ 2) / τ₂)) :=
      by
      have := reservoir_cleaning Gr q c τ₁ τ₂ hτ₁ hτ₂;
      obtain ⟨W, hW₁, hW₂, hW₃, hW₄, hW₅⟩ := this; use W;
      refine ⟨hW₁, hW₂, hW₃, hW₄, fun i => le_trans (hW₅ i) ?_⟩; gcongr;
      · have := sum_monoDeg_eq Gr q c; linarith;
      · convert! crossblue_total_bound Gr q (by linarith) c β δ hmono_b hdens using 1;
  refine ⟨ W, hW.2.1, ?_, ?_, ?_, ?_, ?_ ⟩;
  · intro i
    have h_card_bound :
        |((Finset.univ.filter (fun v => c v = i)).card : ℝ) - (Fintype.card V : ℝ) / q| ≤
          (η / 2) * (Fintype.card V : ℝ) / q := by
      have := colour_balance Gr q ( by linarith ) c β δ hmono_b hdens i;
      rw [ ← Real.sqrt_sq_eq_abs ];
      rw [Real.sqrt_le_left] <;> ring_nf at * <;>
          nlinarith [inv_pos.mpr (by positivity : 0 < (q : ℝ))];
    constructor <;> ring_nf at *;
    · have := hW.2.2.2.2 i;
      nlinarith [abs_le.mp h_card_bound, show (q : ℝ) ≥ 2 by norm_cast,
          mul_inv_cancel₀ (by positivity : (q : ℝ) ≠ 0),
          mul_inv_cancel₀ (by positivity : (q ^ 2 : ℝ) ≠ 0),
          mul_inv_cancel₀ (by positivity : (τ₁ : ℝ) ≠ 0),
          mul_inv_cancel₀ (by positivity : (τ₂ : ℝ) ≠ 0)];
    · have := hW.1 i;
  ...
  · exact fun i j hij w hw =>
        le_trans (Nat.cast_le.mpr (Nat.le_of_lt (hW.2.2.2.1 i j hij w hw))) hτ₂le;
  · convert! reservoir_H_free Gr q hq m W hW.2.1 τ₂ _ _ hFfree using 1;
    ...
    · intro i
      have h_card_Wi : (W i).card ≥ (Fintype.card V : ℝ) / q - (Fintype.card V : ℝ) / q / 4 := by
        have h_card_Wi :
            (Finset.univ.filter (fun v => c v = i)).card ≥
              (Fintype.card V : ℝ) / q - (Fintype.card V : ℝ) / q / 8 := by
          have := colour_balance Gr q ( by linarith ) c β δ hmono_b hdens i;
          nlinarith [ show ( 0 : ℝ ) ≤ Fintype.card V / q by positivity ];
        have h_card_Wi :
            (Finset.univ.filter (fun v => c v = i)).card - (W i).card ≤
              (2 * β * (Fintype.card V) ^ 2 / τ₁ +
                (2 * q ^ 2 + 2 * (δ + β) * (Fintype.card V) ^ 2) / τ₂) := by
          exact hW.2.2.2.2 i;
        nlinarith [ show ( q : ℝ ) ≥ 2 by norm_cast ];
      exact Nat.le_of_lt_succ
          (by rw [← @Nat.cast_lt ℝ]; push_cast at *; nlinarith [show (q : ℝ) ≥ 2 by norm_cast]);
  · have h_sum_card :
      ∑ i, ((Finset.univ.filter (fun v => c v = i)).card : ℝ) = (Fintype.card V : ℝ) :=
      by
      rw_mod_cast [← Finset.card_biUnion];
      · convert! Finset.card_univ; ext v; simp +decide only [mem_biUnion, mem_univ, mem_filter,
          true_and, exists_eq'];
...
lemma crd_hb (q : ℕ) (hq : 1 ≤ q) (η s : ℝ) (hη : 0 < η)
  ...
  have hq0 : (0:ℝ) < q := by exact_mod_cast hq
  have hN1 : 4 * (q : ℝ) ^ 2 / η ≤ N :=
      le_trans (Nat.le_ceil _) (by exact_mod_cast (le_trans (Nat.le_add_right _ _) hN))
  have hN2 : 16 * (q : ℝ) ^ 2 ≤ N :=
      le_trans (Nat.le_ceil _) (by exact_mod_cast (le_trans (Nat.le_add_left _ _) hN))
...
-- The regular matching construction combines the reservoir size and degree estimates.
...
lemma crd_RM (q : ℕ) (hq : 2 ≤ q) (η rate β s : ℝ) (hη : 0 < η)
  ...
  have ht1 : 2 * (q : ℝ) / rate ≤ N :=
      le_trans (Nat.le_ceil _) (by exact_mod_cast (by omega : ⌈2 * (q : ℝ) / rate⌉₊ ≤ N))
  have ht2 : 24 * (q : ℝ) ^ 5 / (η * rate) ≤ N :=
      le_trans (Nat.le_ceil _)
        (by exact_mod_cast (by omega : ⌈24 * (q : ℝ) ^ 5 / (η * rate)⌉₊ ≤ N))
  have ht3 : 48 * (q : ℝ) ^ 5 / rate ≤ N :=
      le_trans (Nat.le_ceil _) (by exact_mod_cast (by omega : ⌈48 * (q : ℝ) ^ 5 / rate⌉₊ ≤ N))
...
lemma crd_Hfree (q : ℕ) (hq : 1 ≤ q) (SM : ℕ) (hSM : 1 ≤ SM) (η rate : ℝ) (_hη : 0 < η)
  ...
  change (3 * (SM : ℝ)) * ⌊rate * ((N:ℝ)/q)⌋₊ + (SM : ℝ) ≤ ((N:ℝ)/q) / 2
  nlinarith [h3, hn8]
...
-- The constant selection verifies all smallness and largeness inequalities simultaneously.
lemma clean_reservoir_constants (q : ℕ) (hq : 2 ≤ q) (SM : ℕ) (hSM : 1 ≤ SM)
  ...
  have hβ3 : β ≤ η * rate / (192 * (q : ℝ) ^ 3) :=
      le_trans (min_le_right _ _) (le_trans (min_le_right _ _) (min_le_left _ _))
  have hβ4 : β ≤ rate / (384 * (q : ℝ) ^ 3) :=
      le_trans (min_le_right _ _)
        (le_trans (min_le_right _ _) (le_trans (min_le_right _ _) (min_le_left _ _)))
  have hβ5 : β ≤ η ^ 2 / (32 * (q : ℝ) ^ 2) :=
      le_trans (min_le_right _ _)
        (le_trans (min_le_right _ _)
          (le_trans (min_le_right _ _) (le_trans (min_le_right _ _) (min_le_left _ _))))
  have hβ6 : β ≤ 1 / (512 * (q : ℝ) ^ 2) :=
      le_trans (min_le_right _ _)
        (le_trans (min_le_right _ _)
  ...
  have hβpos : 0 < β := by
    refine
        lt_min (by positivity)
          (lt_min (by positivity)
            (lt_min ?_ (lt_min ?_ (lt_min (by positivity) (by positivity)))))
