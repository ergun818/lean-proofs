        ...
        (J.biUnion
              (fun j =>
                (C i).filter
                  (fun w =>
                    ((C j).filter (fun b => G.Adj w b)).card <
                      (dtar j - ε) * ((C j).card : ℝ)))).card ≤
          J.card * ε * (C i).card := by
      have h_card_biUnion :
          ∀ j ∈ J,
            (Finset.filter
                  (fun w =>
                    ((C j).filter (fun b => G.Adj w b)).card < (dtar j - ε) * ((C j).card : ℝ))
                  (C i)).card <
              ε * (C i).card := by
        intro j hj
        have h_card_lt :
...
-- The strong induction propagates a separate density bound for every target cluster.
              ...
              have hU :
                ...
                · exact
                      add_le_add
                        ...
                        le_rfl;
                · exact
                      mul_le_of_le_one_left (Nat.cast_nonneg _) (by linarith [hdcap1 (clu a)]);
              convert!
                  exists_good_unused_clusters_weighted (dtar := dcap) G hε0 hε1 (hne (clu a)) _
                    _ _ hU using
                  1;
              any_goals
                (intro j hj
