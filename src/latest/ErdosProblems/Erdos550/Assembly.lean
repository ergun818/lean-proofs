...
lemma obstruction_card_le {V : Type*} [DecidableEq V] (Gr : SimpleGraph V)
  ...
  obtain ⟨E', hE'ref, hE'⟩ : ∃ E' : Finset V,
      E' ⊆ E ∧ (Kbip a b ⊑ Gr.induce (↑(Wi ∪ E'))) ∧ E'.card ≤ a + b := by
    obtain ⟨f, hf⟩ := h.2.2.1;
    refine
        ⟨Finset.image (fun x => (f x : V)) (Finset.univ.filter fun x => (f x : V) ∈ E), ?_, ?_,
          ?_⟩;
    · grind;
    · refine ⟨{toFun := fun x => ⟨f x, by grind +splitImp⟩, map_rel' := ?_}, ?_⟩
  ...
  cases eq_or_ne E' E <;> simp_all +decide only [IsObstruction, SetLike.coe_sort_coe,
    not_nonempty_iff, Std.le_refl, ne_eq];
...
-/
lemma exists_obstruction_of_copy {V : Type*} [Finite V] [DecidableEq V]
  ...
  obtain ⟨E, hE⟩ : ∃ E : Finset V,
      E ⊆ Y ∧ Kbip a b ⊑ Gr.induce (↑(Wi ∪ E)) ∧
        ∀ E' ⊂ E, ¬(Kbip a b ⊑ Gr.induce (↑(Wi ∪ E'))) := by
    -- By the well-ordering principle, there exists a minimal subset of `Y` whose union with `Wi`
    -- contains the copy.
    obtain ⟨E, hE⟩ :
        ∃ E : Finset V,
          E ∈ {E' : Finset V | E' ⊆ Y ∧ Kbip a b ⊑ Gr.induce (↑(Wi ∪ E'))} ∧
            ∀ E' ∈ {E' : Finset V | E' ⊆ Y ∧ Kbip a b ⊑ Gr.induce (↑(Wi ∪ E'))},
              E.card ≤ E'.card :=
        by
        apply_rules [Set.exists_min_image]; · exact Set.toFinite _;
        · exact ⟨Y, Finset.Subset.refl _, hcopy⟩;
    exact
        ⟨E, hE.1.1, hE.1.2, fun E' hE' hE'' =>
          not_lt_of_ge (hE.2 E' ⟨Finset.Subset.trans hE'.1 hE.1.1, hE''⟩)
            (Finset.card_lt_card hE')⟩;
  refine ⟨ E, hE.1, ?_, ?_, hE.2.1, hE.2.2 ⟩ <;> simp_all +decide only [SetLike.coe_sort_coe,
    not_nonempty_iff];
  · refine Finset.nonempty_of_ne_empty ?_;
    rintro rfl;
    convert! hWiHfree using 1;
    simp +decide only [false_iff, not_isEmpty_iff];
...
-- The assembly combines the skeleton and shrub embeddings with their capacity estimates.
...
theorem assembly_core (q a : ℕ) (hq : 2 ≤ q) (m' : Fin (q + 1) → ℕ)
  ...
  obtain ⟨ε₀, hε₀, Hcomp⟩ :=
      null_blocker_compactness_graph q hq a (by linarith) rStar (by linarith);
  ...
  intro V _ _ Gr _ Tt T W X r hdisjW hdisjX hcover hWsize hslack hHfree hFfree hNoBlueT hA1
      hRamsey hcard;
  obtain ⟨Z, φ, hZsub, hZcard, hindep⟩ :=
      Hcomp Gr W X (fun i => {E | IsObstruction Gr (m' 0) (m' 1) (W i) X E}) ε₀ (by linarith)
        (by linarith)
        (fun i => Finset.card_pos.mp (by exact lt_of_lt_of_le (by positivity) (hWsize i)))
        (fun i E hE => by
          exact ⟨hE.1, obstruction_card_le Gr (m' 0) (m' 1) (W i) X E hE, hE.2.1⟩)
        hA1
        (fun S hSsub hScard => by
          specialize
            Ha Gr W X hdisjW hdisjX
              (fun i => le_trans (le_max_of_le_left (le_max_left sa sc)) (hWsize i))
              (fun i j hij w hw =>
                le_trans (hslack i j hij w hw)
                  (mul_le_mul_of_nonneg_right (min_le_left ζa ζc) (Nat.cast_nonneg _)))
              hFfree S hSsub (by rw [← haq]; exact hScard);
          exact
            ⟨Ha.choose, div_le_of_le_mul₀ (Nat.cast_nonneg _) (by positivity) (Ha.choose_spec)⟩)
        (fun i E hE => by
          obtain ⟨j, hj₁, hj₂⟩ :=
            Hc Gr W X hdisjW hdisjX
              (fun i => by
                linarith [hWsize i, le_max_left (max sa sc) 1, le_max_right (max sa sc) 1,
                  le_max_left sa sc, le_max_right sa sc])
              (fun i j hij w hw =>
                le_trans (hslack i j hij w hw)
                  ...
                  (Nat.cast_pos.mpr <|
                    Finset.card_pos.mpr <|
                      Finset.card_pos.mp <| by
                        linarith [hWsize j, le_max_left (max sa sc) 1,
                          le_max_right (max sa sc) 1, le_max_left sa sc, le_max_right sa sc])]⟩);
  ...
  have hclass : ∀ i, ¬ (Kbip (m' 0) (m' 1) ⊑ Gr.induce (↑(B i))) := by
    intro i hcopy
    obtain ⟨E, hEsub, hEcirc⟩ :=
        exists_obstruction_of_copy Gr (m' 0) (m' 1) (W i) X (Xi i) (Finset.filter_subset _ _)
          (hHfree i) hcopy
    exact hindep i E hEcirc (by
    exact fun x hx =>
          ⟨Finset.mem_filter.mp (hEsub hx) |>.2.2, Finset.mem_filter.mp (hEsub hx) |>.2.1⟩);
  apply final_reduction Gr (Kbip (m' 0) (m' 1)) T q a r (by linarith) B Z hZcard (fun i j hij => by
    simp +decide only [ne_eq, Finset.disjoint_left] at hdisjW hdisjX ⊢;
    grind) (fun i => by
    simp only [disjoint_union_left, B];
    exact
        ⟨Finset.disjoint_left.mpr fun x hx hx' =>
            Finset.disjoint_left.mp (hdisjX i) (hZsub hx') hx,
          Finset.disjoint_left.mpr fun x hx hx' => Finset.mem_filter.mp hx |>.2.2 hx'⟩) (by
  ext v; simp only [mem_union, mem_biUnion, mem_univ, true_and, iff_true];
  by_cases hv : v ∈ Z <;> simp +decide only [hv, or_true, or_false];
  replace hcover := Finset.ext_iff.mp hcover v; simp +decide only [mem_union, mem_biUnion,
    mem_univ, true_and, iff_true] at hcover;
  exact hcover.elim (fun ⟨i, hi⟩ => ⟨i, Finset.mem_union_left _ hi⟩)
    (fun hi => ⟨φ v, Finset.mem_union_right _ (Finset.mem_filter.mpr ⟨hi, rfl, hv⟩)⟩))
    hclass hNoBlueT hRamsey hcard
