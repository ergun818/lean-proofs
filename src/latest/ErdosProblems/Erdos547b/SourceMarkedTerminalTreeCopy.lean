...
def terminalTreeCopy
    (A : CutPrefixState W Q S O P (branchForest sourceP).branches (branchForest sourceP).owner
      (branchMarks sourceP) selected (componentReservoirSide sourceP) kinds allocation family locate
        hcover
  ...
  let E := A.state.terminalBranchEmbedding W Q S O P (branchForest sourceP).branches (branchForest
    sourceP).owner
    (branchMarks sourceP) selected (componentReservoirSide sourceP) kinds allocation family locate
      hcover hCV1 hresidual hdisjoint
  have hroot : Function.Injective A.state.ordinary.rootImage :=
    fun i j h => A.state.ordinary.root_injective i j i.isLt j.isLt h
  let graphCopy := (branchForest sourceP).copyOfBranchEmbedding (embeddingHost W)
    A.state.ordinary.rootImage E hroot
    (A.state.root_ne_branchCopy W Q S O P (branchForest sourceP).branches (branchForest
      sourceP).owner
      (branchMarks sourceP) selected (componentReservoirSide sourceP) kinds allocation family locate
        hcover hCV1 haway)
    (fun i => A.state.branchCopy_attach W Q S O P (branchForest sourceP).branches (branchForest
      sourceP).owner
      (branchMarks sourceP) selected (componentReservoirSide sourceP) kinds allocation family locate
        hcover i
      ((branchForest sourceP).owner i).isLt)
  let cutCopy : sourceP.cutForest.Copy (embeddingHost W) := graphCopy.comp (cutBranchGraphIso
    sourceP).toCopy
  have hrootMap (i : Fin sourceP.numParts) : cutCopy (sourceP.roots i) = A.state.ordinary.rootImage
    i := by
  ...
  have hcoordinateMap (x : CutCoordinate (branchForest sourceP).branches sourceP.numParts) :
      cutCopy (coordinateVertex sourceP x) =
        A.state.coordinateImage (branchForest sourceP).branches (branchForest sourceP).owner
          (branchMarks sourceP) selected W Q S O P (componentReservoirSide sourceP) kinds allocation
            family locate hcover x
          (coordinateOwner (branchForest sourceP).branches (branchForest sourceP).owner x).isLt :=
            by
    cases x with
    ...
    | inr a =>
        ...
        change graphCopy (Sum.inr ((partitionBranchEquivNonroots sourceP).symm
          (partitionBranchEquivNonroots sourceP a))) = _
