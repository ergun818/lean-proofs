...
theorem exists_actualReservation
  ...
  let initial := castPartialSelected (listForest F R.reserved) (embeddingHost W) p P.orient
    available
  ...
  obtain ⟨E, _⟩ := P.extend_interval W Q p 0 pending.length (Nat.zero_le _) hlength initial z hz
    hcurrent
  exact ⟨⟨R, P, E⟩⟩
