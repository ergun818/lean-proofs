...
theorem PrefixState.ordinary_copies_ne
  ...
  · rw [A.branchCopy_eq_ordinary W Q S O P F owner marks selected rootSide kinds allocation family
      ...
      A.branchCopy_eq_ordinary W Q S O P F owner marks selected rootSide kinds allocation family
        locate hcover j (owner j).isLt hj] at heq
  ...
  · obtain ⟨e, he, c, ha⟩ := A.ordinary_branch_support W Q S F owner O P marks selected rootSide
      ...
      i hi (owner i).isLt a
    obtain ⟨f, hf, l, hd⟩ := A.ordinary_branch_support W Q S F owner O P marks selected rootSide
      kinds allocation family locate hcover
      j hj (owner j).isLt d
    have hef : e ≠ f := fun h => Finset.disjoint_left.mp (hdisjoint (locate i) (locate j) hloc) he
      (h.symm ▸ hf)
...
theorem PrefixState.marked_copies_ne
  ...
  rw [A.branchCopy_eq_marked W Q S O P F owner marks selected rootSide kinds allocation family
    locate hcover i (owner i).isLt hi,
    A.branchCopy_eq_marked W Q S O P F owner marks selected rootSide kinds allocation family locate
      hcover j (owner j).isLt hj] at heq
  exact Set.disjoint_left.mp (A.marked.forestCopy.disjoint_ranges i (Finset.mem_filter.mpr ⟨hi,
    (owner i).isLt⟩)
...
theorem PrefixState.mixed_copies_ne (hCV1 : C ⊆ O.D.V1)
  ...
  have ha : A.branchCopy W Q S O P F owner marks selected rootSide kinds allocation family locate
    ...
    exact A.marked_copy_mem_support W Q S O P F owner marks selected rootSide kinds allocation
      family i himem a
  obtain ⟨e, he, c, hd⟩ := A.ordinary_branch_support W Q S F owner O P marks selected rootSide kinds
