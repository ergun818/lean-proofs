...
def PrefixState.coordinateImage (x : CutCoordinate F r)
  ...
  | Sum.inr a => A.branchCopy W Q S F owner rootSide kinds allocation family locate hcover a.1 hx
      a.2
...
theorem PrefixState.coordinateImage_degree (x : CutCoordinate F r)
    ...
    ((densityCutoff α : ℝ) - (epsilon α : ℝ)) * (sourceQuota W : ℝ) ≤
      (#((reservoir W Q (coordinateSide F rootSide locate x)).filter ((embeddingHost W).Adj
        (A.coordinateImage F owner W Q S rootSide kinds allocation family locate hcover x hx))) : ℝ)
          := by
  cases x with
  | inl i => exact A.root_degree i hx
  | inr a =>
      exact A.branch_rootColor_degree W Q S F owner rootSide kinds allocation family locate hcover
        a.1 hx a.2 hcolor
...
theorem PrefixState.coordinateImage_preserved
  ...
  | inr a =>
      exact congrArg (fun f : (F.tree a.1).Copy (embeddingHost W) => f a.2)
        (A.branchCopy_preserved W Q S F owner rootSide kinds allocation family locate hcover D
          hcopies a.1 hx)
