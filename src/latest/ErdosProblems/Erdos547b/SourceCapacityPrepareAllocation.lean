...
theorem exists_preparedAllocation
  ...
  have hsourceDisjoint : Disjoint A.completed.toFinset (activeSelected W Q S C F owner kind R.after)
    := by
    rw [hactiveFull]
  ...
  · intro i
    have hi : i.1 ∈ A.completed.toFinset ∪ activeSelected W Q S C F owner kind R.after :=
      hdomain.symm ▸ i.2
    by_cases hc : i.1 ∈ A.completed.toFinset
    · have he := Finset.mem_union_left (activeEdges W Q S C F owner kind A.active)
        (A.closed_edge_mem ⟨i.1, hc⟩)
  ...
  · intro i hi
    ...
    · calc
        ...
        _ = (activePlacement W Q S C F owner kind A.active).forestCopy.componentCopy i ha :=
          R.copies_eq i ha
