...
def PrefixState.coordinateImage (x : CutCoordinate F r)
  ...
  | Sum.inr a => A.branchCopy W Q S O P F owner marks selected rootSide kinds allocation family
      locate hcover a.1 hx a.2
...
theorem PrefixState.coordinateImage_degree
    ...
    ((densityCutoff α : ℝ) - (epsilon α : ℝ)) * (sourceQuota W : ℝ) ≤
      ((reservoir W Q (coordinateSide F rootSide locate x)).filter ((embeddingHost W).Adj
        (A.coordinateImage F owner marks selected W Q S O P rootSide kinds allocation family locate
          hcover x hx))).card := by
  cases x with
  | inl i => exact A.ordinary.root_degree i hx
  | inr a =>
      exact A.branchCopy_degree W Q S O P F owner marks selected rootSide kinds allocation family
        locate hcover
...
theorem PrefixState.coordinateImage_preserved
  ...
  | inr a =>
      exact congrArg (fun f : (F.tree a.1).Copy (embeddingHost W) => f a.2)
        (A.branchCopy_preserved W Q S O P F owner marks selected rootSide kinds allocation family
          locate hcover
