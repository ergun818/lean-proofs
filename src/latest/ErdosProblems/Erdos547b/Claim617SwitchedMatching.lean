...
theorem partnerSet_card [Finite K] (hM : M.IsMatching) : D.partnerSet.card = m := by
  classical
...
omit [DecidableRel R.Adj] in
theorem remainder_isMatching (hM : M.IsMatching) : D.remainder.IsMatching := by
  classical
  exact edgeFinsetSubgraph_isMatching M hM L _
...
omit [DecidableRel R.Adj] [Fintype K] in
private theorem edge_eq_of_endpoint_eq [Finite K] (hM : M.IsMatching) (e f : MatchingEdge M)
...
theorem switched_disjoint_of_disjoint (B : Finset K)
    ...
    Disjoint (matchingSupport D.switched) B := by
  classical
  exact Finset.disjoint_of_subset_left D.switched_support_subset
    (Finset.disjoint_union_left.mpr ⟨hM, hW⟩)
