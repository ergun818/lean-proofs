...
theorem exists_treeCopy_of_sourceBudgets
    ...
    (hside : ∀ s j i, i ∈ family s j → componentReservoirSide sourceP ((branchForest sourceP).owner
      i) = s)
    (hselectedSide : ∀ i ∈ selected, componentReservoirSide sourceP ((branchForest sourceP).owner i)
      = 0)
    ...
    (hmarks : (∑ i ∈ selected, ((branchMarks sourceP i).card : ℝ)) ≤ (epsilon α : ℝ) *
      W.clusterSize)
  ...
  obtain ⟨A⟩ := exists_terminalCutPrefix W Q S O P (branchForest sourceP).branches (branchForest
    sourceP).owner
  ...
  exact ⟨terminalTreeCopy W Q S O P hT sourceP selected kinds allocation family locate hcover
    hlocate
