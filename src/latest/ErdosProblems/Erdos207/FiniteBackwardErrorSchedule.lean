...
theorem exists_finite_backward_exponents (ell target : ℕ) (required : Fin ell → ℕ → ℕ) :
    ∃ exponent : Fin (ell + 1) → ℕ,
      target ≤ exponent (Fin.last ell) ∧
        ∀ i : Fin ell,
          exponent i.succ + 1 ≤ exponent i.castSucc ∧
            required i (exponent i.succ + 1) ≤ exponent i.castSucc :=
  by
    induction ell with
    | zero => exact ⟨fun _ ↦ target, le_rfl, fun i ↦ Fin.elim0 i⟩
    | succ ell ih =>
      obtain ⟨tail, hlast, hstep⟩ := ih (fun i ↦ required i.succ)
      let head := max (tail 0 + 1) (required 0 (tail 0 + 1))
      refine ⟨Fin.cases head tail, ?_, ?_⟩
      · simpa only [show Fin.last (ell + 1) = (Fin.last ell).succ from rfl,
          Fin.cases_succ] using hlast
      · intro i
...
theorem polynomial_error_budget_step (t B error : ℝ≥0) (incoming outgoing cutoff : ℕ)
    (ht : 2 ≤ t) (hB : 1 ≤ B) (hincoming : outgoing + 1 ≤ incoming)
    (hcutoff : outgoing + 1 ≤ cutoff) (herror : error ≤ 1 / t ^ cutoff) :
    B / t ^ incoming + error ≤ B / t ^ outgoing :=
  by
    have ht1 : 1 ≤ t :=
      (by
              norm_num :
    ...
    have herr : error ≤ 1 / t ^ (outgoing + 1) :=
      herror.trans (one_div_le_one_div_of_le (pow_pos ht0 _) (pow_le_pow_right₀ ht1 hcutoff))
    have hnum : B + 1 ≤ B * t :=
      by
        nlinarith only [hB, mul_le_mul_of_nonneg_left ht (show 0 ≤ B from zero_le)]
    calc
