...
theorem card_le_degree_add_one_of_adj_ne {V : Type*} [Fintype V]
...
theorem complete_pair_card_product_bound {V : Type*} [Fintype V]
...
theorem natCard_edges_mono {V : Type*} [Finite V]
...
theorem natCard_edges_induce_le {V : Type*} [Finite V]
    (G : SimpleGraph V) (S : Finset V) : Nat.card (G.induce (S : Set V)).edgeSet ≤ Nat.card
      G.edgeSet := by
  classical
  let := Fintype.ofFinite V
