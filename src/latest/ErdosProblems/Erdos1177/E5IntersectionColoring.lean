...
theorem disjoint_of_edgeIntersectionColoring_same
  ...
  intro x hex hfx
  exact hc (edgeIntersectionGraph_adj_of_common_vertex H hne hex hfx) hcol
...
theorem edgeMark_mem (H : Hypergraph W) (htri : H.IsTripleSystem) (e : H.edges) :
  ...
  exact Classical.choose_spec
    ( Set.nonempty_of_ncard_ne_zero ( by rw [ htri e.1 e.2 ] ; norm_num ) )
...
theorem properColoring_of_edgeIntersectionColoring
  ...
  classical
  let d : W → Fin q → Prop := fun v i => ∃ e : H.edges, c e = i ∧ edgeMark H htri e = v
  refine ⟨fun v i => decide (d v i), ?_⟩
  intro e he
  obtain ⟨v, hv, hne⟩ : ∃ v ∈ e, v ≠ edgeMark H htri ⟨e, he⟩ := by
    exact Set.exists_ne_of_one_lt_ncard (by rw [htri e he]; norm_num) _
  refine ⟨edgeMark H htri ⟨e, he⟩, edgeMark_mem H htri ⟨e, he⟩, v, hv, ?_⟩
  intro heq
  have hmarked : d (edgeMark H htri ⟨e, he⟩) (c ⟨e, he⟩) := ⟨⟨e, he⟩, rfl, rfl⟩
  have hvmarked : decide (d v (c ⟨e, he⟩)) = true := by
    have hcoord := congrFun heq (c ⟨e, he⟩)
    exact hcoord.symm.trans (decide_eq_true hmarked)
  obtain ⟨f, hfc, hfv⟩ := of_decide_eq_true hvmarked
  have hfe : f = ⟨e, he⟩ := by
    by_contra hfe
    exact Set.disjoint_left.mp (disjoint_of_edgeIntersectionColoring_same H hc hfc hfe)
      (hfv ▸ edgeMark_mem H htri f) hv
  exact hne (hfv.symm.trans (congrArg (edgeMark H htri) hfe))
...
theorem colorableBy_of_edgeIntersectionColorable_fin
  ...
  · convert! this using 1;
  · refine ⟨ ?_, ?_ ⟩;
    · exact fun w => Classical.choice ( Cardinal.eq.1 ( by simp +decide [ Cardinal.mk_fintype ] ) )
        ( ULift.up ( d w ) );
...
theorem not_edgeIntersectionColorable_of_not_signatureColorable
  ...
  contrapose! hno;
  have := properColoring_of_edgeIntersectionColoring H htri q hno.choose hno.choose_spec;
  aesop;
...
theorem edgeIntersection_not_finitely_colorable_of_unbounded
  ...
  exact hunbounded ( Fintype.card ( Fin q → Bool ) )
    ( Fintype.card_pos_iff.mpr ⟨ fun _ => Bool.true ⟩ )
    ⟨ fun w => Fintype.equivFin _ ( d w ), by intro e he; specialize hd e he; aesop; ⟩
