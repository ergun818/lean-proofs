...
theorem zmod_const_of_succ_eq {m : ℕ} [NeZero m] {α : Type*} (s : ZMod m → α)
  ...
  have h_ind : ∀ k : ℕ, s (k : ZMod m) = s 0 := by
    ...
    | succ k ih =>
      simpa only [Nat.cast_add, Nat.cast_one] using (h (k : ZMod m)).symm.trans ih
...
theorem zmod_succ_eq_of_unique_change {m : ℕ} [NeZero m] (s : ZMod m → Bool)
  ...
  have hT_card : T.card ≤ 1 := by
    exact Finset.card_le_one.mpr fun x hx y hy => h x y ( Finset.mem_filter.mp hx |>.2 )
      ( Finset.mem_filter.mp hy |>.2 );
  have hT_even : Even T.card := by
    have hT_even : ∑ i : ZMod m, (if s i then 1 else 0 : ZMod 2) + ∑ i : ZMod m, (if s (i + 1) then
      1 else 0 : ZMod 2) = 0 := by
      erw [ Equiv.sum_comp ( Equiv.addRight 1 ) fun i => if s i = true then 1 else 0 ] ; ring_nf;
      grind;
    have hT_even : ∑ i : ZMod m, (if s i ≠ s (i + 1) then 1 else 0 : ZMod 2) = ∑ i : ZMod m, (if s i
      then 1 else 0 : ZMod 2) + ∑ i : ZMod m, (if s (i + 1) then 1 else 0 : ZMod 2) := by
      rw [ ← Finset.sum_add_distrib ] ;
      congr ;
      ext i ;
      by_cases hi : s i <;> by_cases hi' : s ( i + 1 ) <;> simp +decide [ hi, hi' ] ;
    simp_all +decide only [ne_eq, Finset.sum_boole, ite_not, Finset.sum_ite, Finset.sum_const_zero,
      Finset.sum_const, nsmul_eq_mul, mul_one, zero_add];
    rw [ ZMod.natCast_eq_zero_iff ] at hT_even ; exact even_iff_two_dvd.mpr hT_even;
  exact absurd hT_even ( by
    rw [ show T.card = 1 by exact le_antisymm hT_card ( Finset.card_pos.mpr ⟨ i, by aesop ⟩ ) ] ;
    decide )
...
theorem zmod_even_of_alt {m : ℕ} [NeZero m] (s : ZMod m → ZMod 2)
  ...
  specialize h_m_even m; simp_all +decide only [CharP.cast_eq_zero, left_eq_add] ;
...
theorem FTS.intrinsic_reduce_iff (F : FTS) :
  ...
  · obtain ⟨hlin, hbr, hev⟩ := h;
    ...
    · intro ed
      obtain ⟨w, hw⟩ := hbr
        ⟨ed.1.map (Function.Embedding.subtype (fun x => ¬ F.Isolated x)),
          FTS.reduce_edge_map_mem ed.2⟩
      use ⟨w, by
        grind +suggestions⟩
      generalize_proofs at *;
      simp_all +decide only [IsBridgeInc, OnBergeCycle, not_exists, not_and, not_or, and_self_left,
        Subtype.forall, Finset.mem_map, Function.Embedding.subtype_apply, Subtype.exists,
        exists_and_right, exists_eq_right, not_false_eq_true, exists_true_left, true_and];
      intro c i hi;
      specialize hw;
      have := hw.2 ( BergeCycle.ofReduce c ) i;
      simp_all +decide only [BergeCycle.ofReduce, Function.Embedding.subtype_apply, forall_const] ;
      exact ⟨ fun h => this.1 <| by simpa using! congr_arg Subtype.val h,
        fun h => this.2 <| by simpa using! congr_arg Subtype.val h ⟩;
    · exact fun c => hev ( BergeCycle.ofReduce c );
  · refine ⟨ ?_, ?_, ?_ ⟩;
    · intro e₁ he₁ e₂ he₂ hne;
      convert! h.1 ( Finset.subtype ( fun x => ¬ F.Isolated x ) e₁ ) ( FTS.subtype_edge_mem he₁ )
        ( Finset.subtype ( fun x => ¬ F.Isolated x ) e₂ ) ( FTS.subtype_edge_mem he₂ ) _ using 1;
      ...
      · contrapose! hne;
        convert! congr_arg
          ( fun s => s.map ( Function.Embedding.subtype ( fun x => ¬ F.Isolated x ) ) ) hne using 1
          <;> simp +decide only [Finset.subtype_map];
        · grind +suggestions;
        · exact Eq.symm ( Finset.filter_true_of_mem fun x hx => FTS.not_isolated_of_mem he₂ hx );
    · intro ed
      obtain ⟨w, hw⟩ : ∃ w ∈ Finset.subtype (fun x => ¬F.Isolated x) ed.val, IsBridgeInc F.reduce w
        ⟨Finset.subtype (fun x => ¬F.Isolated x) ed.val, FTS.subtype_edge_mem ed.prop⟩ := by
        convert! h.2.1 ⟨ _, FTS.subtype_edge_mem ed.2 ⟩ using 1;
      refine ⟨ w, ?_, ?_ ⟩ <;> simp_all +decide only [Finset.mem_subtype, IsBridgeInc, OnBergeCycle,
        not_exists, not_and, not_or, true_and];
      intro c i hi;
      specialize hw;
      have := hw.2.2 ( BergeCycle.toReduce c ) i;
      simp_all +decide only [BergeCycle.toReduce, forall_const] ;
...
theorem intrinsic_iso {F G : FTS} (h : FTS.Iso F G) (hF : F.IntrinsicObligatory) :
  ...
  · obtain ⟨ φ, hφ ⟩ := h;
    intro e₁ he₁ e₂ he₂ hne;
    have h_card : (Finset.map (Equiv.toEmbedding φ.symm) e₁ ∩ Finset.map (Equiv.toEmbedding φ.symm)
      e₂).card ≤ 1 := by
      have := hF.1 ( Finset.map φ.symm.toEmbedding e₁ ) ?_ ( Finset.map φ.symm.toEmbedding e₂ ) ?_
        ?_ <;> simp_all +decide [ Finset.map_map ];
  ...
  · intro ed;
    ...
    obtain ⟨ w', hw' ⟩ := hF.2.1 ⟨ Finset.map φ.symm.toEmbedding ed.val, by
      ...
      ext; simp +decide; ⟩
    generalize_proofs at *;
    refine ⟨ φ w', ?_, ?_ ⟩ <;> simp_all +decide only [Finset.mem_map_equiv, Equiv.symm_symm,
      IsBridgeInc, and_self_left, true_and];
    contrapose! hw';
    intro hw''; obtain ⟨ c, i, hi, hi' ⟩ := hw';
    use ⟨ c.m, c.hm, fun j => φ.symm ( c.v j ),
      fun j => ⟨ Finset.map φ.symm.toEmbedding ( c.e j |>.1 ), by
        convert! hφ _ |>.2 _ using 1;
        simp +decide [ Finset.map_map ] ⟩, by
        ...
        simp +decide [ c.mem_right ] ⟩ ; aesop;
  · intro c
    obtain ⟨φ, hφ⟩ := h
    have h_cycle : ∃ c' : BergeCycle F, c'.m = c.m := by
      refine ⟨ ⟨ c.m, c.hm, fun i => φ.symm ( c.v i ),
        fun i => ⟨ Finset.map φ.symm.toEmbedding ( c.e i |>.1 ), ?_ ⟩,
        ?_, ?_, ?_, ?_ ⟩, rfl ⟩
      · apply (hφ _).mpr
        simp [Finset.map_map]
      · intro i j hij
...
-/
theorem expansion_endpoints_adj {VJ : Type} [Fintype VJ] (J : SimpleGraph VJ)
...
theorem expansion_linear {VJ : Type} [Fintype VJ] [DecidableEq VJ] (J : SimpleGraph VJ)
  ...
  classical
  intro e₁ he₁ e₂ he₂ hne; simp_all +decide only [ne_eq] ;
  ...
  by_contra h_contra;
  have h_core : ∃ p q : VJ, p ≠ q ∧ Sum.inl p ∈ e₁ ∧ Sum.inl p ∈ e₂ ∧ Sum.inl q ∈ e₁ ∧ Sum.inl q ∈
    e₂ := by
...
theorem expansion_bridge {VJ : Type} [Fintype VJ] [DecidableEq VJ] (J : SimpleGraph VJ)
  ...
  classical
  obtain ⟨ e, he ⟩ := expansion_edge_cases J ed.2;
  use Sum.inr e;
  simp_all +decide only [Finset.mem_insert, reduceCtorEq, Finset.mem_singleton, or_true,
    IsBridgeInc, true_and] ;
  rintro ⟨ c, i, hi, hvi ⟩;
  rcases hvi with hvi | hvi <;> have := c.mem_left i <;> have := c.mem_right i <;>
    simp_all +decide only [Finset.ext_iff, Finset.mem_insert, Finset.mem_singleton, reduceCtorEq,
      or_true];
  · have := c.mem_right ( i - 1 ) ; simp_all +decide only [sub_add_cancel] ;
    have := expansion_private J ( c.e ( i - 1 ) |>.2 ) this; simp_all +decide only [
      Finset.mem_insert, reduceCtorEq, Finset.mem_singleton, or_true, Finset.ext_iff] ;
    have := c.einj ( show c.e ( i - 1 ) = c.e i from ?_ )
    · simp_all +decide only [implies_true, sub_eq_self, add_zero, reduceCtorEq, or_true]
      rcases c with ⟨ _ | _ | m, hm, v, e, vinj, einj, mem_left, mem_right ⟩ <;> cases this ;
      contradiction;
    · exact Subtype.ext <| Finset.ext fun x => by aesop;
  · have := c.mem_left ( i + 1 ) ;
    have := c.mem_right ( i + 1 ) ;
    simp_all +decide only ;
    have := c.einj ( show c.e ( i + 1 ) = c.e i from ?_ )
    · simp_all +decide only [reduceCtorEq, or_true, _root_.add_eq_left]
      rcases c with ⟨ _ | _ | m, hm, v, e, vinj, einj, mem_left, mem_right ⟩ <;> cases this ;
      contradiction;
...
theorem expansion_cycle_core {VJ : Type} [Fintype VJ] [DecidableEq VJ] (J : SimpleGraph VJ)
  ...
  obtain ⟨e, he⟩ : ∃ e : {x : Sym2 VJ // x ∈ J.edgeFinset}, c.v i = Sum.inr e := by
    cases h : c.v i <;> aesop;
  have h_eq : (c.e i).1 = {Sum.inl (Quot.out e.1).1, Sum.inl (Quot.out e.1).2, Sum.inr e} ∧ (c.e (i
    - 1)).1 = {Sum.inl (Quot.out e.1).1, Sum.inl (Quot.out e.1).2, Sum.inr e} := by
    have := c.mem_left i; have := c.mem_right ( i - 1 ) ; simp_all +decide [ Finset.ext_iff ] ;
    have := expansion_private J ( c.e i |>.2 ) ‹_›;
    have := expansion_private J ( c.e ( i - 1 ) |>.2 ) ‹_›;
    aesop;
  cases c ; simp_all +decide only [not_exists, reduceCtorEq, not_false_eq_true, implies_true];
  rename_i k hk₁ hk₂ hk₃ hk₄ hk₅ hk₆;
  have := @hk₄ ( i - 1 ) i; simp_all +decide only [ZMod, sub_eq_self] ;
  cases ‹ℕ› <;> simp_all +decide only [one_ne_zero, imp_false, Fin.one_eq_zero_iff,
    Nat.add_eq_right];
  · contradiction;
...
theorem expansion_even {VJ : Type} [Fintype VJ] [DecidableEq VJ] (J : SimpleGraph VJ)
  ...
  classical
  obtain ⟨ col, hcol ⟩ := hJ;
  -- Define `a : ZMod c.m → VJ` by `a i := (expansion_cycle_core J c i).choose`, so `c.v i = Sum.inl
  -- (a i)` for all `i`.
  ...
  have hadj : ∀ i, J.Adj (a i) (a (i + 1)) := by
    ...
    have hxy : a i ≠ a (i + 1) := by
      have := c.vinj.ne ( show i ≠ i + 1 from by
        have := Fact.mk ( show 1 < c.m from c.hm ) ; simp +decide ; ) ;
...
theorem disjUnion_berge_side {F G : FTS} (c : BergeCycle (F.disjUnion G)) :
  ...
  cases h : s 0 <;> simp_all +decide only;
...
theorem disjUnion_berge_edges_left {F G : FTS} (c : BergeCycle (F.disjUnion G))
  ...
  · obtain ⟨ d, hd, h ⟩ := ‹_›;
    ...
    obtain ⟨ a, ha ⟩ := hv;
    have := c.mem_left i;
    simp_all +decide [ Finset.mem_map ] ;
    tauto
...
theorem disjUnion_linear {F G : FTS} (hF : F.Linear) (hG : G.Linear) :
  ...
  classical
  intro e₁ he₁ e₂ he₂ hne;
  rcases disjUnion_edge_cases he₁ with ( ⟨ d₁, hd₁, rfl ⟩ | ⟨ d₁, hd₁, rfl ⟩ ) <;> rcases
    disjUnion_edge_cases he₂ with ( ⟨ d₂, hd₂, rfl ⟩ | ⟨ d₂, hd₂, rfl ⟩ ) <;>
      simp_all +decide only [ne_eq, Finset.map_inj];
  · rw [ ← Finset.map_inter ];
    rw [ Finset.card_map ] ; exact hF _ hd₁ _ hd₂ hne;
  · rw [ Finset.card_eq_zero.mpr ] <;> norm_num;
    simp +decide only [Finset.ext_iff, Finset.mem_inter, Finset.mem_map, Finset.notMem_empty,
      iff_false, not_and, not_exists, forall_exists_index, and_imp, forall_apply_eq_imp_iff₂];
...
theorem disjUnion_even {F G : FTS} (hF : ∀ c : BergeCycle F, Even c.m)
  ...
  classical
  rcases disjUnion_berge_side c with hv | hv;
...
theorem disjUnion_bridge {F G : FTS} (hF : F.IntrinsicObligatory)
  ...
  · obtain ⟨ w, hw₁, hw₂ ⟩ := hF.2.1 ⟨ d, hd ⟩;
    refine ⟨ Sum.inl w, ?_, ?_ ⟩ <;> simp_all +decide only [IsBridgeInc, true_and, Finset.mem_map];
    · exact ⟨ w, hw₁, rfl ⟩;
    · refine ⟨ ⟨ w, hw₁, rfl ⟩, ?_ ⟩;
      rintro ⟨ c, i, hi, hi' ⟩;
      -- By `disjUnion_berge_side`, `c` is all-left or all-right. If it were the right case, `c.v i`
      -- and `c.v (i+1)` would be `Sum.inr _`, contradicting `hi'` (which says one of them is
      -- `Sum.inl w`, and `Sum.inl w ≠ Sum.inr _`). So `c` must be all-left.
      have h_all_left : ∀ i, ∃ a : F.V, c.v i = Sum.inl a := by
        rcases disjUnion_berge_side c with hv | hv;
        · exact hv;
        · cases hv i ; cases hv ( i + 1 ) ; aesop;
      refine hw₂ ⟨ BergeCycle.ofDisjLeft c h_all_left ( disjUnion_berge_edges_left c h_all_left ),
        i, ?_, ?_ ⟩ <;> simp_all +decide only [BergeCycle.ofDisjLeft, Finset.map_inj,
          Subtype.mk.injEq];
      · grind +splitImp;
  ...
  · obtain ⟨ w, hw₁, hw₂ ⟩ := hG.2.1 ⟨ d, hd ⟩;
    refine ⟨ Sum.inr w, ?_, ?_ ⟩ <;> simp_all +decide only [IsBridgeInc, OnBergeCycle, not_exists,
      not_and, not_or, true_and, Finset.mem_map];
    · exact ⟨ w, hw₁, rfl ⟩;
    · refine ⟨ ⟨ w, hw₁, rfl ⟩, ?_ ⟩;
      intro c i hi;
      rcases disjUnion_berge_side c with hv | hv;
...
theorem amalgamate_cross_inter {F G : FTS} {x : F.V} {y : G.V}
    {dF : Finset F.V} {dG : Finset G.V} (_ : dG ∈ G.edges)
...
theorem amalgamate_berge_side {F G : FTS} {x : F.V} {y : G.V}
  ...
  classical
  -- By definition of $s$, we know that for any $i$, $s i$ is true if and only if there exists a $d
  -- \in F.edges$ such that $(c.e i).1 = d.map Function.Embedding.inl$.
  ...
  have h_change : ∀ i j, s i ≠ s (i + 1) → s j ≠ s (j + 1) → i = j := by
    ...
    have h_vertex : c.v (i + 1) = Sum.inl x ∧ c.v (j + 1) = Sum.inl x := by
      have h_vertex : ∀ i, s i ≠ s (i + 1) → c.v (i + 1) = Sum.inl x := by
        intros i hi; by_cases hi' : s i <;> simp_all +decide only [ne_eq, Bool.true_eq,
          Bool.not_eq_true, Bool.false_eq, Bool.not_eq_false] ;
        · obtain ⟨ dF, hdF, hdF' ⟩ := (by
          grind +splitImp : ∃ dF ∈ F.edges, (c.e i).1 = dF.map Function.Embedding.inl)
          obtain ⟨ dG, hdG, hdG' ⟩ := (by
            have := amalgamate_edge_cases ( c.e ( i + 1 ) |>.2 ) ; aesop;
            : ∃ dG ∈ G.edges, (c.e (i + 1)).1 = dG.map (amalgEmbG F G x y));
          have := c.mem_right i;
          have := c.mem_left ( i + 1 ) ;
        ...
        · obtain ⟨ d₁, hd₁, hd₁' ⟩ := (by
          ...
          have := amalgamate_cross_inter hd₁
            ( show c.v ( i + 1 ) ∈ d₂.map Function.Embedding.inl from by aesop )
            ( show c.v ( i + 1 ) ∈ d₁.map ( amalgEmbG F G x y ) from by aesop ) ; aesop;
      exact ⟨ h_vertex i hi, h_vertex j hj ⟩;
    have := c.vinj ( show c.v ( i + 1 ) = c.v ( j + 1 ) from by aesop ) ; aesop;
  -- By `zmod_succ_eq_of_unique_change s hchange`, `∀ i, s i = s (i+1)`, and then
  -- `zmod_const_of_succ_eq s _` gives `s i = s 0` for all `i`.
  have h_const : ∀ i, s i = s 0 := by
    convert! zmod_const_of_succ_eq s ( zmod_succ_eq_of_unique_change s h_change ) using 1; all_goals
      exact ⟨ by linarith [ c.hm ] ⟩;
...
theorem amalgamate_linear {F G : FTS} (x : F.V) (y : G.V) (hF : F.Linear) (hG : G.Linear) :
  ...
  intro a ha b hb;
  obtain ⟨ d₁, hd₁, rfl ⟩ | ⟨ d₁, hd₁, rfl ⟩ := amalgamate_edge_cases he₁ <;> obtain ⟨ d₂, hd₂, rfl
    ⟩ | ⟨ d₂, hd₂, rfl ⟩ := amalgamate_edge_cases he₂ <;> simp_all +decide [ Finset.ext_iff ] ;
  · have := hF d₁ hd₁ d₂ hd₂ ( by aesop ) ; simp_all +decide [ Finset.card_le_one ] ;
    grind +suggestions;
  · have := amalgamate_cross_inter hd₂ ( show a ∈ d₁.map Function.Embedding.inl from by aesop )
      ( show a ∈ d₂.map ( amalgEmbG F G x y ) from by aesop ) ; have := amalgamate_cross_inter hd₂
      ( show b ∈ d₁.map Function.Embedding.inl from by aesop )
      ( show b ∈ d₂.map ( amalgEmbG F G x y ) from by aesop ) ; aesop;
  · have := amalgamate_cross_inter hd₁ ( show a ∈ d₂.map Function.Embedding.inl from by aesop )
      ( show a ∈ d₁.map ( amalgEmbG F G x y ) from by aesop ) ; have := amalgamate_cross_inter hd₁
      ( show b ∈ d₂.map Function.Embedding.inl from by aesop )
      ( show b ∈ d₁.map ( amalgEmbG F G x y ) from by aesop ) ; aesop;
...
theorem amalgamate_even {F G : FTS} (x : F.V) (y : G.V) (hF : ∀ c : BergeCycle F, Even c.m)
  ...
  classical
  rcases amalgamate_berge_side c with heF | heG;
  · convert! hF ( BergeCycle.ofEmbedding ( Function.Embedding.inl : F.V ↪ ( F.amalgamate G x y ).V )
      c ( amalgamate_berge_vertsF c heF ) heF ) using 1;
  · convert! hG
      ( BergeCycle.ofEmbedding ( amalgEmbG F G x y ) c ( amalgamate_berge_vertsG c heG ) heG ) using
      1
...
theorem amalgamate_FG_ne {F G : FTS} {x : F.V} {y : G.V} {dF : Finset F.V} {dG : Finset G.V}
  ...
  classical
  by_contra h_eq;
  obtain ⟨ b, hb, hb' ⟩ := Finset.exists_mem_ne
    ( by linarith [ FTS.card3 G dG hdG ] : 1 < Finset.card dG ) y ; have := Finset.mem_map.mp
    ( h_eq.symm ▸ Finset.mem_map.mpr ⟨ b, hb, rfl ⟩ ) ; simp_all +decide ;
...
theorem amalgamate_bridge {F G : FTS} (x : F.V) (y : G.V) (hF : F.IntrinsicObligatory)
  ...
  classical
  rcases amalgamate_edge_cases ed.2 with ( ⟨ d, hd, hde ⟩ | ⟨ d, hd, hde ⟩ ) <;>
    simp_all +decide only [ne_eq, Finset.mem_map, IsBridgeInc, and_self_left,
    exists_exists_and_eq_and];
  · obtain ⟨ w, hw₁, hw₂ ⟩ := hF.2.1 ⟨ d, hd ⟩;
    refine ⟨ w, hw₁, ?_ ⟩;
    rintro ⟨ c, i, hi, hi' ⟩;
    rcases amalgamate_berge_side c with heF | heG;
    · refine hw₂.2 ⟨ BergeCycle.ofEmbedding ( Function.Embedding.inl : F.V ↪ ( F.amalgamate G x y
        ).V ) c ( amalgamate_berge_vertsF c heF ) heF, i, ?_, ?_ ⟩ <;>
        simp_all +decide only [ne_eq];
      · apply Subtype.ext
        apply Finset.map_injective
          (Function.Embedding.inl : F.V ↪ (F.amalgamate G x y).V)
        exact (heF i).choose_spec.2.symm.trans ((congrArg Subtype.val hi).trans hde)
      · have hvi := (amalgamate_berge_vertsF c heF i).choose_spec
        have hvj := (amalgamate_berge_vertsF c heF (i + 1)).choose_spec
        exact hi'.imp (fun h => Sum.inl_injective (hvi.symm.trans h))
          (fun h => Sum.inl_injective (hvj.symm.trans h))
    · obtain ⟨ d', hd', hde' ⟩ := heG i; simp_all +decide only [Finset.ext_iff, Finset.mem_map,
      implies_true] ;
      have := amalgamate_FG_ne hd' ( show d.map Function.Embedding.inl = d'.map ( amalgEmbG F G x y
        ) from ?_ ) ;
      · aesop;
      · ext; simp only [ne_eq, Finset.mem_map, Function.Embedding.inl_apply];
        convert! hde' _ using 1;
  · obtain ⟨ w, hw₁, hw₂ ⟩ := hG.2.1 ⟨ d, hd ⟩;
    refine ⟨ w, hw₁, ?_ ⟩ ; contrapose! hw₂ ; simp_all +decide only [OnBergeCycle, IsBridgeInc,
      not_exists, not_and, not_or, true_and, not_forall, Decidable.not_not] ;
    obtain ⟨ c, i, hi, hi' ⟩ := hw₂; rcases amalgamate_berge_side c with heF | heG;
    · obtain ⟨ dF, hdF, hdF' ⟩ := heF i;
      have := Finset.mem_map.mp ( hdF' ▸ hi.symm ▸ hde.symm ▸ Finset.mem_map.mpr ⟨ w, hw₁, rfl ⟩ ) ;
      simp_all +decide;
      grind +suggestions;
    · refine ⟨ BergeCycle.ofEmbedding ( amalgEmbG F G x y ) c ( amalgamate_berge_vertsG c heG )
        heG, i, ?_, ?_ ⟩ <;> simp_all +decide [ BergeCycle.ofEmbedding ];
      · grind +revert;
