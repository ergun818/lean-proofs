...
theorem amalgamate_embeds_of_copies {W : Type u} {H : Hypergraph W} {F G : FTS}
  ...
  · -- By definition of amalgamation, split into the cases where the edge comes from F or G.
    intro e he
    simp only [FTS.amalgamate, ne_eq, Finset.mem_union, Finset.mem_image] at he;
    rcases he with ( ⟨ a, ha, rfl ⟩ | ⟨ a, ha, rfl ⟩ ) <;> simp +decide only [Finset.coe_map];
    · convert! hfe a ha using 1 ; ext ; aesop;
    · convert! hge a ha using 1;
      rw [← Set.image_comp]
      apply congrArg (fun h : G.V → W => h '' (a : Set G.V))
      funext b
      change Sum.elim f (fun b : {b : G.V // b ≠ y} => g b.1)
        (if h : b = y then Sum.inl x else Sum.inr ⟨b, h⟩) = g b
      by_cases hb : b = y
      · rw [dif_pos hb]
        exact hglue.trans (congrArg g hb.symm)
      · rw [dif_neg hb]
        rfl
...
theorem reroute_isolated {W : Type u} [Infinite W] {G : FTS} (g : G.V → W)
  ...
  have hcompl : Set.Infinite (Set.univ \ Bad) := Set.infinite_univ.sdiff hBad
...
theorem rootSetF_compl_Ffree {W : Type u} (H : Hypergraph W) {F : FTS} (x : F.V)
  ...
  obtain ⟨e0, he0, hxe0⟩ : ∃ e ∈ F.edges, x ∈ e := by
    by_contra hc; push Not at hc; exact hx hc
...
theorem class_Gfree {W : Type u} [Infinite W] {H : Hypergraph W}
  ...
  obtain ⟨v, hvC⟩ : ∃ v ∈ C, g y = v := by
    obtain ⟨e, he, hye⟩ : ∃ e ∈ G.edges, y ∈ e := by simpa [FTS.Isolated] using hy
    exact ⟨g y, (hg_e e he).2 ⟨y, hye, rfl⟩, rfl⟩
  obtain ⟨g', hg'inj, hg'0, hg'1⟩ : ∃ g' : G.V → W, Function.Injective g' ∧ (∀ b, ¬ G.Isolated b →
    g' b = g b) ∧ (∀ b, G.Isolated b → g' b ∉ Set.range (K v) ∧ g' b ∉ g '' {b' : G.V | ¬ G.Isolated
    b'}) := by
    apply reroute_isolated g hg_inj (Set.range (K v)) (Set.finite_range _);
  apply amalgamate_embeds_of_copies x y (K v) (hKinj v (hCB hvC.left))
    (hKe v (hCB hvC.left)) g' hg'inj (by
      intro e he
      have h_image : g' '' (↑e : Set G.V) = g '' (↑e : Set G.V) := by
        ext w; simp only [Set.mem_image, SetLike.mem_coe];
        constructor <;> rintro ⟨ x, hx, rfl ⟩ <;> use x <;> simp_all +decide only [FTS.Isolated,
          not_forall, Decidable.not_not, ne_eq, forall_exists_index,
          Set.mem_range, not_exists, Set.mem_image, Set.mem_ofPred_eq, not_and, true_and];
        · rw [ hg'0 x e he hx ];
      ...
      intro a b hab;
      by_cases hb : G.Isolated b <;> simp_all +decide only [ne_eq, Set.mem_range, not_exists,
        Set.mem_image, Set.mem_ofPred_eq, not_and, not_false_eq_true];
      have h_contra : g b ∈ C := by
        obtain ⟨ e, he ⟩ := not_forall.mp hb;
        simp +zetaDelta only [Classical.not_imp, Decidable.not_not] at *;
...
theorem amalgamate_obligatory_of_nonisolated {F G : FTS} (x : F.V) (y : G.V)
  ...
  | some c =>
    have hmemCc : ∀ w, w ∈ part ⁻¹' {some c} ↔ ∃ h : w ∈ RootSetF H F x, χ w = c := by
      ...
      · simp only [dif_neg hb]; exact
          ⟨fun h => absurd h (by simp), fun h => h.elim (fun hh => absurd hh hb)⟩
...
theorem amalgamate_symm_iso {F G : FTS} (x : F.V) (y : G.V) :
  ...
  -- Construct the equivalence map `φ : (F.amalgamate G x y).V ≃ (G.amalgamate F y x).V` by swapping
  ...
  refine ⟨ φ, fun e => ?_ ⟩;
  constructor <;> intro he <;> simp_all +decide only [FTS.amalgamate, ne_eq, Finset.mem_union,
    Finset.mem_image];
  · rcases he with ( ⟨ a, ha, rfl ⟩ | ⟨ a, ha, rfl ⟩ );
    ...
    · refine Or.inl ⟨a, ha, ?_⟩
      rw [Finset.map_map]
      apply congrArg (fun f : G.V ↪ (G.V ⊕ {a : F.V // a ≠ x}) => a.map f)
      apply Function.Embedding.ext
      intro b
      exact (φ.apply_symm_apply (Sum.inl b)).symm
  · rcases he with ( ⟨ a, ha, he ⟩ | ⟨ a, ha, he ⟩ );
    · refine Or.inr ⟨ a, ha, ?_ ⟩;
      convert! congr_arg ( Finset.map
        ( show ( G.V ⊕ { a // a ≠ x } ) ↪ ( F.V ⊕ { b // b ≠ y } ) from
          ⟨ φ.symm, by exact φ.symm.injective ⟩ ) ) he using 1
      · generalize_proofs at *;
        ext; simp [φ];
      · simp +decide [Finset.map_map]
    · refine Or.inl ⟨a, ha, ?_⟩
      apply Finset.map_injective φ.toEmbedding
      rw [← he, Finset.map_map]
      rfl
...
theorem amalgamate_obligatory_of_isolated {F G : FTS} (x : F.V) (y : G.V)
  ...
  -- Restriction of H off `S`: The hypergraph `H1 := ⟨{e | e ∈ H.edges ∧ e ⊆ Sᶜ}⟩` is uncountably
  -- chromatic by `restrict_uc htri huc hS` (this lemma is now available), and a triple system by
  -- `fun e he => htri e he.1`.
  ...
  -- Copy of F in H1: `obtain ⟨f0, hf0_inj, hf0_e⟩ := hF H1 hH1_tri hH1_uc`; for `e ∈ F.edges`,
  -- `hf0_e e he` gives `f0 '' ↑e ∈ H.edges` (`.1`) and `f0 '' ↑e ⊆ Sᶜ` (`.2`).
  obtain ⟨f0, hf0_inj, hf0_e⟩ := hF H1 hH1_tri hH1_uc;
  -- Reroute isolated vertices of `f0` off `S`: `obtain ⟨f1, hf1_inj, hf1_0, hf1_1⟩ :=
  -- reroute_isolated f0 hf0_inj S hS`, where `hf1_0 : ∀ a, ¬ F.Isolated a → f1 a = f0 a` and `hf1_1
  -- : ∀ a, F.Isolated a → f1 a ∉ S ∧ f1 a ∉ f0 '' {a' | ¬ F.Isolated a'}`.
  obtain ⟨f1, hf1_inj, hf1_0, hf1_1⟩ := reroute_isolated f0 hf0_inj S hS;
  have hf1_avoid : ∀ a, f1 a ∉ S := by
    intro a; by_cases ha : F.Isolated a <;> simp_all +decide only [Set.mem_image, Set.mem_ofPred_eq,
      not_exists, not_and, not_false_eq_true] ;
    obtain ⟨ e, he ⟩ := not_forall.mp ha;
    exact fun h =>
      by have := hf0_e e ( by tauto ) |>.2 ( Set.mem_image_of_mem _ ( by tauto : a ∈ e ) ) ; aesop;;
