...
theorem exists_signed_offset {p v n Y U : ℕ} (_hp : 0 < p)
  ...
  · obtain ⟨h, hh⟩ := hm.symm.dvd'
    ...
    have hhpos : 0 < h := by
      by_contra hzero
      have : h = 0 := by omega
      simp only [this, mul_zero, add_zero] at hform
      exact hnv hform
    have hhU : h < U := by
      by_contra hnot
...
theorem same_signed_witness_modEq {p q v n m U s : ℕ} (hs : s.Prime)
  ...
  have hmul : p * o.2.val ≡ q * o.2.val [MOD s] := by
    cases ho : o.1
    · have hpform : v = n + p * o.2.val := by
        simpa only [SignedOffsetWitness, ho, Bool.false_eq_true, if_false] using hn.2
      have hqform : v = m + q * o.2.val := by
        simpa only [SignedOffsetWitness, ho, Bool.false_eq_true, if_false] using hm.2
      have hsum : n + p * o.2.val ≡ m + q * o.2.val [MOD s] := by rw [← hpform, ← hqform]
      exact hnm.add_left_cancel hsum
    · have hpform : n = v + p * o.2.val := by
        simpa only [SignedOffsetWitness, ho, if_true] using hn.2
      have hqform : m = v + q * o.2.val := by
        simpa only [SignedOffsetWitness, ho, if_true] using hm.2
