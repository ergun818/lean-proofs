...
theorem list_take_split {α : Type*} (l : List α) (a b : ℕ) (_h : a + b ≤ l.length) :
...
theorem count_take_full_split (w : List (Fin 4)) (k m : ℕ) (c : Fin 4)
  ...
  -- Apply the count_take_split_head lemma to split the count into the sum of the counts of the
  -- individual parts.
  have h_split : List.count c (applyKeranenG (List.take (m - 1) w)) = List.count c (applyKeranenG
    (List.take k w)) + List.count c (applyKeranenG (List.take (m - 1 - k) (List.drop k w))) := by
    rw [ show List.take ( m - 1 ) w = List.take k w ++ List.take ( m - 1 - k ) ( List.drop k w )
      from ?_, applyKeranenG_append ];
...
theorem map_sum_eq_weighted_count (l : List (Fin 4)) (f : Fin 4 → ℕ) :
  ...
  | cons x xs ih =>
    fin_cases x <;>
      simp +decide only [Fin.mk_one, Fin.isValue, List.map_cons, List.sum_cons,
        ne_eq, List.count_cons_of_ne, List.count_cons_self, Fin.reduceFinMk, Fin.zero_eta] <;>
      linarith!
...
theorem inner_count_bridge (w : List (Fin 4)) (r L : ℕ) (c : Fin 4)
  ...
  refine Eq.symm ?_
  have h_eq : 2 * ((applyKeranenG (w.take ((r + L) / 85))).count c + ((keranenG (w.get ⟨(r + L) /
    85, by
    ...
    ((keranenG (w.get ⟨w.length - 1, by
      exact Nat.pred_lt ( ne_bot_of_gt hm_ge )⟩)).take (r + 2 * L - 85 * (w.length - 1))).count c
        := by
      all_goals generalize_proofs at *;
      have h_eq : 2 * ((applyKeranenG w).take (r + L)).count c = ((applyKeranenG w).take r).count
        c + ((applyKeranenG w).take (r + 2 * L)).count c := by
        have h_eq : ((applyKeranenG w).take (r + L)).count c - ((applyKeranenG w).take r).count c
          = ((applyKeranenG w).take (r + 2 * L)).count c - ((applyKeranenG w).take (r + L)).count
          c := by
          have h_eq : ((applyKeranenG w).drop r |>.take L).count c = ((applyKeranenG w).drop (r +
            L) |>.take L).count c := by
        ...
        grind;
      have h_eq : ((applyKeranenG w).take (r + L)).count c = ((applyKeranenG (w.take ((r + L) /
        85))).count c) + ((keranenG (w.get ⟨(r + L) / 85, by
      ...
      have h_eq : ((applyKeranenG w).take (r + 2 * L)).count c = ((applyKeranenG (w.take (w.length
        - 1))).count c) + ((keranenG (w.get ⟨w.length - 1, by
        grind +splitImp⟩)).take (r + 2 * L - 85 * (w.length - 1))).count c := by
        all_goals generalize_proofs at *;
        have h_eq : (applyKeranenG w).take (r + 2 * L) = applyKeranenG (w.take (w.length - 1)) ++
          (keranenG (w.get ⟨w.length - 1, by
          grind +splitImp⟩)).take (r + 2 * L - 85 * (w.length - 1)) := by
          all_goals generalize_proofs at *;
          convert applyKeranenG_take_blocks w ( w.length - 1 ) ( r + 2 * L - 85 * ( w.length - 1 )
            ) _ _ using 1 <;> norm_num [ hspan.symm ];
  ...
  · have h_eq : (applyKeranenG (w.take ((r + L) / 85))).count c = ((keranenG (w.get ⟨0, by
    ...
    have h_eq : (applyKeranenG (w.take (w.length - 1))).count c = ((keranenG (w.get ⟨0, by
      linarith⟩)).count c) + ((applyKeranenG (w.drop 1 |>.take ((r + L) / 85 - 1))).count c) +
        ((keranenG (w.get ⟨(r + L) / 85, by
      assumption⟩)).count c) + ((applyKeranenG (w.drop ((r + L) / 85 + 1) |>.take (w.length - 2 -
        (r + L) / 85))).count c) := by
      all_goals generalize_proofs at *;
      convert count_take_full_split w ( ( r + L ) / 85 ) w.length c hk ( by omega ) ( by omega )
        using 1
    generalize_proofs at *;
    unfold boundaryDelta;
    unfold sliceParikhCount;
    unfold cumParikhCount;
    split_ifs <;> simp_all +decide only [List.get_eq_getElem, Nat.reduceMul, List.take_zero,
      List.count_nil, CharP.cast_eq_zero, sub_zero, List.drop_one, add_zero]
    · rw [show (2 * ((r + L) % 85) + 85000 - r) % 85 = 0 from ?_]
      · norm_num
        ring_nf
        rw [ show List.take 85 ( keranenG w[0] ) = keranenG w[0] from ?_, show List.take 85 (
        keranenG w[w.length - 1] ) = keranenG w[w.length - 1] from ?_ ] at * <;> norm_num at *;
        · rw [ show List.take 85 ( keranenG w[(r + L) / 85] ) = keranenG w[(r + L) / 85] from ?_ ]
          at *
          · grind;
        ...
        · exact le_of_eq ( keranenG_length _ );
      · omega;
    · rw [ show List.take 85 ( keranenG w[(r + L) / 85] ) = keranenG w[(r + L) / 85] from ?_, show
      List.take 85 ( keranenG w[0] ) = keranenG w[0] from ?_ ];
      · rw [ show ( 2 * ( ( r + L ) % 85 ) + 85000 - r ) % 85 = ( r + 2 * L - 85 * ( w.length - 1
        ) ) % 85 from ?_ ];
        · rw [ show ( r + 2 * L - 85 * ( w.length - 1 ) ) % 85 = ( r + 2 * L - 85 * ( w.length - 1
          ) ) from ?_ ];
          · grind;
          · omega;
        · omega;
