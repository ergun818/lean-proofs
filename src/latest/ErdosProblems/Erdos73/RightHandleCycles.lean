...
omit [Fintype V] in
theorem oddCyclePacking_of_right_hooks [Finite V] (F : ColumnHandleFamily S col (Fin k))
...
omit [Fintype V] in
theorem oddCyclePacking_of_right_series [Finite V] (F : ColumnHandleFamily S col (Fin k))
    (hc : 3 ≤ c)
    (hrow : ∀ i, (F.sourceNail i).val.1.val ≤ (F.targetNail i).val.1.val)
...
omit [Fintype V] in
theorem oddCyclePacking_of_right_nested [Finite V] (F : ColumnHandleFamily S col (Fin k))
    (hc : k + 1 < c)
    (hrow : ∀ i, (F.sourceNail i).val.1.val ≤ (F.targetNail i).val.1.val)
    (hs : ∀ i, 2 * (c - 1) ≤ (F.sourceNail i).val.2.val)
    (ht : ∀ i, 2 * (c - 1) ≤ (F.targetNail i).val.2.val)
