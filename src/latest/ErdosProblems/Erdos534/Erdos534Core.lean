...
nontrivial gcd. -/
lemma one_lt_gcd_of_dvd_left {a b c : ℕ} (_ha : 0 < a) (hb : 0 < b)
...
lemma one_lt_gcd_pullGenerator_of_cross {r l g : ℕ} (hr : r.Prime)
    (hl0 : l ≠ 0) (_hg0 : g ≠ 0) (_hrg : r ∣ g) (hrl : ¬r ∣ l)
...
lemma primePowerReplace_inverse {p q n : ℕ} (hp : p.Prime) (_hq : q.Prime)
  ...
  have hfac : (primePowerReplace p q n).factorization p = k := by
    ...
    simp [k, hpq, Nat.factorization_eq_zero_of_not_dvd hpn]
...
prime `q` to `p`. -/
lemma primePowerReplace_two_mul {p q : ℕ} (_hp : p.Prime) (hq : q.Prime)
    (_hpq : p ≠ q) (hq2 : q ≠ 2) :
...
lemma greatestPrimeFactor_le_of_dvd {a b : ℕ} (_ha : a ≠ 0) (hb : b ≠ 0)
    (hab : a ∣ b) :
...
theorem E_nu_bound (x : ℝ) (hx : x ≥ 0) : 0 ≤ E ν x ∧ E ν x ≤ 1 := by
  have : ∀ y, 0 ≤ y → y < 30 → 0 ≤ E ν y ∧ E ν y ≤ 1 := fun y hy0 hy30 ↦ by
    simp only [E_nu_expand, Nat.floor_eq_zero.mpr (by linarith : y / 30 < 1),
...
theorem a_bound : a ∈ Set.Icc 0.92129 0.92130 := by
  ...
  constructor <;> nlinarith [Real.log_two_gt_d9, Real.log_two_lt_d9,
    Real.log_three_gt_d9, Real.log_three_lt_d9, Real.log_five_gt_d9, Real.log_five_lt_d9]
...
theorem psi_num (x : ℝ) (hx : x > 0) (hx2 : x ≤ 30) : ψ x ≤ 1.1 * x := by
  ...
  have primes : Λ 2 = log 2 ∧ Λ 3 = log 3 ∧ Λ 5 = log 5 ∧ Λ 7 = log 7 ∧
      Λ 11 = log 11 ∧ Λ 13 = log 13 ∧ Λ 17 = log 17 ∧ Λ 19 = log 19 ∧
      Λ 23 = log 23 ∧ Λ 29 = log 29 := by
    split_ands <;> exact vonMangoldt_apply_prime (by decide)
  have lam_pow : (Λ (2 ^ 2) = log 2) ∧ Λ (2 ^ 3) = log 2 ∧ Λ (2 ^ 4) = log 2 ∧
      Λ (3 ^ 2) = log 3 ∧ Λ (3 ^ 3) = log 3 ∧ Λ (5 ^ 2) = log 5 := by
    split_ands <;> rw [vonMangoldt_apply_pow (by norm_num)] <;> (try rw [primes.1]) <;> simp_all
  have comps : Λ 6 = 0 ∧ Λ 10 = 0 ∧ Λ 12 = 0 ∧ Λ 14 = 0 ∧ Λ 15 = 0 ∧ Λ 18 = 0 ∧
      Λ 20 = 0 ∧ Λ 21 = 0 ∧ Λ 22 = 0 ∧ Λ 24 = 0 ∧ Λ 26 = 0 ∧ Λ 28 = 0 ∧ Λ 30 = 0 := by
  ...
  all_goals
    ...
    try grw [Real.log_two_lt_d9]
    try grw [Real.log_three_lt_d9]
    try grw [Real.log_five_lt_d9]
    try grw [log_seven_lt]
    try grw [log_eleven_lt]
    try grw [log_thirteen_lt]
    try grw [log_seventeen_lt]
    try grw [log_nineteen_lt]
    try grw [log_twentyThree_lt]; try grw [log_twentyNine_lt]
    norm_num

theorem psi_upper (x : ℝ) (hx : 30 ≤ x) :
    ψ x ≤ 6 * a * x / 5 + (log (x/5) / log 6) * (5 * log x - 5) := by
  ...
  have telescope (n : ℕ) :
      ψ x - ψ (x / 6 ^ n) = ∑ i ∈ Ico 0 n, (ψ (x / 6 ^ i) - ψ (x / 6 ^ (i + 1))) := by
    induction n with
    | zero => simp
    | succ n hn =>
  ...
  have bound (n : ℕ) (h : ∀ i < n, 30 ≤ x / 6 ^ i) :
      ψ x - ψ (x / 6 ^ n) ≤ ∑ i ∈ Ico 0 n, (a * x / 6 ^ i + 5 * log (x / 6 ^ i) - 5) := by
    rw [telescope]
    ...
    · trans (30 * 6 ^ (log (x / 5) / log 6 - 1))
      · rw [← rpow_natCast, Nat.cast_sub]
        ...
        · apply Nat.le_floor
          ...
          linarith
      · exact rpow_key.le
  simp_rw [← add_sub, sum_add_distrib, sum_const, Nat.Ico_zero_eq_range,
    Finset.card_range, nsmul_eq_mul, tsub_le_iff_right] at bound
  apply bound.trans
...
lemma oneBasedPrime_prime {k : ℕ} (_hk : 1 ≤ k) :
...
lemma erased_supportSignature_prod_erase_mul {N r m : ℕ} {S : Finset ℕ}
    (_hrS : r ∈ S) (hSscope : S ⊆ coreScope N r)
    (hSprime : ∀ p ∈ S, p.Prime)
    (hm : m ∈ sifted (signatureForbidden N r S) (m + 1)) :
...
lemma ordCompl_signatureProd_eq_prod_erase {r : ℕ} {S : Finset ℕ}
...
lemma biUnion_relaxedSupportFibers_subset_pull_remainder
  ...
  · obtain ⟨g, hgR, hgProd⟩ := hS.2.1
    ...
    rw [ordCompl_signatureProd_eq_prod_erase hrS hSprime] at hcompl
...
noncomputable def expandedPrime {T : Finset ℕ} {p U : ℕ}
    (_hsmall : ∀ r, r.Prime → r < p → r ∈ T)
    (hexpand : PrimeIntervalExpansion T p)
    (m : ↥(sifted T U)) (_hm1 : m.1 ≠ 1) : ℕ :=
...
lemma sieveDensity_nonneg {T : Finset ℕ} (_hT : ∀ p ∈ T, p.Prime) :
    0 ≤ sieveDensity T := by
...
lemma candidate_two_card {N : ℕ} (_hN : 2 ≤ N) (hTwo : 2 ∈ N.primeFactors) :
