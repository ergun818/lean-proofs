...
lemma opensOver_sub_τ (a : α) : opensOver a ⊆ {S | (τ a).IsOpen S} := by
  ...
  · -- principalOpen a: preimage of {True} under (a ∈ ·)
    change @IsOpen (Set α) (@TopologicalSpace.induced (Set α) Prop (fun S => a ∈ S) Prop_space)
      (principalOpen a)
...
lemma τ_le_product_topology (a : α) :
  ...
  change (Pi.topologicalSpace : TopologicalSpace (Set α)) ≤ τ a
...
@[simp] lemma principalOpenFinset_insert [DecidableEq α] {F : Finset α} {a : α} :
  ...
  ext x
  simp only [principalOpenFinset, Set.mem_ofPred_eq, Set.mem_inter_iff,
    Finset.coe_singleton, Set.singleton_subset_iff, Finset.coe_insert, Set.insert_subset_iff]

lemma principalOpenFinset_eq_inter (F : Finset α) :
  ...
  | insert _ _ =>
    ...
    ext x
    simp only [principalOpenFinset, principalOpen, Set.mem_ofPred_eq,
...
@[simp] lemma coPrincipalOpenFinset_insert [DecidableEq α] {F : Finset α} {a : α} :
  ...
  ext x
  simp only [coPrincipalOpenFinset, Set.mem_ofPred_eq, Set.mem_inter_iff,
    Finset.coe_singleton, Set.singleton_subset_iff, Finset.coe_insert, Set.insert_subset_iff]

lemma coPrincipalOpenFinset_eq_inter (F : Finset α) :
  ...
  | insert _ _ =>
    ...
    rw [Finset.inf_insert, ← ih]
    ext x
    simp only [coPrincipalOpenFinset, coPrincipalOpen, Set.mem_ofPred_eq,
      Finset.coe_insert, Set.insert_subset_iff]
    simp [Set.mem_inter_iff]

lemma isClopen_principalOpenFinset (F : Finset α) :
...
lemma isClopen_coPrincipalOpenFinset (F : Finset α) :
...
lemma product_topology_generate_from :
  ...
  change Pi.topologicalSpace = generateFrom (⋃ a : α, opensOver a)
...
@[simp] lemma principalOpen_mem_standardBasis {a : α} :
  ...
  simp only [standardBasis, Set.mem_union, Set.mem_ofPred_eq, Set.mem_singleton_iff]
  left
  classical
...
lemma intersection_standardBasis_nonempty' {p_ins p_out : Finset α}
  ...
  simp only [principalOpenFinset, coPrincipalOpenFinset, Set.mem_inter_iff, Set.mem_ofPred_eq]
...
lemma standard_basis_reindex {T : Set (Set α)} {p_ins p_out : Finset α}
  ...
  simp only [Finset.coe_union, Finset.coe_image, Set.mem_sInter, Set.mem_union,
    ...
    principalOpenFinset, coPrincipalOpenFinset, Set.mem_ofPred_eq]
  constructor
  · intro h
...
lemma intersection_standardBasis_nonempty {T : Set (Set α)} {p_ins p_out : Finset α}
    ...
    ¬⋂₀ (↑(p_ins.image principalOpen ∪ p_out.image coPrincipalOpen) :
      Set (Set (Set α))) = ∅ := by
  classical
  rw [standard_basis_reindex (H_eq := H_eq) (_H := H)]
  subst H_eq
...
lemma is_topological_basis_standardBasis :
  ...
  · -- exists_subset_inter
    ...
    simp only [standardBasis, Set.mem_union, Set.mem_ofPred_eq, Set.mem_singleton_iff] at ht₁ ht₂
    ...
    · rcases ht₂ with ⟨p_ins₂, p_out₂, H_eq₂, H_disj₂⟩ | rfl
      · classical
        ...
        · simp only [standardBasis, Set.mem_union, Set.mem_ofPred_eq, Set.mem_singleton_iff]
          ...
          · exact ins₁_out₂_disjoint ⟨hx_ins₁, hx_out₁⟩ ⟨hx_ins₂, hx_out₂⟩ H_disj₁ H_disj₂ ha_ins₁
              ha_out₂
          · exact ins₁_out₂_disjoint ⟨hx_ins₂, hx_out₂⟩ ⟨hx_ins₁, hx_out₁⟩ H_disj₂ H_disj₁ ha_ins₂
              ha_out₁
          · exact Finset.disjoint_left.mp H_disj₂ ha_ins₂ ha_out₂
  ...
  · -- eq_generateFrom
    ...
    · rw [le_generateFrom_iff_subset_isOpen]
      ...
      · simp only [Set.mem_ofPred_eq]; exact isOpen_empty
