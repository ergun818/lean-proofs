...
noncomputable def blockValue (τ : MaskChoice K) (ω : IntegerParameters K) (f : Label K) (i : ℕ) :
    ℕ :=
...
theorem encoded_block_decomposition (τ : MaskChoice K) (ω : IntegerParameters K) (f : Label K)
    ...
    ∃ lower upper : ℕ, 2 * lower < blockPlace K 0 i ∧
      encoded K τ ω f = lower + blockPlace K 0 i *
        (blockValue K τ ω f i + blockRadix K i * upper) := by
  obtain ⟨t, ht⟩ := encoded_prefix_decomposition K τ ω f (i + 1) (by omega)
  have hone :
      digitBlocks K τ ω f i 1 = blockDigits K i (τ i) (labelResidue K f i) (ω.block f i) := by
...
theorem blockTrace_pair (τ : MaskChoice K) (ω : IntegerParameters K) (f g : Label K)
    ...
    blockTrace K (encoded K τ ω f + encoded K τ ω g) i = blockValue K τ ω f i +
      blockValue K τ ω g i := by
...
theorem blockTrace_long_short (τ : MaskChoice K) (ω : IntegerParameters K) (f g : Label K)
  ...
  have he' : encoded K τ ω f + encoded K τ ω g =
      (lower + encoded K τ ω g) + blockPlace K 0 i *
        (blockValue K τ ω f i + blockRadix K i * upper) := by
    rw [he]
    omega
