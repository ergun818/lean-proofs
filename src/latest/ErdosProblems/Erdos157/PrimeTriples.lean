...
theorem primeSetProduct_monic {n : ℕ} (s : Finset (PrimeDegree K n)) :
    (primeSetProduct s).Monic := by
...
theorem prime_dvd_primeSetProduct_iff {n : ℕ} (f : PrimeDegree K n) (s : Finset (PrimeDegree K n)) :
  ...
  constructor
  · intro hdvd
    obtain ⟨g, hg, hfg⟩ := f.2.prime.dvd_finsetProd_iff
      (fun g : PrimeDegree K n => g.1.1) |>.mp hdvd
...
omit [DecidableEq K] [Fintype K] in
theorem PrimeTriple.product_monic {n : ℕ} (T : PrimeTriple K n) : T.product.Monic := by
  exact primeSetProduct_monic T.1

omit [DecidableEq K] [Fintype K] in
theorem PrimeTriple.product_natDegree {n : ℕ} (T : PrimeTriple K n) :
    T.product.natDegree = 3 * n := by
...
theorem PrimeTriple.eq_of_shared_factor_residue {n : ℕ} (g : K[X])
  ...
  have hlt :
      (primeSetProduct (U.1.erase f) - primeSetProduct (V.1.erase f)).natDegree < g.natDegree := by
