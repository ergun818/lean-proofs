...
theorem omegaCount_le_log2 (m : ℕ) : omegaCount m ≤ Nat.log 2 m := by
  ...
  by_cases hm : m = 0;
  · aesop;
  · exact Nat.le_log_of_pow_le ( by
      decide ) ( by
      exact le_trans ( by
        simpa using! Finset.prod_le_prod' fun p hp => Nat.Prime.two_le <|
          Nat.prime_of_mem_primeFactors hp ) <| Nat.le_of_dvd ( Nat.pos_of_ne_zero hm ) <|
...
theorem val_le_Ht (t : ℕ) : Real.log (t + 2) / (2 * Real.log 2) ≤ (Ht t : ℝ) := by
  classical
  exact le_trans ( Nat.le_ceil _ ) ( by simp +decide only [Ht, Nat.cast_add, Nat.cast_ofNat,
      le_add_iff_nonneg_right, Nat.ofNat_nonneg] )
...
theorem hr_le_Ht {r t : ℕ} (h : r ≤ t) : hr r ≤ Ht t := by
  ...
  · refine Nat.div_le_of_le_mul ?_;
    ...
    have h_log : (Nat.log 2 t : ℝ) ≤ Real.log (t + 2) / Real.log 2 := by
      rw [ le_div_iff₀ ( Real.log_pos ( by
        norm_num ) ), ← Real.log_pow ] ; exact Real.log_le_log ( by
        positivity ) ( by
        norm_cast; linarith [ Nat.pow_log_le_self 2 ( by
          linarith : t ≠ 0 ) ] ) ;
    rw [ show ( Real.log ( t + 2 ) / ( 2 * Real.log 2 ) ) = ( Real.log ( t + 2 ) / Real.log 2 ) / 2
        by
      ring ];
    exact Nat.le_of_lt_succ <| by
      rw [ ← @Nat.cast_lt ℝ ] ; push_cast; linarith [ Nat.le_ceil ( Real.log ( t + 2 ) / Real.log 2
        / 2 ), show ( Nat.log 2 r : ℝ ) ≤ Nat.log 2 t from mod_cast Nat.log_mono_right h ] ;
...
theorem sr_le_Lt {r t : ℕ} (hr2 : 2 ≤ r) (hrt : r ≤ t) :
  ...
  have h_log_bound : (Nat.log 2 r : ℝ) ≤ Real.log r / Real.log 2 ∧ Real.log r ≤ Real.log (t + 2) :=
      by
    exact ⟨ by
      rw [ le_div_iff₀ ( Real.log_pos ( by
        norm_num ) ) ] ; erw [ ← Real.log_pow ] ; exact Real.log_le_log ( by
  ...
  · rw [ le_div_iff₀ ( Real.log_pos one_lt_two ) ] at h_log_bound;
    unfold sr; norm_num; nlinarith [ show ( Real.log 2 :ℝ ) > 0 by
      positivity, show ( Real.log 4 :ℝ ) = 2 * Real.log 2 by
      rw [ ← Real.log_rpow ] <;> norm_num, Real.log_le_log ( by
      norm_num ) ( show ( t:ℝ ) + 2 ≥ 4 by
      norm_cast; linarith ) ] ;
  · positivity
...
theorem one_add_log2_le (x : ℝ) (hx : 3 ≤ x) :
  ...
  -- From Fact 2, we have `Nat.log 2 ⌊x⌋₊ ≤ Real.log x / Real.log 2`. So `1 + Nat.log 2 ⌊x⌋₊ ≤ 1 +
  ...
  have h_bound : 1 + Nat.log 2 ⌊x⌋₊ ≤ 1 + Real.log x / Real.log 2 := by
    ...
    rw [ le_div_iff₀ ( Real.log_pos ( by
      norm_num ) ), ← Real.log_pow ] ; exact Real.log_le_log ( by
      positivity ) ( by
      exact le_trans ( mod_cast Nat.pow_log_le_self _ <| Nat.ne_of_gt <| Nat.floor_pos.mpr <| by
        linarith ) <| Nat.floor_le <| by
        positivity ) ;
  -- We'll use that $Real.log (3 * x) ≥ 2$ since $x ≥ 3$.
  have h_log_ge_two : 2 ≤ Real.log (3 * x) := by
    rw [ Real.le_log_iff_exp_le ( by positivity ) ];
    have := Real.exp_one_lt_d9.le ; norm_num1 at * ; rw [ show ( 2 : ℝ ) = 1 + 1 by
      norm_num, Real.exp_add ] ; nlinarith [ Real.add_one_le_exp 1 ];
  rw [ Real.log_mul ( by positivity ) ( by positivity ) ] at *;
  have := Real.log_two_gt_d9 ; norm_num at * ; nlinarith [ Real.log_le_sub_one_of_pos zero_lt_two,
      Real.log_le_log ( by
    positivity ) ( by
    linarith : ( 3 : ℝ ) ≥ 2 ), mul_div_cancel₀ ( Real.log x ) ( ne_of_gt ( Real.log_pos one_lt_two
      ) ) ]
...
theorem one_le_Lt (t : ℕ) (ht : 2 ≤ t) : 1 ≤ Real.log (t + 2) := by
  ...
  exact Real.le_log_iff_exp_le ( by
    positivity ) |>.2 ( by
    linarith [ Real.exp_one_lt_d9.le, show ( t : ℝ ) ≥ 2 by
      norm_cast ] )
...
theorem comb_G_le (x : ℝ) (hx : 3 ≤ x) (t r m : ℕ) (hr2 : 2 ≤ r) (hrt : r ≤ t)
  ...
  have h_simplified : Real.log (2 ^ (hr r) * (1 + omegaCount m) ^ (sr r) * 2 ^ (hr r * sr r) * (1 +
      Nat.log 2 ⌊x⌋₊) ^ (hr r)) ≤ 100 * ((Real.log (t + 2)) ^ 2 + Real.log (t + 2) * Real.log (
      Real.log (3 * x))) := by
    -- Applying the properties of logarithms and exponentials, we can simplify the inequality to:
    have h_simplified : (hr r : ℝ) * Real.log 2 + (sr r : ℝ) * Real.log (1 + omegaCount m) + (hr r *
        sr r : ℝ) * Real.log 2 + (hr r : ℝ) * Real.log (1 + Nat.log 2 ⌊x⌋₊) ≤ 100 * ((Real.log (t +
        2)) ^ 2 + Real.log (t + 2) * Real.log (Real.log (3 * x))) := by
      ...
      have h_subst : (hr r : ℝ) * Real.log 2 + (sr r : ℝ) * (2 * Real.log (Real.log (3 * x))) + (hr
          r * sr r : ℝ) * Real.log 2 + (hr r : ℝ) * (2 * Real.log (Real.log (3 * x))) ≤ (c + c^2) *
          (Real.log (t + 2))^2 + 4 * c * Real.log (t + 2) * Real.log (Real.log (3 * x)) := by
        nlinarith [ mul_le_mul_of_nonneg_left hlog2 ( show 0 ≤ ( hr r : ℝ ) by
          positivity ), mul_le_mul_of_nonneg_left hlog2 ( show 0 ≤ ( sr r : ℝ ) by
          positivity ), mul_le_mul_of_nonneg_left hlog2 ( show 0 ≤ c * Real.log ( t + 2 ) by
          positivity ), mul_le_mul_of_nonneg_left hlog2 ( show 0 ≤ c ^ 2 * Real.log ( t + 2 ) ^ 2 by
          positivity ) ];
      ...
      nlinarith [ mul_le_mul_of_nonneg_left hc_approx.le ( sq_nonneg ( Real.log ( t + 2 ) ) ),
          mul_le_mul_of_nonneg_left hc_approx.le ( mul_nonneg ( Real.log_nonneg ( show ( t:ℝ ) + 2 ≥
          1 by
        linarith ) ) hLL ) ];
    convert! h_simplified using 1 ; rw [ Real.log_mul, Real.log_mul, Real.log_mul ] <;> first
      | positivity
      | norm_num;
  exact le_trans ( by
    rw [ Real.exp_log ( by
      positivity ) ] ; norm_cast ) ( Real.exp_le_exp.mpr h_simplified )
...
theorem fixed_prefix_exp :
  ...
  · obtain ⟨ n₀, hn₀ ⟩ := h;
    ...
    · refine le_trans ?_ ( mul_le_mul_of_nonneg_right ( pow_le_pow_right₀ ( mod_cast
          Nat.one_le_iff_ne_zero.mpr <| ?_ ) <| hr_le_Ht hrt ) <| by
        positivity );
      · convert! fixed_prefix_card_le x t r m hx hr2 hrt using 1;
        norm_cast ; ring_nf;
      · exact ne_of_gt <| Finset.card_pos.mpr ⟨ 1, Nat.one_mem_divisors.mpr <| by
          intro h; simp_all +decide only [fixedFiber, Nat.div_eq_zero_iff, mem_filter, mem_Icc] ;
          exact absurd ( hn₀.2.2.2.resolve_left ( by
                  ...
                  exact ne_of_gt ( Nat.pos_of_ne_zero ( by
                    exact fun h => by
                      have := Qr_dvd n₀ r ( by
                        linarith ) ( by
                        linarith ) ( by
                        linarith ) ; aesop ) ) ) ) ) ) ) ) ) ) ( not_lt_of_ge ( Nat.le_of_dvd ( by
...
theorem pnth1_mem (n : ℕ) (h : 1 ≤ omegaCount n) : pnth n 1 ∈ n.primeFactors := by
  classical
  unfold pnth; simp +decide only [ge_iff_le, tsub_self, List.getD_eq_getElem?_getD,
      Nat.mem_primeFactors, ne_eq] at h ⊢;
  rcases x : n.primeFactors.sort ( fun x1 x2 => x2 ≤ x1 ) with ( _ | ⟨ a, _ | ⟨ b, l ⟩ ⟩ ) <;>
      simp_all +decide only [one_le_card, Nat.nonempty_primeFactors, not_false_eq_true,
          getElem?_neg, Option.getD_none, isUnit_iff_eq_one, IsUnit.dvd, true_and, false_and,
          List.length_cons, List.length_nil, zero_add, Order.lt_one_iff, getElem?_pos,
          List.getElem_cons_zero, Option.getD_some, Order.lt_add_one_iff, zero_le];
...
noncomputable def oneprimeSet (x : ℝ) (t m : ℕ) : Finset ℕ := by
  classical
  exact (Finset.Icc 1 ⌊x⌋₊).filter (fun n => n ∈ Acal ∧ omegaCount n = t ∧ n / pnth n 1 = m)
...
theorem oneprime_card_le (x : ℝ) (t m : ℕ) (ht : 2 ≤ t) :
  ...
  classical
  set f : ℕ → ℕ × ℕ := fun n => (Dwit n (pnth n 1), List.idxOf (pnth n 1) ((Dwit n (pnth n 1) -
      1).primeFactors.sort (· ≤ ·))) with hf_def;
  have h_mapsTo : ∀ n ∈ oneprimeSet x t m, f n ∈ m.divisors ×ˢ Finset.range (1 + Nat.log 2 ⌊x⌋₊) :=
      by
    intro n hn
    ...
    · have h_coprime : Nat.Coprime (Dwit n (pnth n 1)) (pnth n 1) := by
        apply Nat.Coprime.symm; exact (by
          ...
          exact Nat.prime_of_mem_primeFactors ( pnth1_mem n ( by
            linarith [ Finset.mem_filter.mp hn ] ) )) (by
          exact Nat.dvd_of_mem_primeFactors ( pnth1_mem n ( by
            linarith [ Finset.mem_filter.mp hn ] ) )) (by
          exact Nat.ne_of_gt ( Finset.mem_Icc.mp ( Finset.mem_filter.mp hn |>.1 ) |>.1 ))
          exact Nat.Prime.coprime_iff_not_dvd ( by
            exact Nat.prime_of_mem_primeFactors ( pnth1_mem n ( by
              linarith [ Finset.mem_filter.mp hn ] ) ) ) |>.2 this);
      have h_div : Dwit n (pnth n 1) ∣ n := by
        apply (Dwit_spec n (pnth n 1) (by
        exact Finset.mem_filter.mp hn |>.2.1) (by
        exact Nat.prime_of_mem_primeFactors ( pnth1_mem n ( by
          linarith [ Finset.mem_filter.mp hn |>.2.2.1 ] ) )) (by
        exact Nat.dvd_of_mem_primeFactors ( pnth1_mem n ( by
          linarith [ Finset.mem_filter.mp hn |>.2.2.1 ] ) )) (by
      ...
      have h_div_m : Dwit n (pnth n 1) ∣ m * pnth n 1 := by
        ...
        exact Nat.dvd_of_mem_primeFactors ( pnth1_mem n ( by
          linarith [ Finset.mem_filter.mp hn |>.2.2.1 ] ) );
      exact h_coprime.dvd_of_dvd_mul_right h_div_m;
    · intro hm; simp_all +decide only [oneprimeSet, Nat.div_eq_zero_iff, mem_filter, mem_Icc] ;
      have := pnth1_mem n ( by linarith ) ; simp_all +decide only [Nat.mem_primeFactors, ne_eq] ;
      exact hn.2.2.2.elim ( fun h => by
        aesop ) fun h => by
        linarith [ Nat.le_of_dvd ( Nat.pos_of_ne_zero this.2.2 ) this.2.1 ] ;
    · refine lt_of_lt_of_le ( List.idxOf_lt_length_iff.mpr ?_ ) ?_;
      · have hn_info := Finset.mem_filter.mp hn
        ...
        have hpn := pnth1_mem n (by omega)
        have hp := Nat.prime_of_mem_primeFactors hpn
        have hspec := Dwit_spec n (pnth n 1) hn_info.2.1 hp
          (Nat.dvd_of_mem_primeFactors hpn) (Nat.ne_of_gt hn_bounds.1)
        apply (Finset.mem_sort (· ≤ ·)).mpr
        refine Nat.mem_primeFactors.mpr ⟨hp, ?_, Nat.sub_ne_zero_of_lt hspec.2.1⟩
        apply Nat.dvd_of_mod_eq_zero
        rw [← Nat.mod_add_div (Dwit n (pnth n 1)) (pnth n 1), hspec.2.2]
      ...
      · have h_Dwit_le_floor : Dwit n (pnth n 1) - 1 ≤ ⌊x⌋₊ := by
          ...
          exact Nat.sub_le_of_le_add <| by
            linarith [ show n ≤ ⌊x⌋₊ from Finset.mem_Icc.mp ( Finset.mem_filter.mp hn |>.1 ) |>.2 ]
              ;
        have h_prime_factors_card : (Dwit n (pnth n 1) - 1).primeFactors.card ≤ Nat.log 2 (Dwit n (
            pnth n 1) - 1) := by
          exact omegaCount_le_log2 _;
        simp +zetaDelta only [length_sort, ge_iff_le] at *;
        exact h_prime_factors_card.trans
          ((Nat.log_mono_right h_Dwit_le_floor).trans (by omega))
  have h_injOn : ∀ n n' : ℕ, n ∈ oneprimeSet x t m → n' ∈ oneprimeSet x t m → f n = f n' → n = n' :=
      by
    ...
    have h_n : n = m * pnth n 1 := by
      rw [← Finset.mem_filter.mp hn |>.2.2.2, Nat.div_mul_cancel (Nat.dvd_of_mem_primeFactors
          (hp_mem n hn))]
    have h_n' : n' = m * pnth n' 1 := by
      rw [← Finset.mem_filter.mp hn' |>.2.2.2, Nat.div_mul_cancel (Nat.dvd_of_mem_primeFactors
          (hp_mem n' hn'))]
...
theorem oneprime_exp :
  ...
  · by_cases hm : m = 0 <;> simp_all +decide only [tauCount, Nat.divisors_zero, card_empty,
      CharP.cast_eq_zero, card_eq_zero, Nat.divisors_eq_empty];
    rw [ Finset.card_eq_zero.mpr ] <;> norm_num [ oneprimeSet ];
    · positivity;
    · intro n hn₁ hn₂ hn₃ hn₄; exact ⟨ Nat.ne_of_gt <| Nat.pos_of_mem_primeFactors <| pnth1_mem n <|
          by
        linarith, Nat.le_of_dvd hn₁ <| Nat.dvd_of_mem_primeFactors <| pnth1_mem n <| by
        linarith ⟩ ;
  · -- Applying the bounds from `oneprime_card_le` and `one_add_log2_le`.
    have h_bound : (oneprimeSet x t m).card ≤ (tauCount m : ℝ) * Real.exp (2 * Real.log (Real.log (3
        * x))) := by
    ...
    · nlinarith only [ show 1 ≤ Real.log ( t + 2 ) from by
        rw [ Real.le_log_iff_exp_le ( by
          positivity ) ] ; exact Real.exp_one_lt_d9.le.trans ( by
          norm_num; linarith [ show ( t : ℝ ) ≥ 2 by
            norm_cast ] ), show 0 ≤ Real.log ( Real.log ( 3 * x ) ) from Real.log_nonneg <| by
        rw [ Real.le_log_iff_exp_le <| by
          positivity ] ; exact Real.exp_one_lt_d9.le.trans <| by
...
theorem canonical_fiber_bound :
  ...
  -- Let `F := (Finset.Icc 1 ⌊x⌋₊).filter (fun n => n ∈ Acal ∧ omegaCount n = t ∧ n / Qcanon n =
  -- m)` (this is the goal's set).
  set F := (Finset.Icc 1 ⌊x⌋₊).filter (fun n => n ∈ Acal ∧ omegaCount n = t ∧ n / Qcanon n = m);
  -- Let `Fr := F.filter (fun n => rhoIdx n = r)`.
  have hFfib : ∀ r ∈ Finset.Icc 1 t, ((F.filter (fun n => rhoIdx n = r)).card : ℝ) ≤ (tauCount m : ℝ
      ) ^ (Ht t) * Real.exp (max C₁ C₂ * ((Real.log (t + 2)) ^ 2 + Real.log (t + 2) * Real.log (
      Real.log (3 * x)))) := by
    intro r hr; by_cases hr1 : r = 1 <;> simp_all +decide only [mem_Icc, true_and] ;
    · refine le_trans ?_ ( le_trans ( hone x hx t m ht ) ?_ );
      · refine mod_cast Finset.card_le_card ?_;
        simp +contextual only [oneprimeSet, subset_iff, mem_filter, mem_Icc, and_imp];
        simp +zetaDelta only [mem_filter, mem_Icc, and_imp] at *;
        unfold Qcanon; aesop;
      · gcongr;
        · exact add_nonneg ( sq_nonneg _ ) ( mul_nonneg ( Real.log_nonneg ( by
            linarith ) ) ( Real.log_nonneg ( by
            rw [ Real.le_log_iff_exp_le ( by
              positivity ) ] ; exact Real.exp_one_lt_d9.le.trans ( by
              norm_num; linarith ) ) ) );
        · exact le_max_right _ _;
    · refine le_trans ?_ ( mul_le_mul_of_nonneg_left ( Real.exp_le_exp.mpr <|
          mul_le_mul_of_nonneg_right (le_max_left _ _) <| add_nonneg (sq_nonneg _ ) <| mul_nonneg
          ( Real.log_nonneg <| by
        norm_cast; linarith ) <| Real.log_nonneg <| ?_ ) <| by
        positivity );
      · refine le_trans ?_ ( hfix x hx t r m ( Nat.lt_of_le_of_ne hr.1 ( Ne.symm hr1 ) ) hr.2 );
        refine mod_cast Finset.card_le_card ?_;
        simp +contextual only [fixedFiber, subset_iff, mem_filter, mem_Icc, and_imp];
        simp +zetaDelta only [mem_filter, mem_Icc, and_imp] at *;
        unfold Qcanon; aesop;
      · exact Real.le_log_iff_exp_le ( by
          positivity ) |>.2 ( by
  ...
  have hFfib_sum : ((F).card : ℝ) ≤ (t : ℝ) * (tauCount m : ℝ) ^ (Ht t) * Real.exp (max C₁ C₂ * ((
      Real.log (t + 2)) ^ 2 + Real.log (t + 2) * Real.log (Real.log (3 * x)))) := by
    have hFfib_sum : ((F).card : ℝ) = ∑ r ∈ Finset.Icc 1 t, ((F.filter (fun n => rhoIdx n = r)).card
        : ℝ) := by
      rw_mod_cast [ ← Finset.card_biUnion ];
      · congr with n ; simp +decide only [mem_biUnion, mem_Icc, mem_filter,
  ...
  have h_bound : (t : ℝ) * Real.exp (max C₁ C₂ * ((Real.log (t + 2)) ^ 2 + Real.log (t + 2) *
      Real.log (Real.log (3 * x)))) ≤ Real.exp ((max C₁ C₂ + 1) * ((Real.log (t + 2)) ^ 2 + Real.log
      (t + 2) * Real.log (Real.log (3 * x)))) := by
    rw [ ← Real.log_le_log_iff ( by
      positivity ) ( by
      positivity ), Real.log_mul ( by
      positivity ) ( by
      positivity ), Real.log_exp, Real.log_exp ];
    nlinarith only [ show Real.log t ≤ Real.log ( t + 2 ) by
      exact Real.log_le_log ( by
        positivity ) ( by
        linarith ), show Real.log ( t + 2 ) ≥ 1 by
      exact Real.le_log_iff_exp_le ( by
        positivity ) |>.2 <| by
        exact Real.exp_one_lt_d9.le.trans <| by
          norm_num; linarith [ show ( t : ℝ ) ≥ 2 by
            norm_cast ], show 0 ≤ Real.log ( Real.log ( 3 * x ) ) by
      exact Real.log_nonneg <| by
        rw [ Real.le_log_iff_exp_le <| by
          positivity ] ; exact Real.exp_one_lt_d9.le.trans <| by
          norm_num; linarith, show 0 ≤ max C₁ C₂ by
      positivity ];
  exact hFfib_sum.trans ( by
    convert! mul_le_mul_of_nonneg_left h_bound ( by
      positivity : 0 ≤ ( tauCount m : ℝ ) ^ Ht t ) using 1 ; ring )
...
theorem omega_Qcanon_le (n : ℕ) (h2 : 2 ≤ omegaCount n) :
  ...
  · -- Since `rhoIdx n = 1`, we have `Qcanon n = pnth n 1`.
    ...
    have homegaCount_pnth : omegaCount (pnth n 1) ≤ 1 := by
      have hp := Nat.prime_of_mem_primeFactors (pnth1_mem n (by omega))
      simp only [omegaCount, hp.primeFactors, Finset.card_singleton, le_refl]
