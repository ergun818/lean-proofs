...
theorem homog (n : ℕ) (S0 : Finset ℕ) (i : ℕ) (a b : ℕ)
  ...
  · unfold Siter at ha hb;
    ...
    · grind;
    · exact le_antisymm ( Finset.sup_le fun x hx => WithBot.coe_le_coe.mpr <| Finset.le_max' _ _
        hx ) ( Finset.le_sup ( f := WithBot.some ) <| Finset.max'_mem _ h_nonempty );
...
theorem Dwit_spec (n p : ℕ) (hn : n ∈ Acal) (hp : p.Prime) (hpn : p ∣ n) (hn0 : n ≠ 0) :
  ...
  classical
  unfold Dwit;
  unfold Acal at hn; simp_all +decide only [SylowDivisor, Set.mem_ofPred_eq, ne_eq] ;
  obtain ⟨ d, hd₁, hd₂, hd₃ ⟩ := hn p hp hpn; have := Finset.min_of_mem ( show d ∈ { d ∈ n.divisors
      | 1 < d ∧ d % p = 1 } from by
    aesop ) ; simp_all +decide only [Finset.min, ENat.some_eq_natCast] ;
...
theorem valuation_bit (Q m D : ℕ) (hQ : Squarefree Q) (hm : m ≠ 0)
    ...
    (∀ q ∈ Q.primeFactors, (D.factorization q) - ((Nat.gcd D m).factorization q) ≤ 1) ∧
      D = Nat.gcd D m * ∏ q ∈ Q.primeFactors, q ^ ((D.factorization q) - ((Nat.gcd D
          m).factorization q)) := by
  classical
  refine ⟨ fun q hq => ?_, ?_ ⟩;
  · have := Nat.factorization_le_iff_dvd ( by
      aesop ) ( by
      aesop ) |>.2 hD ; simp_all +decide only [tsub_le_iff_right, ge_iff_le] ;
  ...
  · refine Nat.factorization_inj ?_ ?_ ?_;
    ...
    · ext p; by_cases hp : p.Prime <;> simp_all +decide only [ne_eq, Nat.gcd_eq_zero_iff,
        and_self, not_false_eq_true, prod_eq_zero_iff, Nat.mem_primeFactors, Nat.pow_eq_zero,
        ↓existsAndEq, zero_dvd_iff, and_not_self, Nat.factorization_eq_zero_of_not_prime,
        zero_tsub, not_true_eq_false, and_false, Nat.factorization_mul, not_and, implies_true,
        Nat.factorization_prod, Nat.factorization_pow, Finsupp.coe_add, Finsupp.coe_finsetSum,
        Finsupp.coe_smul, nsmul_eq_mul, Nat.cast_tsub, Pi.add_apply, Finset.sum_apply,
        Pi.mul_apply, Pi.sub_apply, Pi.natCast_apply, Nat.cast_id, mul_zero, sum_const_zero,
        add_zero] ;
      by_cases hpQ : p ∈ Q.primeFactors <;> simp_all +decide only [Nat.mem_primeFactors, ne_eq,
          true_and, not_false_eq_true, Nat.factorization_gcd, Finsupp.inf_apply, and_true,
          Nat.Prime.factorization, Finsupp.single_eq_of_ne', mul_zero, implies_true, and_self,
          not_true_eq_false, Nat.factorization_eq_zero_of_not_prime, min_self, tsub_self,
          Finsupp.single_eq_same, mul_one, Finset.sum_eq_single p, inf_le_left,
          add_tsub_cancel_of_le, not_and, Decidable.not_not];
      rw [ Finset.sum_eq_zero ] <;> simp_all +decide only [add_zero, left_eq_inf,
          Nat.mem_primeFactors, ne_eq, mul_eq_zero, and_imp];
      · have := Nat.factorization_le_iff_dvd (by aesop) (by aesop) |>.2 hD
        simp_all +decide only [ge_iff_le] ;
        by_cases hQ0 : Q = 0 <;> simp_all +decide only [dvd_zero, not_squarefree_zero, imp_false,
            ne_eq, not_false_eq_true, Nat.factorization_mul];
        replace this := this p; simp_all +decide [ Nat.factorization_eq_zero_of_not_dvd hpQ ] ;
      · intro x hx hxQ hQ0
        rw [hx.factorization, Finsupp.single_apply]
        aesop
...
noncomputable def Iplus (n r : ℕ) : Finset ℕ := by
  classical
  exact (Finset.range (hr r)).filter (fun i => rowPos n r i)

/-- Zero-row index set among the first `h_r` rows. -/
noncomputable def Izero (n r : ℕ) : Finset ℕ := by
  classical
  exact (Finset.range (hr r)).filter (fun i => ¬ rowPos n
      r i)
...
theorem topR_card (n r : ℕ) (hrn : r ≤ omegaCount n) : (topR n r).card = r := by
  classical
  -- The list `L := n.primeFactors.sort (· ≥ ·)` is `Nodup` (`Finset.sort_nodup`) and has length
  -- `n.primeFactors.card = omegaCount n` (`Finset.length_sort`).
...
theorem step_card_ge (n : ℕ) (S : Finset ℕ) :
  ...
  · unfold step;
    split_ifs;
    have h_card : (Finset.filter (fun r => r ∣ Dwit n (S.max' hS)) (S.erase (S.max' hS))).card + (
        Finset.filter (fun r => ¬r ∣ Dwit n (S.max' hS)) (S.erase (S.max' hS))).card = S.card - 1 :=
        by
...
theorem Siter_card_ge (n r i : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (hi : i ≤ Nat.log 2 r) :
  ...
  | zero =>
    norm_num at *
    rw [ show Siter n ( topR n r ) 0 = topR n r from rfl, topR_card n r hrn ] ; exact
...
theorem qi_strictAnti (n r i : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (hi : i + 1 < sr r) :
  ...
  classical
  -- By definition of `Siter`, we know that `Siter n (topR n r) (i + 1)` is a subset of `Siter n
  -- (topR n r) i` with at least one element removed.
  have h_subset : Siter n (topR n r) (i + 1) ⊆ (Siter n (topR n r) i).erase (qi n r i) := by
    unfold qi;
    unfold qsel;
    rw [ show Siter n ( topR n r ) ( i + 1 ) = step n ( Siter n ( topR n r ) i ) from rfl ] ;
        unfold step; split_ifs <;> simp_all +decide only [subset_iff, mem_erase, ne_eq,
            not_nonempty_iff_eq_empty, max_empty, notMem_empty, not_false_eq_true,
            erase_eq_of_notMem, Std.le_refl] ;
    split_ifs <;> simp_all +decide only [mem_filter, mem_erase, ne_eq, and_imp];
    · cases h : Finset.max ( Siter n ( topR n r ) i ) <;> simp_all +decide only [and_true];
      · simp_all +decide [ Finset.max ];
      · intro x hx₁ hx₂ hx₃; contrapose! hx₁; simp_all +decide only [max_eq_sup_coe] ;
        exact le_antisymm ( Finset.le_sup' ( fun x => x ) hx₂ ) ( Finset.sup'_le _ _ fun x hx =>
            WithBot.coe_le_coe.mp <| h ▸ Finset.le_sup ( f := WithBot.some ) hx );
    · cases h : ( Siter n ( topR n r ) i ).max <;> simp_all +decide only [and_true];
      · simp_all +decide [ Finset.max ];
      · intro x hx₁ hx₂ hx₃; contrapose! hx₁; simp_all +decide only [not_lt, max_eq_sup_coe] ;
        exact le_antisymm ( Finset.le_sup' ( fun x => x ) hx₂ ) ( Finset.sup'_le _ _ fun x hx =>
            WithBot.coe_le_coe.mp <| h ▸ Finset.le_sup ( f := WithBot.some ) hx );
  -- Since `Siter n (topR n r) (i + 1)` is a subset of `Siter n (topR n r) i` with at least one
  -- element removed, its maximum element must be strictly less than the maximum element of `Siter
  -- n (topR n r) i`.
  have h_max_lt : ∀ {S T : Finset ℕ}, S ⊆ T.erase (T.max.getD 0) → S.Nonempty → S.max.getD 0 <
      T.max.getD 0 := by
    ...
    have h_max_lt : ∀ x ∈ S, x < T.max.getD 0 := by
      ...
      cases h : T.max <;> simp_all +decide only [ne_eq, le_bot_iff, WithBot.coe_ne_bot,
          WithBot.coe_le_coe, gt_iff_lt]
  ...
  have := Siter_card_ge n r ( i + 1 ) hr1 hrn ( by
    linarith [ show sr r = 1 + Nat.log 2 r from rfl ] ) ; contrapose! this; aesop;
...
theorem Esel_lt_sr (n r j : ℕ) (hj : j ∈ Esel n r) : j < sr r := by
  ...
  · exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp ‹_› |>.1 ) ) ( by
      linarith [ two_hr_le_sr r ] );
...
theorem qi_injOn_Esel (n r : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) :
  ...
  by_contra h_neq;
  cases lt_or_gt_of_ne h_neq <;> have := Esel_lt_sr n r j hj <;> have := Esel_lt_sr n r k hk <;>
      simp_all +decide only [SetLike.mem_coe];
...
theorem Esel_card (n r : ℕ) : (Esel n r).card = hr r := by
  classical
  -- The cardinality of the union of $I_{\text{zero}}$ and $[h_r, h_r+h_+)$ is $h_r - h_+ + h_+ =
  -- h_r$.
  have h_union_card : (Izero n r).card + (Finset.Ico (hr r) (hr r + hplus n r)).card = hr r := by
    ...
    simpa [add_comm] using
      (Finset.card_filter_add_card_filter_not (s := Finset.range (hr r))
        (fun i => rowPos n r i))
  rw [ ← h_union_card, show Esel n r = Izero n r ∪ Finset.Ico ( hr r ) ( hr r + hplus n r ) from
      rfl, Finset.card_union_of_disjoint ];
  exact Finset.disjoint_left.mpr fun x hx₁ hx₂ => by
    linarith [ Finset.mem_Ico.mp hx₂, Finset.mem_range.mp ( Finset.mem_filter.mp hx₁ |>.1 ) ] ;
...
theorem Qr_squarefree (n r : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) :
  ...
  have h_prod_squarefree (s : Finset ℕ) (hs : ∀ p ∈ s, Nat.Prime p) : Squarefree (∏ p ∈ s, p) := by
    induction s using Finset.induction <;> simp_all +decide only [prod_empty, isUnit_iff_eq_one,
  ...
  · rw [ Finset.prod_image ];
    · rfl;
    · grind +suggestions;
  · rintro p hp; obtain ⟨ j, hj, rfl ⟩ := Finset.mem_image.mp hp; exact qi_prime n r j hr1 hrn (
      Esel_lt_sr n r j hj ) ;
...
theorem Qr_dvd (n r : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (_hn0 : n ≠ 0) :
  ...
  have h_prod_dvd : ∏ p ∈ (Finset.image (fun j => qi n r j) (Esel n r)), p ∣ ∏ p ∈ n.primeFactors, p
      := by
    apply_rules [ Finset.prod_dvd_prod_of_subset ];
    exact Finset.image_subset_iff.mpr fun j hj => qi_mem_primeFactors n r j hr1 hrn ( Esel_lt_sr n
        r j hj );
  rwa [ Finset.prod_image ( by
    exact fun x hx y hy hxy => by
      have := qi_injOn_Esel n r hr1 hrn; aesop ) ] at h_prod_dvd
...
theorem qi_mem_Siter (n r j : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (hj : j < sr r) :
  ...
  obtain ⟨q, hq⟩ : ∃ q, q ∈ Siter n (topR n r) j ∧ ∀ p ∈ Siter n (topR n r) j, p ≤ q := by
    refine ⟨ Finset.max' _ ?_, Finset.max'_mem _ _, fun p hp => Finset.le_max' _ p hp ⟩;
    have := Siter_card_ge n r j hr1 hrn ( by
      linarith [ show j ≤ Nat.log 2 r from Nat.le_of_lt_succ ( by
        linarith [ show sr r = 1 + Nat.log 2 r from rfl ] ) ] ) ; exact Finset.card_pos.mp (
        lt_of_lt_of_le ( by
      positivity ) this ) ;
  unfold qi;
  unfold qsel;
  ...
  · exact hq.1;
  · exact le_antisymm ( Finset.sup_le fun x hx => WithBot.coe_le_coe.mpr ( hq.2 x hx ) ) (
      Finset.le_sup ( f := WithBot.some ) hq.1 )
...
theorem zero_row_later_not_dvd (n r i j : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  -- Since `Siter n (topR n r) j ⊆ Siter n (topR n r) (i + 1)`, we have `qi n r j ∈ Siter n (topR n
  -- r) (i + 1)`.
  ...
  have h_mem : qi n r i = (Siter n (topR n r) i).max' hi_ne := by
    unfold qi qsel;
    rw [ Finset.max_eq_sup_coe ];
    rw [ show ( Siter n ( topR n r ) i ).sup WithBot.some = WithBot.some ( Finset.max' _ hi_ne )
        from ?_ ];
    · rfl;
    · exact le_antisymm ( Finset.sup_le fun x hx => WithBot.coe_le_coe.mpr <| Finset.le_max' _ _
        hx ) ( Finset.le_sup ( f := WithBot.some ) <| Finset.max'_mem _ hi_ne );
...
theorem pos_row_later_dvd (n r i j : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  have h_step : step n (Siter n (topR n r) i) = ((Siter n (topR n r) i).erase (qi n r i)).filter (
      fun z => z ∣ Dwit n (qi n r i)) := by
    unfold step;
    ...
    have h_max_eq : (Siter n (topR n r) i).max' h_max = qi n r i := by
      have h_max_eq : (Siter n (topR n r) i).max' h_max = (Siter n (topR n r) i).max.getD 0 := by
        rw [ Finset.max_eq_sup_coe ];
        rw [ show ( Siter n ( topR n r ) i ).sup WithBot.some = ↑ ( Finset.max' ( Siter n ( topR n
            r ) i ) h_max ) from ?_ ];
        · rfl;
        · exact le_antisymm ( Finset.sup_le fun x hx => WithBot.coe_le_coe.mpr <| Finset.le_max' _
            _ hx ) ( Finset.le_sup ( f := WithBot.some ) <| Finset.max'_mem _ h_max );
      exact h_max_eq;
    aesop;
...
theorem epsv_zero_of_ge (n r m i k : ℕ) (hn : n ∈ Acal) (hr2 : 2 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  by_cases h : qi n r k ∣ Dwit n ( qi n r i ) <;> simp_all +decide only [ne_eq, not_false_eq_true,
      Nat.factorization_eq_zero_of_not_dvd, zero_le];
  by_cases hik' : i < k;
  · exact absurd h ( zero_row_later_not_dvd n r i k ( by
      linarith ) hrn hik' ( by
      linarith [ Esel_lt_sr n r k hk ] ) hzero );
  · cases eq_or_lt_of_le hik <;> simp_all +decide only [Std.le_refl, lt_self_iff_false,
      not_false_eq_true, not_true_eq_false];
    exact absurd h ( Dwit_not_dvd n ( qi n r k ) hn ( qi_prime n r k ( by
      linarith ) hrn ( by
      linarith [ two_hr_le_sr r ] ) ) ( qi_dvd n r k ( by
      linarith ) hrn ( by
      linarith [ two_hr_le_sr r ] ) ) hn0 )
...
theorem zero_row_recovery (n r m i : ℕ) (hn : n ∈ Acal) (hr2 : 2 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  classical
  have h_val : Nat.gcd (Dwit n (qi n r i)) m * ∏ k ∈ Esel n r, qi n r k ^ epsv n r m i k = Dwit n (
      qi n r i) := by
    have := valuation_bit (Qr n r) m (Dwit n (qi n r i)) (Qr_squarefree n r (by
      linarith) (by
      linarith)) (by
    linarith [ Nat.div_pos ( show n ≥ Qr n r from Nat.le_of_dvd ( Nat.pos_of_ne_zero hn0 ) ( Qr_dvd
        n r ( by
      linarith ) ( by
      linarith ) ( by
      aesop ) ) ) ( Nat.pos_of_ne_zero ( show Qr n r ≠ 0 from Nat.ne_of_gt <| Nat.pos_of_ne_zero <|
        by
      intros h; have := Qr_squarefree n r ( by
        linarith ) ( by
        linarith ) ; aesop ) ) ]) (by
    rw [ hm, Nat.div_mul_cancel ];
    · exact Dwit_spec n ( qi n r i ) hn ( qi_prime n r i ( by
        linarith ) hrn ( by
        linarith [ hr2, hrn, hi, hr r, sr r, two_hr_le_sr r ] ) ) ( qi_dvd n r i ( by
        linarith ) hrn ( by
        linarith [ hr2, hrn, hi, hr r, sr r, two_hr_le_sr r ] ) ) hn0 |>.1;
    · exact Qr_dvd n r ( by linarith ) ( by linarith ) hn0) (by
    exact Nat.ne_of_gt ( Nat.pos_of_dvd_of_pos ( Dwit_spec n ( qi n r i ) hn ( qi_prime n r i ( by
      linarith ) ( by
      linarith ) ( by
      linarith [ show hr r ≤ sr r from Nat.div_le_self _ _ ] ) ) ( qi_dvd n r i ( by
      linarith ) ( by
      linarith ) ( by
      linarith [ show hr r ≤ sr r from Nat.div_le_self _ _ ] ) ) hn0 |>.1 ) ( Nat.pos_of_ne_zero hn0
        ) ));
...
theorem vn_eq (n r m k : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (hn0 : n ≠ 0)
  ...
  classical
  convert! congr_arg ( fun x : ℕ => x.factorization ( qi n r k ) ) ( n_eq_m_mul_Qr n r m hr1 hrn
      hn0 hm ) using 1;
  rw [ Nat.factorization_mul ] <;> norm_num [ hn0, hm, Qr_squarefree ];
  · rw [ vQr_eq_one n r k hr1 hrn hk ];
  · exact ⟨ Nat.ne_of_gt ( Nat.pos_of_dvd_of_pos ( Qr_dvd n r hr1 hrn hn0 ) ( Nat.pos_of_ne_zero
      hn0 ) ), Nat.le_of_dvd ( Nat.pos_of_ne_zero hn0 ) ( Qr_dvd n r hr1 hrn hn0 ) ⟩;
  · exact Finset.prod_ne_zero_iff.mpr fun x hx => Nat.ne_of_gt <| Nat.pos_of_mem_primeFactors <|
      qi_mem_primeFactors n r x hr1 hrn <| Esel_lt_sr n r x hx
...
theorem Dwit_prod_eq (n r m i : ℕ) (hn : n ∈ Acal) (hr2 : 2 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  · exact hm.symm ▸ Nat.ne_of_gt ( Nat.div_pos ( Nat.le_of_dvd ( Nat.pos_of_ne_zero hn0 ) ( Qr_dvd n
        r ( by
      linarith ) ( by
      linarith ) hn0 ) ) ( Nat.pos_of_ne_zero ( by
      exact Finset.prod_ne_zero_iff.mpr fun x hx => Nat.Prime.ne_zero ( qi_prime n r x ( by
        linarith ) ( by
        linarith ) ( by
        linarith [ Esel_lt_sr n r x hx ] ) ) ) ) );
  · convert! Dwit_spec n ( qi n r i ) hn ( qi_prime n r i ( by
      linarith ) ( by
      linarith ) hi ) ( qi_dvd n r i ( by
      linarith ) ( by
      linarith ) hi ) hn0 |> And.left using 1;
    rw [ hm, Nat.div_mul_cancel ( Qr_dvd n r ( by linarith ) ( by linarith ) hn0 ) ];
  · exact Nat.ne_of_gt ( Nat.pos_of_dvd_of_pos ( Dwit_spec n ( qi n r i ) hn ( qi_prime n r i ( by
      linarith ) ( by
      linarith ) hi ) ( qi_dvd n r i ( by
      linarith ) ( by
      linarith ) hi ) hn0 |>.1 ) ( Nat.pos_of_ne_zero hn0 ) );
  · rw [ Qr_primeFactors ];
    · rw [ Finset.prod_image ];
      · rfl;
      · exact qi_injOn_Esel n r ( by linarith ) ( by linarith );
...
theorem epsv_one_of_F (n r m i f : ℕ) (hn : n ∈ Acal) (hr2 : 2 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  have h_factorization_D : (Dwit n (qi n r i)).factorization (qi n r f) = 1 := by
    have h_factorization_Dwit : qi n r f ∣ Dwit n (qi n r i) := by
      ...
      linarith) (by
      exact Esel_lt_sr n r f ( Finset.mem_union_right _ ( Finset.mem_Ico.mpr ⟨ Finset.mem_Ico.mp (
          Finset.mem_filter.mp hf |>.1 ) |>.1, Finset.mem_Ico.mp ( Finset.mem_filter.mp hf |>.1 )
          |>.2 ⟩ ) )) (by
      exact Finset.mem_filter.mp hi |>.2);
    have h_factorization_Dwit_le : (Dwit n (qi n r i)).factorization (qi n r f) ≤ 1 := by
      have h_factorization_Dwit_le : (Dwit n (qi n r i)).factorization (qi n r f) ≤ (n.factorization
          (qi n r f)) := by
        have h_factorization_Dwit_le : Dwit n (qi n r i) ∣ n := by
          apply (Dwit_spec n (qi n r i) hn (qi_prime n r i (by linarith) (by linarith) (by
          exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
              Nat.div_le_self _ _ ))) (qi_dvd n r i (by
            linarith) (by
            linarith) (by
          exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
              Nat.div_le_self _ _ ))) hn0).left;
      ...
      linarith [ vn_eq n r m f ( by linarith ) ( by linarith ) hn0 hm ( by
        exact Finset.mem_union_right _ ( Finset.mem_Ico.mpr ⟨ by
  ...
  simp_all +decide only [ne_eq, Fsel, mem_filter, mem_Ico, Nat.left_eq_add];
  exact Nat.factorization_eq_zero_of_not_dvd ( fun h => hf.2 <| Nat.dvd_trans h <|
...
theorem positive_common_product (n r m i : ℕ) (hn : n ∈ Acal) (hr2 : 2 ≤ r)
  ...
  · rw [ ← Finset.prod_sdiff <| show Fsel n r m ⊆ Esel n r from ?_ ];
    · simp +decide only [Pval, Cval, mul_left_comm, mul_comm, mul_eq_mul_left_iff,
        mul_eq_mul_right_iff];
      exact Or.inl <| Or.inl <| Finset.prod_congr rfl fun x hx => by
        rw [ epsv_one_of_F n r m i x hn hr2 hrn hn0 hm hi hx ] ; norm_num;
    · exact fun x hx => Finset.mem_union_right _ <| Finset.mem_Ico.mpr ⟨ Finset.mem_Ico.mp (
        Finset.mem_filter.mp hx |>.1 ) |>.1, Finset.mem_Ico.mp ( Finset.mem_filter.mp hx |>.1 )
        |>.2 ⟩;
  · exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
      Nat.div_le_self _ _ )
...
theorem Cval_coprime (n r m i : ℕ) (hn : n ∈ Acal) (hr2 : 2 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  classical
  refine Nat.Coprime.symm ( Nat.Prime.coprime_iff_not_dvd ( qi_prime n r i ( by
    linarith ) ( by
    linarith ) ( by
    exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
        Nat.div_le_self _ _ ) |> lt_of_lt_of_le <| Nat.le_refl _ ) ) |>.2 ?_ );
  refine Nat.Prime.not_dvd_mul ( qi_prime n r i ( by linarith ) ( by linarith ) ( by
    exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
        Nat.div_le_self _ _ ) |> lt_of_lt_of_le <| Nat.le_refl _ ) ) ?_ ?_;
  · refine fun h => ?_;
    exact Dwit_not_dvd n ( qi n r i ) hn ( qi_prime n r i ( by linarith ) ( by linarith ) ( by
      exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
        ...
        linarith ) ( by
      exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
          Nat.div_le_self _ _ ) ) ) hn0 ( Nat.dvd_trans h ( Nat.gcd_dvd_left _ _ ) );
    ...
    · refine Nat.Coprime.prod_right fun k hk => ?_;
      ...
      · refine Nat.Coprime.pow_right _ ?_;
        have h_distinct : ∀ j k, j < sr r → k < sr r → j ≠ k → qi n r j ≠ qi n r k := by
          ...
          cases lt_or_gt_of_ne hneq
          · exact ne_of_gt (qi_lt_of_lt n r j k (by linarith) (by linarith) ‹_› ‹_›)
          · exact ne_of_lt (qi_lt_of_lt n r k j (by linarith) (by linarith) ‹_› ‹_›)
        exact Nat.coprime_iff_gcd_eq_one.mpr ( by
          have := h_distinct i k ( by
            linarith [ Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ), two_hr_le_sr r ] ) ( by
            linarith [ Esel_lt_sr n r k ( Finset.mem_sdiff.mp hk |>.1 ), two_hr_le_sr r ] ) hik;
            have := Nat.coprime_primes ( qi_prime n r i ( by
            linarith ) ( by
            linarith ) ( by
            linarith [ Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ), two_hr_le_sr r ] ) ) (
            qi_prime n r k ( by
            linarith ) ( by
            linarith ) ( by
            linarith [ Esel_lt_sr n r k ( Finset.mem_sdiff.mp hk |>.1 ), two_hr_le_sr r ] ) ) ;
            tauto );
    · exact qi_prime n r i ( by linarith ) ( by linarith ) ( by
        exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
            Nat.div_le_self _ _ ) |> lt_of_lt_of_le <| Nat.le_refl _ )
...
theorem qi_dvd_Dwit_sub_one (n r i : ℕ) (hn : n ∈ Acal) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  classical
  obtain ⟨d, hd⟩ := Dwit_spec n (qi n r i) hn (qi_prime n r i hr1 hrn hi) (qi_dvd n r i hr1 hrn
      hi) hn0;
  exact ⟨ Dwit n ( qi n r i ) / qi n r i, Nat.sub_eq_of_eq_add <| by
    linarith [ Nat.mod_add_div ( Dwit n ( qi n r i ) ) ( qi n r i ) ] ⟩
...
theorem Pval_lt_Rval (n r m : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  have ha_prime : Nat.Prime a := by
    ...
    exact Finset.mem_range.mp ( Finset.mem_filter.mp hiM.1 |>.1 ) |> lt_of_lt_of_le <| by
      linarith [ show hr r ≤ sr r from by
        linarith [ two_hr_le_sr r ] ] ;)
  have ha_ge_two : 2 ≤ a := by
    exact ha_prime.two_le;
  -- Now chain: `Pval n r m ≤ (a-1)^(Fsel).card ≤ (a-1)^(Iplus).card < a^(Iplus).card ≤ Rval n r`.
  have h_chain : Pval n r m ≤ (a - 1) ^ (Fsel n r m).card ∧ (a - 1) ^ (Fsel n r m).card ≤ (a - 1) ^
      (Iplus n r).card ∧ (a - 1) ^ (Iplus n r).card < a ^ (Iplus n r).card ∧ a ^ (Iplus n r).card ≤
      Rval n r := by
    refine ⟨ ?_, ?_, ?_, ?_ ⟩;
    ...
    · exact Nat.pow_le_pow_right ( Nat.sub_pos_of_lt ha_ge_two ) ( by
        simpa using! Finset.card_le_card ( show Fsel n r m ⊆ Finset.Ico ( hr r ) ( hr r + hplus n r
          ) from Finset.filter_subset _ _ ) |> le_trans <| by
          simp +decide [ hplus ] );
    · gcongr ; omega;
    · have h_prod_ge_a_pow : ∀ i ∈ Iplus n r, qi n r i ≥ a := by
        ...
        exact (by
        exact if h : i = iM then h.symm ▸ le_rfl else le_of_lt ( qi_lt_of_lt n r i iM hr1 hrn (
            lt_of_le_of_ne h_le h ) h_lt_iM ));
...
theorem Fsel_congr (n n' r m : ℕ) (hr2 : 2 ≤ r) (_hrn : r ≤ omegaCount n) (hrn' : r ≤ omegaCount n')
  ...
  classical
  unfold Fsel;
  simp_all +decide only [Finset.ext_iff, mem_filter, mem_Ico];
  intro a; specialize hb a; simp_all +decide only [bvec] ;
  by_cases ha : a < sr r <;> simp_all +decide only [forall_const, hplus, IsEmpty.forall_iff,
      not_lt];
  · split_ifs at hb <;> simp_all +decide only [not_true_eq_false, and_false, zero_dvd_iff,
      dvd_zero, not_false_eq_true, and_true, iff_false, not_and, not_lt, and_congr_right_iff];
    · exact absurd hb.symm ( Nat.Prime.ne_zero ( qi_prime n' r a ( by
        linarith ) ( by
        linarith ) ( by
        linarith ) ) );
...
theorem qi_recon_eq (n n' r m t : ℕ)
    (hn : n ∈ Acal) (hn' : n' ∈ Acal) (hr2 : 2 ≤ r) (ht : omegaCount n = t) (ht' : omegaCount n' =
        t)
  ...
  classical
  intro k hk_mem hk_not_in_Fsel
  induction k using Nat.strong_induction_on with
  | h k ih =>
    rcases lt_or_ge k (hr r) with hk_lt | hk_ge
    · -- Apply the induction hypothesis to the product.
      have h_prod_eq : ∏ j ∈ (Esel n r).filter (· < k), (qi n r j) ^ (epsv n r m k j) =
          ∏ j ∈ (Esel n r).filter (· < k), (qi n' r j) ^ (epsv n' r m k j) := by
      ...
      have h_simp : Dwit n (qi n r k) = Dwit n' (qi n' r k) := by
        ...
        convert! zero_row_recovery n' r m k hn' hr2 ( by linarith ) hn0' hmn' hk_lt ( by
          unfold Esel at hk_mem; simp_all +decide only [ne_eq, Izero_eq_sdiff, mem_union,
              mem_sdiff, mem_range, true_and, mem_Ico] ;
          contrapose! hk_mem; simp_all +decide [ Iplus ] ; ) using 1;
        rw [ ha k hk_lt, h_prod_eq, Esel_congr n n' r hIp ];
      rw [ kappa_recovery n r k hn ( by
        linarith ) ( by
        linarith ) hn0 ( by
        linarith [ Esel_lt_sr n r k hk_mem ] ), kappa_recovery n' r k hn' ( by
        linarith ) ( by
        linarith ) hn0' ( by
        linarith [ Esel_lt_sr n r k hk_mem ] ) ] ; aesop ( simp_config := { singlePass := true } ) ;
...
theorem qi_dvd_m_of_notMem (n r m i : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (hn0 : n ≠ 0)
  ...
  · exact Nat.Coprime.mul_dvd_of_dvd_of_dvd ( Nat.Coprime.symm <| Nat.Prime.coprime_iff_not_dvd (
      qi_prime n r i hr1 hrn hi ) |>.2 h_div_m.2 ) ( Qr_dvd n r hr1 hrn hn0 ) h_div_m.1;
...
theorem qi_eq_Iplus (n n' r m : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (hrn' : r ≤ omegaCount n')
  ...
  classical
  by_cases hi' : i < sr r <;> simp_all +decide only [ne_eq, bvec, not_lt];
...
theorem Cval_eq_Iplus (n n' r m : ℕ) (hr1 : 1 ≤ r) (_hrn : r ≤ omegaCount n) (hrn' : r ≤
    omegaCount n')
    (hIp : Iplus n r = Iplus n' r) (hb : ∀ j, j < sr r → bvec n r m j = bvec n' r m j)
    (ha : ∀ i, i < hr r → avec n r m i = avec n' r m i) (he : ∀ i k, i < hr r → k ∈ Esel n r →
        epsv n r m i k = epsv n' r m i k)
  ...
  intro k hk hk'; rw [ Esel_congr n n' r hIp ] at *; simp_all +decide only [Fsel, mem_filter,
      mem_Ico, not_and, Decidable.not_not, and_imp, true_and] ;
  intro hk'' hk'''; specialize hqi k hk hk'; simp_all +decide only [bvec, forall_const] ;
  exact hk' ( by
    linarith [ show hplus n r = hplus n' r from by
      rw [ show hplus n r = ( Iplus n r ).card from rfl, show hplus n' r = ( Iplus n' r ).card from
        rfl, hIp ] ] )) (by
  grind) (by
  ...
  intro k hk hk'; rw [ Esel_congr n n' r hIp ] at *; simp_all +decide only [Fsel, mem_filter,
      mem_Ico, not_and, Decidable.not_not, and_imp, true_and] ;
  contrapose! hb;
  use k; simp_all +decide only [forall_const, bvec, ↓reduceIte, ne_eq, right_eq_ite_iff,
      Classical.not_imp];
  exact ⟨ by
    linarith [ show hr r + hplus n r ≤ sr r from by
      linarith [ two_hr_le_sr r, hplus_le_hr n r ] ], hk' ( by
    linarith [ show hr r + hplus n r = hr r + hplus n' r from by
      rw [ show hplus n r = hplus n' r from by
        { unfold hplus; aesop } ] ] ), Ne.symm <| Nat.ne_of_gt <| Nat.Prime.pos <| qi_prime n' r k
      hr1 hrn' <| by
    linarith [ show hr r + hplus n r ≤ sr r from by
      linarith [ two_hr_le_sr r, hplus_le_hr n r ] ] ⟩) (by
  grind +locals))
...
theorem Pval_congr_mod (n n' r m : ℕ) (hn : n ∈ Acal) (hn' : n' ∈ Acal) (hr2 : 2 ≤ r)
  ...
  classical
  have hPcong_step : (Pval n r m * Cval n r m i) ≡ (Pval n' r m * Cval n' r m i) [MOD qi n r i] :=
      by
    rw [ ← positive_common_product, ← positive_common_product ];
    any_goals assumption;
    · have hDwit_mod : Dwit n (qi n r i) ≡ 1 [MOD qi n r i] ∧ Dwit n' (qi n' r i) ≡ 1 [MOD qi n' r i
          ] := by
        have hPcong_step : ∀ n r i, n ∈ Acal → 1 ≤ r → r ≤ omegaCount n → i < sr r → Dwit n (qi n r
            i) ≡ 1 [MOD qi n r i] := by
          intros n r i hn hr1 hrn hi
          have hDwit : Dwit n (qi n r i) ∣ n ∧ 1 < Dwit n (qi n r i) ∧ Dwit n (qi n r i) % (qi n r i
              ) = 1 := by
            apply Dwit_spec n (qi n r i) hn (qi_prime n r i hr1 hrn hi) (qi_dvd n r i hr1 hrn hi) (
                by
        ...
        exact Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) |> lt_of_lt_of_le <|
            Nat.div_le_self _ _)) (hPcong_step n' r i hn' (by
          linarith) hrn' (by
        exact Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) |> lt_of_lt_of_le <|
            Nat.div_le_self _ _)));
      rw [ ← qi_eq_Iplus n n' r m ( by
        linarith ) hrn hrn' hn0 hn0' hmn hmn' hIp hb i hi ] at * ; simp_all +decide [ Nat.ModEq ];
    · exact hIp ▸ hi;
  ...
  simp_all +decide only [ne_eq, Nat.cast_mul, ← sub_mul];
...
theorem Pval_eq (n n' r m t : ℕ)
  ...
  · have hPval_congr : Pval n r m ≡ Pval n' r m [MOD Rval n r] := by
      ...
      have hPval_congr : ∀ i j, i ∈ Iplus n r → j ∈ Iplus n r → i ≠ j → Nat.Coprime (qi n r i) (qi n
          r j) := by
        ...
        have h_distinct : qi n r i ≠ qi n r j := by
          have h_distinct : StrictAntiOn (qi n r) (Iplus n r) := by
            ...
            · grind;
            · exact hij;
            · exact Finset.mem_range.mp ( Finset.mem_filter.mp hj |>.1 ) |> lt_of_lt_of_le <| by
                unfold hr; unfold sr; omega;
          exact fun h => hij <| StrictAntiOn.injOn h_distinct hi hj h;
        have h_prime : Nat.Prime (qi n r i) ∧ Nat.Prime (qi n r j) := by
          exact ⟨ qi_prime n r i ( by linarith ) ( by linarith ) ( by
            exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hi |>.1 ) ) (
                Nat.div_le_self _ _ ) ), qi_prime n r j ( by
              linarith ) ( by
              linarith ) ( by
            exact lt_of_lt_of_le ( Finset.mem_range.mp ( Finset.mem_filter.mp hj |>.1 ) ) ( by
              linarith [ show hr r ≤ sr r from by
                linarith [ two_hr_le_sr r ] ] ) ) ⟩;
        simpa [ h_distinct ] using! Nat.coprime_primes h_prime.1 h_prime.2;
      have hPval_congr : ∀ {S : Finset ℕ}, (∀ i ∈ S, Pval n r m ≡ Pval n' r m [MOD qi n r i]) → (∀ i
          j, i ∈ S → j ∈ S → i ≠ j → Nat.Coprime (qi n r i) (qi n r j)) → Pval n r m ≡ Pval n'
              r m [MOD ∏ i ∈ S, qi n r i] := by
        intros S hS hS_coprime
        induction S using Finset.induction with
        | empty => simp_all +decide [Nat.modEq_iff_dvd]
        | @insert i S hiS ih =>
          rw [ Finset.prod_insert hiS ];
          rw [ ← Nat.modEq_and_modEq_iff_modEq_mul ];
          · exact ⟨ hS i ( Finset.mem_insert_self _ _ ), ih ( fun j hj => hS j (
              Finset.mem_insert_of_mem hj ) ) ( fun j k hj hk hjk => hS_coprime j k (
              Finset.mem_insert_of_mem hj ) ( Finset.mem_insert_of_mem hk ) hjk ) ⟩;
          · exact Nat.Coprime.prod_right fun j hj => hS_coprime i j ( Finset.mem_insert_self _ _ ) (
      ...
      rw [show Rval n' r = Rval n r from by
            exact Finset.prod_congr hIp.symm fun i hi => by
              apply Eq.symm; exact (by
                have := qi_eq_Iplus n n' r m (by
                  linarith) (by
                  linarith) (by
                  linarith) hn0 hn0' hmn hmn' hIp hb i hi;
  ...
  · unfold Pval; simp_all +decide only [ne_eq, Fsel, mem_filter, mem_Ico, not_and,
      Decidable.not_not, and_imp, not_nonempty_iff_eq_empty] ;
...
theorem Qr_eq_of_data (n n' r m t : ℕ)
    (_hn : n ∈ Acal) (_hn' : n' ∈ Acal) (hr2 : 2 ≤ r) (ht : omegaCount n = t) (ht' : omegaCount n'
        = t)
...
noncomputable def fixedFiber (x : ℝ) (t r m : ℕ) : Finset ℕ := by
  classical
  exact (Finset.Icc 1 ⌊x⌋₊).filter (fun n => n ∈ Acal ∧ omegaCount n = t ∧ n / Qr n r = m)

/-- The record-encoding map into a finite product type. -/
noncomputable def encRec (_x : ℝ) (r m : ℕ) (n : ℕ) :
    Finset ℕ × (Fin (sr r) → ℕ) × (Fin (hr r) → ℕ) × (Fin (hr r) → Fin (sr r) → ℕ) × (Fin (hr r) →
        ℕ) :=
  (Iplus n r,
   (fun j => bvec n r m j),
   (fun i => avec n r m i),
...
noncomputable def encTarget (x : ℝ) (r m : ℕ) :
    Finset (Finset ℕ × (Fin (sr r) → ℕ) × (Fin (hr r) → ℕ) × (Fin (hr r) → Fin (sr r) → ℕ) × (Fin
        (hr r) → ℕ)) :=
...
theorem epsv_le_one (n r m i k : ℕ) (hn : n ∈ Acal) (hr2 : 2 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  exact not_or.mpr ⟨ Nat.ne_of_gt ( Finset.prod_pos fun x hx => Nat.Prime.pos ( qi_prime n r x ( by
    linarith ) ( by
    linarith ) ( by
    exact Esel_lt_sr n r x hx ) ) ), not_lt_of_ge ( Nat.le_of_dvd ( Nat.pos_of_ne_zero hn0 ) (
        Qr_dvd n r ( by
      linarith ) ( by
      linarith ) hn0 ) ) ⟩) (by
  rw [ hm, Nat.div_mul_cancel ( Qr_dvd n r ( by linarith ) hrn hn0 ) ];
  exact Dwit_spec n ( qi n r i ) hn ( qi_prime n r i ( by
    linarith ) hrn hi ) ( qi_dvd n r i ( by
    linarith ) hrn hi ) hn0 |>.1) (by
  exact Nat.ne_of_gt ( Nat.pos_of_dvd_of_pos ( Dwit_spec n ( qi n r i ) hn ( qi_prime n r i ( by
    linarith ) ( by
    linarith ) hi ) ( qi_dvd n r i ( by
    linarith ) ( by
    linarith ) hi ) hn0 |>.1 ) ( Nat.pos_of_ne_zero hn0 ) ))).left (qi n r k) (by
  convert! Qr_primeFactors n r ( by
    linarith ) hrn |> fun h => h.symm ▸ Finset.mem_image_of_mem _ hk using 1)
...
theorem kappav_le_logx (n r i N : ℕ) (hn : n ∈ Acal) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n)
  ...
  classical
  -- The index kappav n r i is the position of qi n r i in the sorted list of prime factors of
  -- Dwit n (qi n r i) - 1.
  have h_index : kappav n r i < (Dwit n (qi n r i) - 1).primeFactors.card := by
    have h_index : qi n r i ∈ ((Dwit n (qi n r i) - 1).primeFactors.sort (· ≤ ·)) := by
      ...
      exact Nat.mem_primeFactors.mpr ⟨ qi_prime n r i hr1 hrn hi, this, Nat.sub_ne_zero_of_lt <|
          Nat.one_lt_iff_ne_zero_and_ne_one.mpr ⟨ by
        linarith [ Dwit_spec n ( qi n r i ) hn ( qi_prime n r i hr1 hrn hi ) ( qi_dvd n r i hr1 hrn
          hi ) hn0 ], by
        linarith [ Dwit_spec n ( qi n r i ) hn ( qi_prime n r i hr1 hrn hi ) ( qi_dvd n r i hr1 hrn
          hi ) hn0 ] ⟩ ⟩;
  ...
  refine le_trans ?_ ( Nat.le_trans ( Nat.le_of_dvd ( Nat.pos_of_ne_zero hn0 ) ( Dwit_spec n ( qi
      n r i ) hn ( qi_prime n r i hr1 hrn hi ) ( qi_dvd n r i hr1 hrn hi ) hn0 |>.1 ) ) hnN );
  have h_prod_le : ∏ p ∈ (Dwit n (qi n r i) - 1).primeFactors, p ≤ Dwit n (qi n r i) - 1 := by
    exact Nat.le_of_dvd ( Nat.sub_pos_of_lt ( Dwit_spec n ( qi n r i ) hn ( qi_prime n r i hr1 hrn
        hi ) ( qi_dvd n r i hr1 hrn hi ) hn0 |>.2.1 ) ) ( Nat.prod_primeFactors_dvd _ );
  exact le_trans ( by
    simpa using! Finset.prod_le_prod' fun p hp => Nat.Prime.two_le <| Nat.prime_of_mem_primeFactors
      hp ) ( h_prod_le.trans <| Nat.pred_le _ )
...
theorem mval_pos (n r m : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (hn1 : 1 ≤ n)
  ...
  classical
  rw [ hm ];
  refine Nat.div_pos ( Nat.le_of_dvd hn1 ?_ ) ( Nat.pos_of_dvd_of_pos ?_ hn1 ); all_goals
    exact Qr_dvd n r hr1 hrn ( by
      linarith )
...
theorem encRec_mapsTo (x : ℝ) (t r m : ℕ) (_hx : 3 ≤ x) (hr2 : 2 ≤ r) (hrt : r ≤ t) :
  ...
  classical
  intro n hn; simp +decide only [encTarget, mem_product, mem_powerset, Fintype.mem_piFinset,
      mem_insert, Nat.mem_primeFactors, ne_eq, Nat.mem_divisors, mem_singleton, mem_range,
      Order.lt_add_one_iff] ;
  refine ⟨ ?_, ?_, ?_, ?_, ?_ ⟩;
  · exact fun i hi => Finset.mem_range.mpr ( Finset.mem_filter.mp hi |>.1 |> Finset.mem_range.mp );
  · unfold fixedFiber at hn; simp_all +decide only [mem_filter, mem_Icc, encRec] ;
    intro a; unfold bvec; split_ifs <;> simp_all +decide only [true_and, zero_dvd_iff,
        and_not_self, and_self, or_false] ;
    exact Or.inr ⟨ qi_prime n r a ( by
      linarith ) ( by
      linarith ) ( by
      exact a.2 ), by
      linarith [ Nat.div_pos ( show n ≥ Qr n r from Nat.le_of_dvd ( by
        linarith ) ( Qr_dvd n r ( by
        linarith ) ( by
        linarith ) ( by
        linarith ) ) ) ( show 0 < Qr n r from Nat.pos_of_ne_zero ( by
        exact Finset.prod_ne_zero_iff.mpr fun i hi => Nat.Prime.ne_zero ( qi_prime n r i ( by
          linarith ) ( by
          linarith ) ( by
          exact Esel_lt_sr n r i hi ) ) ) ) ] ⟩;
  · unfold fixedFiber at hn; simp_all +decide only [mem_filter, mem_Icc, encRec] ;
    refine fun a => ⟨ Nat.gcd_dvd_right _ _, ?_ ⟩;
    exact Nat.ne_of_gt ( mval_pos n r m ( by
      linarith ) ( by
      linarith ) ( by
      linarith ) ( by
      tauto ) );
  · intro i j; by_cases hij : j.val ∈ Esel n r <;> simp +decide only [encRec, ↓reduceIte, hij,
      zero_ne_one, or_false] ;
    have := epsv_le_one n r m i j ?_ ?_ ?_ ?_ ?_ ?_ ?_ <;> simp_all +decide only [fixedFiber,
        mem_filter, mem_Icc, ne_eq];
  ...
  · intro i; exact kappav_le_logx n r i ⌊x⌋₊ (by
    ...
    exact lt_of_lt_of_le i.2 ( Nat.le_of_lt_succ <| by
      linarith [ show sr r = 1 + Nat.log 2 r from rfl, show hr r = sr r / 2 from rfl,
        Nat.div_mul_le_self ( sr r ) 2 ] )) (by
...
theorem fixed_prefix_card_le (x : ℝ) (t r m : ℕ) (hx : 3 ≤ x) (hr2 : 2 ≤ r) (hrt : r ≤ t) :
  ...
  classical
  rw [ ← encTarget_card ];
  refine le_trans ?_ ( Finset.card_le_card <| show encTarget x r m ≥ Finset.image ( encRec x r m )
      ( fixedFiber x t r m ) from ?_ );
  · rw [ Finset.card_image_of_injOn ] ; exact fun a ha b hb hab => by
      have := encRec_injOn x t r m hr2 hrt ; exact this ha hb hab;
...
theorem pnth_le_qi (n r j : ℕ) (hr1 : 1 ≤ r) (hrn : r ≤ omegaCount n) (hj : j ∈ Esel n r) :
  ...
  classical
  -- Let `L = n.primeFactors.sort (· ≥ ·)`. `L` is sorted in `≥` order (`Finset.sort_sorted`) and
  -- has length `omegaCount n` (`Finset.length_sort`, `= n.primeFactors.card`).
  ...
  have hL_getD' : pnth n r = L.get ⟨r - 1, by
    ...
    grind
  generalize_proofs at *;
  -- `j ∈ Esel n r` gives `j < sr r` (`Esel_lt_sr`), so `qi n r j ∈ Siter n (topR n r) j`
  -- (`qi_mem_Siter`) `subseteq topR n r` (`Siter_subset`).
  have hq_subset_topR : qi n r j ∈ topR n r := by
    exact Siter_subset n r j |> fun h => h <| qi_mem_Siter n r j hr1 hrn <| Esel_lt_sr n r j hj;
  -- `topR n r = (L.take r).toFinset`, so `qi n r j ∈ (L.take r).toFinset`, i.e. `qi n r j ∈
  -- L.take r` (`List.mem_toFinset`).
...
theorem rhoIdx_max (n r : ℕ) (h : 1 ≤ omegaCount n) (hr : r ∈ Finset.Icc 1 (omegaCount n)) :
  ...
  classical
  convert! ( Finset.exists_max_image ( Finset.Icc 1 ( omegaCount n ) ) ( score n ) ( by
    aesop ) ) |> Classical.choose_spec |> And.right |> fun h => h r hr using 1;
  unfold rhoIdx; aesop;
...
theorem Qcanon_squarefree (n : ℕ) : Squarefree (Qcanon n) := by
  ...
  unfold Qcanon;
  split_ifs;
  · -- If `rhoIdx n = 1`, then by definition `omegaCount n` is empty or `pnth n 1 = 1`. In either
  -- case, `pnth n 1` is squarefree.
    ...
    · -- Since `omegaCount n ≠ 0`, `pnth n 1` is a prime factor of `n`.
      have h_prime : ∃ p, p ∈ n.primeFactors ∧ pnth n 1 = p := by
        unfold pnth;
        rcases x : n.primeFactors.sort ( · ≥ · ) with ( _ | ⟨ p, _ | ⟨ q, l ⟩ ⟩ ) <;> simp_all
            +decide only [↓existsAndEq, tsub_self, List.getD_eq_getElem?_getD, List.length_cons,
    ...
    Nat.mem_primeFactors, ne_eq, and_true, List.length_nil, zero_add, Order.lt_one_iff,
     not_false_eq_true, getElem?_neg, Option.getD_none, isUnit_iff_eq_one,
    IsUnit.dvd, true_and];
        ...
        · replace x := congr_arg List.toFinset x; rw [ Finset.ext_iff ] at x; specialize x p; aesop;
      rcases h_prime with ⟨ p, hp₁, hp₂ ⟩ ; rw [ hp₂ ] ; exact Nat.prime_iff.mp (
          Nat.prime_of_mem_primeFactors hp₁ ) |> fun h => h.squarefree;
  · apply Qr_squarefree;
    · unfold rhoIdx at *;
      grind;
...
theorem Qcanon_dvd (n : ℕ) (hn : n ∈ Acal) (h2 : 2 ≤ omegaCount n) : Qcanon n ∣ n := by
  classical
  by_cases h : rhoIdx n = 1 <;> simp_all +decide only [Qcanon, ↓reduceIte];
  · unfold pnth;
    cases k : n.primeFactors.sort (fun x1 x2 => x1 ≥ x2) with
    | nil =>
        have hempty : n.primeFactors = ∅ := by
          simpa only [Finset.sort_toFinset, List.toFinset_nil] using congrArg List.toFinset k
        simp only [omegaCount, hempty, Finset.card_empty] at h2
        omega
    | cons p ps =>
        have hp : p ∈ n.primeFactors := (Finset.mem_sort (fun x1 x2 => x1 ≥ x2)).mp (by
          rw [k]
          exact List.mem_cons_self)
        simpa only [tsub_self, k, List.getD_cons_zero] using Nat.dvd_of_mem_primeFactors hp
  · apply Qr_dvd;
...
theorem hr_small_lt (r K : ℕ) (h : hr r < K) : r < 2 ^ (2 * K) := by
  classical
  rcases r with ( _ | _ | r ) <;> simp_all +arith +decide only [hr, Order.lt_two_iff, pow_pos,
      zero_add, Nat.one_lt_two_pow_iff, ne_eq, mul_eq_zero, false_or];
  · grind;
  · unfold sr at h;
    have := Nat.lt_pow_of_log_lt ( by
      linarith ) ( show Nat.log 2 ( r + 2 ) < 2 * K from by
      omega ) ; linarith
...
theorem hr_div_log_tendsto :
  ...
  have h_hr_log : Filter.Tendsto (fun t : ℕ => (hr t : ℝ) / (Nat.log 2 t / 2)) Filter.atTop (nhds 1)
      := by
    -- We'll use the fact that `hr t` is approximately `Nat.log 2 t / 2`.
    have hr_approx : ∀ t : ℕ, t ≥ 2 → (hr t : ℝ) ≥ (Nat.log 2 t / 2 : ℝ) ∧ (hr t : ℝ) ≤ (Nat.log 2 t
        / 2 : ℝ) + 1 := by
      intro t _
    ...
    have hr_bound : ∀ t : ℕ, t ≥ 2 → |(hr t : ℝ) / (Nat.log 2 t / 2) - 1| ≤ 2 / (Nat.log 2 t) := by
      intro t ht; rw [ abs_le ] ; constructor <;> ring_nf at * <;> nlinarith [ hr_approx t ht,
          inv_mul_cancel₀ ( show ( Nat.log 2 t : ℝ ) ≠ 0 by
        exact ne_of_gt <| Nat.cast_pos.mpr <| Nat.log_pos one_lt_two ht ), show ( Nat.log 2 t : ℝ )
          ≥ 1 by
        exact_mod_cast Nat.log_pos one_lt_two ht ] ;
    generalize_proofs at *; (
    exact tendsto_iff_norm_sub_tendsto_zero.mpr <| squeeze_zero_norm' ( Filter.eventually_atTop.mpr
        ⟨ 2, fun t ht => by
      simpa using! hr_bound t ht ⟩ ) <| tendsto_const_nhds.div_atTop <|
        tendsto_natCast_atTop_atTop.comp <| Filter.tendsto_atTop_atTop.mpr fun x => ⟨ 2 ^ x, fun t
        ht => Nat.le_log_of_pow_le ( by
      norm_num ) ht ⟩ ;);
  -- We'll use the fact that $Nat.log 2 t \sim \frac{\log t}{\log 2}$ as $t \to \infty$.
  have h_log : Filter.Tendsto (fun t : ℕ => (Nat.log 2 t : ℝ) / (Real.log t / Real.log 2))
      Filter.atTop (nhds 1) := by
    -- We'll use the fact that $Nat.log 2 t \leq \frac{\log t}{\log 2} < Nat.log 2 t + 1$ to bound
    -- the ratio.
    have h_log_bound : ∀ t : ℕ, t ≥ 2 → (Nat.log 2 t : ℝ) ≤ (Real.log t) / (Real.log 2) ∧ (Real.log
        t) / (Real.log 2) < (Nat.log 2 t + 1 : ℝ) := by
      intro t ht; rw [ le_div_iff₀ ( Real.log_pos one_lt_two ), div_lt_iff₀ ( Real.log_pos
          one_lt_two ) ] ; norm_cast;
      exact ⟨ by
        rw [ ← Real.log_rpow zero_lt_two ] ; exact Real.log_le_log ( by
          positivity ) ( mod_cast Nat.pow_log_le_self 2 ( by
          positivity ) ), by
        rw [ ← Real.log_rpow zero_lt_two ] ; exact Real.log_lt_log ( by
          positivity ) ( mod_cast Nat.lt_pow_succ_log_self ( by
          decide ) _ ) ⟩;
    -- Using the bounds, we can show that the ratio is squeezed between two sequences that tend to
    -- 1.
    have h_squeeze : ∀ t : ℕ, t ≥ 2 → 1 - 1 / (Nat.log 2 t + 1 : ℝ) ≤ (Nat.log 2 t : ℝ) / (Real.log
        t / Real.log 2) ∧ (Nat.log 2 t : ℝ) / (Real.log t / Real.log 2) ≤ 1 := by
      intro t ht
      specialize h_log_bound t ht
      have hpos : 0 < Real.log t / Real.log 2 :=
        div_pos (Real.log_pos (Nat.one_lt_cast.mpr ht)) (Real.log_pos (by norm_num))
      rw [one_sub_div (by positivity)]
      rw [div_le_div_iff₀] <;> try linarith [hpos]
    ...
    have h_seq_tendsto : Filter.Tendsto (fun t : ℕ => 1 - 1 / (Nat.log 2 t + 1 : ℝ)) Filter.atTop (
        nhds 1) := by
      exact le_trans ( tendsto_const_nhds.sub <| tendsto_const_nhds.div_atTop <|
          Filter.tendsto_atTop_add_const_right _ _ <| tendsto_natCast_atTop_atTop.comp <|
          Filter.tendsto_atTop_atTop.mpr fun x => ⟨ 2 ^ x, fun t ht => Nat.le_log_of_pow_le ( by
        norm_num ) ht ⟩ ) <| by
        norm_num;
    exact tendsto_of_tendsto_of_tendsto_of_le_of_le' h_seq_tendsto tendsto_const_nhds (
        Filter.eventually_atTop.mpr ⟨ 2, fun t ht => h_squeeze t ht |>.1 ⟩ ) (
        Filter.eventually_atTop.mpr ⟨ 2, fun t ht => h_squeeze t ht |>.2 ⟩ )
  have hmul : Filter.Tendsto
      (fun t : ℕ => ((hr t : ℝ) / (Nat.log 2 t / 2)) *
        ((Nat.log 2 t : ℝ) / (Real.log t / Real.log 2))) Filter.atTop (nhds 1) := by
    simpa only [one_mul] using h_hr_log.mul h_log
  refine hmul.congr' ?_
  filter_upwards [Filter.eventually_ge_atTop 2] with t ht
  have hNatlog : (Nat.log 2 t : ℝ) ≠ 0 :=
    Nat.cast_ne_zero.mpr (Nat.log_pos one_lt_two ht).ne'
  have hlog : Real.log (t : ℝ) ≠ 0 := (Real.log_pos (Nat.one_lt_cast.mpr ht)).ne'
  have hlog2 : Real.log 2 ≠ 0 := (Real.log_pos one_lt_two).ne'
  field_simp [hNatlog, hlog, hlog2]
...
theorem sum_inv_hr_le (ε : ℝ) (hε : 0 < ε) :
  ...
  obtain ⟨δ, hδ_pos, hδ_lt⟩ : ∃ δ : ℝ, 0 < δ ∧ δ < 1 ∧ 2 * Real.log 2 / (1 - δ) < 2 * Real.log 2 + ε
      / 2 := by
    have hδ : Filter.Tendsto (fun δ : ℝ => 2 * Real.log 2 / (1 - δ)) (nhdsWithin 0 (Set.Ioi 0)) (
        nhds (2 * Real.log 2)) := by
      exact le_trans ( tendsto_const_nhds.div ( tendsto_const_nhds.sub ( Filter.tendsto_id.mono_left
          inf_le_left ) ) ( by
        norm_num ) ) ( by
        norm_num );
    have := hδ.eventually ( gt_mem_nhds <| show 2 * Real.log 2 < 2 * Real.log 2 + ε / 2 by
      linarith ) ; have := this.and ( Ioo_mem_nhdsGT_of_mem ⟨ le_rfl, zero_lt_one ⟩ ) ; obtain ⟨ δ,
        hδ₁, hδ₂ ⟩ := this.exists; exact ⟨ δ, hδ₂.1, hδ₂.2, hδ₁ ⟩ ;
  ...
  have h_sum_L : ∃ T : ℕ, ∀ t ≥ T, (∑ r ∈ Finset.filter (fun r => K t ≤ hr r) (Finset.Icc 2 t), (1 /
      (hr r : ℝ))) ≤ (2 * Real.log 2 + ε / 2) * t / Real.log t := by
    -- For `r ∈ L`, `1/(hr r) ≤ 1/(K t)` (since `hr r ≥ K t ≥ ...`, use `one_le_hr`, and `K t ≥
    -- (1-δ) hr t`).
    have h_sum_L_bound : ∀ t : ℕ, t ≥ 2 → (∑ r ∈ Finset.filter (fun r => K t ≤ hr r) (Finset.Icc 2 t
        ), (1 / (hr r : ℝ))) ≤ (t : ℝ) / (K t : ℝ) := by
      intros t ht
      have h_card_L : (Finset.filter (fun r => K t ≤ hr r) (Finset.Icc 2 t)).card ≤ t := by
        exact le_trans ( Finset.card_filter_le _ _ ) ( by simp );
      refine le_trans ( Finset.sum_le_sum fun x hx => one_div_le_one_div_of_le ?_ <|
          Nat.cast_le.mpr <| Finset.mem_filter.mp hx |>.2 ) ?_ <;> norm_num [ div_eq_mul_inv ];
    ...
    have h_sum_L_bound' : ∀ t : ℕ, t ≥ 2 → (∑ r ∈ Finset.filter (fun r => K t ≤ hr r) (Finset.Icc 2
        t), (1 / (hr r : ℝ))) ≤ (t : ℝ) / ((1 - δ) * (hr t : ℝ)) := by
      ...
      · exact Nat.le_ceil _;
    -- Since `hr t / (log t / (2 log 2)) → 1`, we have `t / ((1-δ) hr t) / (t/log t) → 2 log
    -- 2/(1-δ)`.
    have h_lim : Filter.Tendsto (fun t : ℕ => (t : ℝ) / ((1 - δ) * (hr t : ℝ)) / (t / Real.log t))
        Filter.atTop (nhds (2 * Real.log 2 / (1 - δ))) := by
      have h_lim : Filter.Tendsto (fun t : ℕ => (hr t : ℝ) / (Real.log t / (2 * Real.log 2)))
          Filter.atTop (nhds 1) := by
      ...
      · ring;
      · filter_upwards [ Filter.eventually_gt_atTop 1 ] with t ht ; norm_num [ div_eq_mul_inv,
          mul_assoc, mul_comm, mul_left_comm, ne_of_gt ( zero_lt_one.trans_le (
          Nat.one_le_cast.mpr ht.le ) ) ];
        exact Or.inl <| Or.inl <| by ring;
    have := h_lim.eventually ( gt_mem_nhds <| show 2 * Real.log 2 / ( 1 - δ ) < 2 * Real.log 2 + ε
        / 2 from hδ_lt.2 );
    simp +zetaDelta only [ge_iff_le, one_div] at *;
    obtain ⟨ T, hT ⟩ := Filter.eventually_atTop.mp this; use T + 2; intros t ht; specialize hT t (
        by
      linarith ) ; rw [ div_lt_iff₀ ] at hT;
    · exact le_trans ( h_sum_L_bound' t ( by
        linarith ) ) ( by
        simpa only [ mul_div_assoc ] using! hT.le );
    · exact div_pos ( Nat.cast_pos.mpr ( by
        linarith ) ) ( Real.log_pos ( Nat.one_lt_cast.mpr ( by
        linarith ) ) );
  -- Show that eventually `∑_{r∈S} 1/(hr r) ≤ (ε/2) t/log t`.
  have h_sum_S : ∃ T : ℕ, ∀ t ≥ T, (∑ r ∈ Finset.filter (fun r => hr r < K t) (Finset.Icc 2 t), (1 /
      (hr r : ℝ))) ≤ (ε / 2) * t / Real.log t := by
    -- Since $hr r < K t$, we have $r < 2^{2K t}$.
    have h_bound_S : ∀ t : ℕ, t ≥ 2 → (∑ r ∈ Finset.filter (fun r => hr r < K t) (Finset.Icc 2 t), (
        1 / (hr r : ℝ))) ≤ (2 ^ (2 * K t) : ℝ) := by
      intros t ht
      have h_bound_S : ∀ r ∈ Finset.filter (fun r => hr r < K t) (Finset.Icc 2 t), r < 2 ^ (2 * K t)
          := by
        intros r hr;
        have := hr_small_lt r ( K t ) ?_ <;> aesop;
      refine le_trans ( Finset.sum_le_sum fun x hx => one_div_le_one_div_of_le ?_ <| show ( hr x :
          ℝ ) ≥ 1 from ?_ ) ?_ <;> norm_num;
      · exact one_le_hr x ( by linarith [ Finset.mem_Icc.mp ( Finset.mem_filter.mp hx |>.1 ) ] );
      · exact_mod_cast le_trans ( Finset.card_le_card <| show Finset.filter ( fun r => hr r < K t )
            ( Finset.Icc 2 t ) ⊆ Finset.Ico 2 ( 2 ^ ( 2 * K t ) ) from fun x hx =>
            Finset.mem_Ico.mpr ⟨ Finset.mem_Icc.mp ( Finset.mem_filter.mp hx |>.1 ) |>.1, h_bound_S
            x hx ⟩ ) ( by
          norm_num );
    ...
    have h_exp_bound : ∀ t : ℕ, t ≥ 2 → (2 ^ (2 * K t) : ℝ) ≤ 4 * (2 ^ (2 * hr t)) ^ (1 - δ) := by
      ...
      have h_exp_bound : (2 : ℝ) ^ (2 * K t) ≤ (2 : ℝ) ^ (2 * ((1 - δ) * (hr t : ℝ) + 1)) := by
        exact_mod_cast Real.rpow_le_rpow_of_exponent_le one_le_two ( show ( 2 * K t : ℝ ) ≤ 2 * ( (
            1 - δ ) * hr t + 1 ) by
          linarith [ Nat.ceil_lt_add_one ( show 0 ≤ ( 1 - δ ) * hr t by
            exact mul_nonneg ( sub_nonneg.mpr hδ_lt.1.le ) ( Nat.cast_nonneg _ ) ) ] );
      ...
      rw [ ← Real.rpow_natCast, ← Real.rpow_mul ( by
        positivity ), mul_comm, Real.rpow_mul ( by
        positivity ), Real.rpow_natCast ];
    -- Since $hr t \leq \log t / (2 \log 2) + 1$, we have $(2^{2 hr t})^{1-δ} \leq (t \cdot
    -- 2^{2})^{1-δ}$.
    have h_log_bound : ∀ t : ℕ, t ≥ 2 → (2 ^ (2 * hr t) : ℝ) ≤ (t * 2 ^ 2) := by
      ...
      have h_hr_bound : (hr t : ℝ) ≤ Real.log t / (2 * Real.log 2) + 1 := by
        ...
        have := Real.log_le_log ( by
          positivity ) ( show ( t : ℝ ) ≥ 2 ^ Nat.log 2 t by
          exact_mod_cast Nat.pow_log_le_self 2 <| by
            linarith ) ; norm_num at * ; linarith;
      rw [ ← Real.log_le_log_iff ( by positivity ) ( by positivity ), Real.log_pow ];
      rw [ Real.log_mul ( by
        positivity ) ( by
        positivity ), Real.log_pow ] ; norm_num ; nlinarith [ Real.log_pos one_lt_two,
          mul_div_cancel₀ ( Real.log t ) ( by
        positivity : ( 2 * Real.log 2 ) ≠ 0 ) ];
    -- Since $t^{1-δ} \cdot \log t / t \to 0$, we have $4 \cdot (t \cdot 2^{2})^{1-δ} \leq (ε/2) t
    -- / \log t$ eventually.
    have h_lim_zero : Filter.Tendsto (fun t : ℕ => (4 * (t * 2 ^ 2 : ℝ) ^ (1 - δ)) / (t / Real.log t
        )) Filter.atTop (nhds 0) := by
      -- Simplify the expression inside the limit.
      suffices h_simplify : Filter.Tendsto (fun t : ℕ => (4 * (2 ^ 2) ^ (1 - δ) * (t : ℝ) ^ (1 - δ)
          * Real.log t) / t) Filter.atTop (nhds 0) by
        refine h_simplify.congr' ?_;
        filter_upwards [ Filter.eventually_gt_atTop 0 ] with t ht using by
          rw [ Real.mul_rpow ( by
            positivity ) ( by
            positivity ) ] ; rw [ div_div_eq_mul_div ] ; ring;
      -- We can factor out $t^{1-δ}$ and use the fact that $\frac{\log t}{t^δ}$ tends to $0$ as
      -- $t$ tends to infinity.
      have h_factor : Filter.Tendsto (fun t : ℕ => (Real.log t : ℝ) / t ^ δ) Filter.atTop (nhds 0)
          := by
        ...
        suffices h_log : Filter.Tendsto (fun y : ℝ => y / Real.exp (y * δ)) Filter.atTop (nhds 0) by
          ...
          refine this.congr' ( by
            filter_upwards [ Filter.eventually_gt_atTop 0 ] with t ht using by
              simp +decide [ Real.rpow_def_of_pos ( Nat.cast_pos.mpr ht ), mul_comm ] );
        ...
        suffices h_z : Filter.Tendsto (fun z : ℝ => z / Real.exp z) Filter.atTop (nhds 0) by
          ...
          convert! this.div_const δ using 2 <;> norm_num [ div_eq_mul_inv, mul_assoc, mul_comm δ,
              hδ_pos.ne' ];
    ...
    refine ⟨ T + 2, fun t ht => le_trans ( h_bound_S t ( by
      linarith ) ) ( le_trans ( h_exp_bound t ( by
      linarith ) ) ?_ ) ⟩;
    have := hT t ( by linarith );
    rw [ div_lt_iff₀ ( div_pos ( Nat.cast_pos.mpr ( by
      linarith ) ) ( Real.log_pos ( Nat.one_lt_cast.mpr ( by
      linarith ) ) ) ) ] at this;
    rw [ mul_div_assoc ];
    exact le_trans ( mul_le_mul_of_nonneg_left ( Real.rpow_le_rpow ( by
      positivity ) ( h_log_bound t ( by
      linarith ) ) ( by
      linarith ) ) ( by
      positivity ) ) this.le;
  obtain ⟨ T₁, hT₁ ⟩ := h_sum_L; obtain ⟨ T₂, hT₂ ⟩ := h_sum_S; use Max.max T₁ T₂; intro t ht;
      specialize hT₁ t ( le_trans ( le_max_left _ _ ) ht ) ; specialize hT₂ t ( le_trans (
      le_max_right _ _ ) ht ) ; simp_all +decide only [one_div] ;
...
theorem log_Qcanon_ge_score (n : ℕ) (h2 : 2 ≤ omegaCount n) :
  ...
  classical
  by_cases h : rhoIdx n = 1 <;> simp_all +decide only [score, ↓reduceIte];
  · unfold Qcanon; aesop;
  · rw [ Qcanon ];
    convert! Real.log_le_log ?_ ( Nat.cast_le.mpr ( Qr_ge n ( rhoIdx n ) ?_ ?_ ) ) using 1 <;>
        norm_num [ h ];
    · exact pow_pos ( Nat.cast_pos.mpr <| Nat.pos_of_ne_zero <| by
        unfold pnth;
        have h_prime_factors : ∀ x ∈ n.primeFactors.sort (· ≥ ·), x ≠ 0 := by
          exact fun x hx => Nat.ne_of_gt <| Nat.pos_of_mem_primeFactors <| Finset.mem_sort ( α :=
              ℕ ) ( · ≥ · ) |>.1 hx;
        grind ) _;
    · exact Finset.mem_Icc.mp ( rhoIdx_mem n ( by linarith ) ) |>.1;
    · exact Finset.mem_Icc.mp ( rhoIdx_mem n ( by linarith ) ) |>.2
...
theorem W_le_M_sum (n : ℕ) (h2 : 2 ≤ omegaCount n) :
  ...
  classical
  -- By definition of $score$, we know that for any $r \in \{1, 2, \ldots, \omega(n)\}$, $score n
  -- r \leq score n (rhoIdx n)$.
  have h_score_le : ∀ r ∈ Finset.Icc 1 (omegaCount n), (if r = 1 then Real.log (pnth n 1) else (hr r
      : ℝ) * Real.log (pnth n r)) ≤ score n (rhoIdx n) := by
  ...
  have h_sum_bound : ∑ r ∈ Finset.Icc 2 (omegaCount n), Real.log (pnth n r) ≤ ∑ r ∈ Finset.Icc 2 (
      omegaCount n), (score n (rhoIdx n)) / (hr r : ℝ) := by
    gcongr;
  ...
  convert! add_le_add ( show Real.log ( pnth n 1 ) ≤ score n ( rhoIdx n ) from by
                        simpa using! h_score_le 1 ( Finset.mem_Icc.mpr ⟨ by
                          norm_num, by
                          linarith ⟩ ) ) h_sum_bound using 1;
  · rw [ ← sum_log_pnth ];
...
theorem weighted_prefix (η : ℝ) (hη : 0 < η) :
  ...
  classical
  by_cases hη1 : η ≥ 1;
  · use 1; intros n hn hn'; exact le_trans ( div_nonpos_of_nonpos_of_nonneg (
        mul_nonpos_of_nonpos_of_nonneg ( mul_nonpos_of_nonpos_of_nonneg (
        div_nonpos_of_nonpos_of_nonneg ( by
      linarith ) ( by
      positivity ) ) ( Real.log_nonneg ( mod_cast Nat.pos_of_ne_zero ( by
      exact Finset.prod_ne_zero_iff.mpr fun p hp => Nat.ne_of_gt <| Nat.pos_of_mem_primeFactors hp )
          ) ) ) ( Real.log_nonneg ( mod_cast Nat.one_le_iff_ne_zero.mpr ( by
      finiteness ) ) ) ) ( Nat.cast_nonneg _ ) ) ( Real.log_nonneg ( mod_cast Nat.pos_of_ne_zero (
          by
      exact Nat.ne_of_gt ( Nat.pos_of_ne_zero ( by
        exact fun h => by
          have := Qcanon_squarefree n; simp_all +decide ) ) ) ) ) ;
  · -- Choose ε such that (1 - η) * (2 * Real.log 2 + ε) / (2 * Real.log 2) < 1.
    obtain ⟨ε, hε_pos, hε⟩ : ∃ ε > 0, (1 - η) * (2 * Real.log 2 + ε) / (2 * Real.log 2) < 1 := by
      exact ⟨ ( 2 * Real.log 2 * ( 1 - ( 1 - η ) ) / 2 ) / ( 1 - η ), div_pos ( div_pos ( mul_pos (
          mul_pos two_pos ( Real.log_pos one_lt_two ) ) ( by
        linarith ) ) zero_lt_two ) ( by
        linarith ), by
        rw [ div_lt_iff₀ ( by
          positivity ) ] ; nlinarith [ Real.log_pos one_lt_two, mul_div_cancel₀ ( 2 * Real.log 2 * (
          1 - ( 1 - η ) ) / 2 ) ( by
          linarith : ( 1 - η ) ≠ 0 ) ] ⟩;
    ...
    have hκ_lt_1 : κ < 1 := by
      exact hε;
    obtain ⟨T₁, hT₁⟩ : ∃ T₁ : ℕ, ∀ t : ℕ, T₁ ≤ t → (1 - η) / (2 * Real.log 2) * (Real.log t / t) ≤ 1
        - κ := by
      have h_log_div_t_zero : Filter.Tendsto (fun t : ℕ => Real.log t / t) Filter.atTop (nhds 0) :=
          by
        -- Let $y = \frac{1}{x}$, so we can rewrite the limit as $\lim_{y \to 0^+} y \log(1/y)$.
        suffices h_log_recip : Filter.Tendsto (fun y : ℝ => y * Real.log (1 / y)) (Filter.map (fun x
      ...
      simpa using! h_log_div_t_zero.const_mul _ |> fun h => h.eventually ( ge_mem_nhds <| by
        linarith );
    obtain ⟨T₀, hT₀⟩ : ∃ T₀ : ℕ, ∀ t : ℕ, T₀ ≤ t → (∑ r ∈ Finset.Icc 2 t, (1 : ℝ) / (hr r)) ≤ (2 *
        Real.log 2 + ε) * t / Real.log t := by
