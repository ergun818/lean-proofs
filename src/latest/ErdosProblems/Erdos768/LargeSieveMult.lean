...
-- The finite Fourier expansion and orthogonality sums use the existing proof budget.
lemma plancherel_dft (q : ℕ) [NeZero q] (Φ : ZMod q → ℂ) :
  ...
  have h_dft_def : ∀ k : ZMod q, ZMod.dft Φ k = ∑ a : ZMod q, Φ a * ZMod.stdAddChar (-(k * a)) := by
    simp +decide only [ZMod.dft_apply, mul_comm, smul_eq_mul, implies_true];
  -- By Fubini's theorem, we can interchange the order of summation.
  have h_fubini : ∑ k : ZMod q, ‖∑ a : ZMod q, Φ a * ZMod.stdAddChar (-(k * a))‖ ^ 2 = ∑ a : ZMod q,
      ∑ b : ZMod q, Φ a * starRingEnd ℂ (Φ b) * ∑ k : ZMod q, ZMod.stdAddChar (-(k * a)) *
      ZMod.stdAddChar (k * b) := by
    have h_fubini : ∀ k : ZMod q, ‖∑ a : ZMod q, Φ a * ZMod.stdAddChar (-(k * a))‖ ^ 2 = ∑ a : ZMod
        q, ∑ b : ZMod q, Φ a * starRingEnd ℂ (Φ b) * ZMod.stdAddChar (-(k * a)) * ZMod.stdAddChar (k
        * b) := by
      intro k
      have h_fubini : ‖∑ a : ZMod q, Φ a * ZMod.stdAddChar (-(k * a))‖ ^ 2 = (∑ a : ZMod q, Φ a *
          ZMod.stdAddChar (-(k * a))) * (∑ b : ZMod q, starRingEnd ℂ (Φ b) * ZMod.stdAddChar (k * b)
          ) := by
  ...
  have h_orthogonality : ∀ a b : ZMod q, ∑ k : ZMod q, ZMod.stdAddChar (-(k * a)) * ZMod.stdAddChar
      (k * b) = if a = b then q else 0 := by
    intro a b; split_ifs with h; focus simp_all +decide only [Complex.ofReal_sum,
        Complex.ofReal_pow,
        ← ZMod.stdAddChar.map_add_eq_mul, neg_add_cancel, AddChar.map_zero_eq_one, sum_const,
    ...
    · simp +decide only [CharP.cast_eq_zero, sub_eq_zero, ZMod.card, Nat.cast_ite,
        right_eq_ite_iff];
      aesop;
  simp_all +decide only [Complex.ofReal_sum, Complex.ofReal_pow, Nat.cast_ite, CharP.cast_eq_zero,
      mul_ite, Complex.mul_conj, Complex.normSq_eq_norm_sq, mul_zero, sum_ite_eq, ↓reduceIte,
      Finset.mul_sum _ _ _];
  norm_cast at * ; simp_all +decide only [mul_comm, Nat.cast_ite, CharP.cast_eq_zero]
...
-/
set_option maxHeartbeats 1600000 in
-- The Plancherel reduction and character norm calculation use the existing proof budget.
lemma gauss_norm (q : ℕ) [NeZero q] (χ : DirichletCharacter ℂ q)
    (hχ : χ.IsPrimitive) :
    ‖gaussSum χ ZMod.stdAddChar‖ ^ 2 = q := by
  classical
  by_contra h_contra;
  have h_plancherel : ∑ k : ZMod q, ‖(ZMod.dft (fun a => χ a) k)‖ ^ 2 = q * ∑ a : ZMod q, ‖χ a‖ ^ 2
      := by
    convert! plancherel_dft q ( fun a => χ a ) using 1;
  -- By definition of $gaussSum$, we know that $‖(ZMod.dft (fun a => χ a) k)‖^2 = ‖χ⁻¹ (-k) *
  -- gaussSum χ ZMod.stdAddChar‖^2$.
  have h_gauss_sum : ∀ k : ZMod q, ‖(ZMod.dft (fun a => χ a) k)‖ ^ 2 = ‖χ⁻¹ (-k) * gaussSum χ
      ZMod.stdAddChar‖ ^ 2 := by
    intro k; rw [ DirichletCharacter.IsPrimitive.fourierTransform_eq_inv_mul_gaussSum hχ k ] ;
  -- By definition of $gaussSum$, we know that $‖χ⁻¹ (-k) * gaussSum χ ZMod.stdAddChar‖^2 =
  -- ‖gaussSum χ ZMod.stdAddChar‖^2$ if $-k$ is a unit, and $0$ otherwise.
  have h_gauss_sum_unit : ∀ k : ZMod q, ‖χ⁻¹ (-k) * gaussSum χ ZMod.stdAddChar‖ ^ 2 = if IsUnit (-k)
      then ‖gaussSum χ ZMod.stdAddChar‖ ^ 2 else 0 := by
    intro k
    split_ifs with hk
    · obtain ⟨u, hu⟩ := hk
      simp only [DirichletCharacter, ← hu, Complex.norm_mul, DirichletCharacter.unit_norm_eq_one,
          one_mul]
    · have hz : χ⁻¹ (-k) = 0 := MulChar.map_nonunit _ hk
      simp only [hz, zero_mul, norm_zero, ne_eq, OfNat.ofNat_ne_zero, not_false_eq_true, zero_pow]
  ...
  have h_gauss_sum_norm : ∀ a : ZMod q, ‖χ a‖ ^ 2 = if IsUnit a then 1 else 0 := by
    intro a; split_ifs <;> simp_all +decide only [DirichletCharacter, Complex.norm_mul,
        IsUnit.neg_iff, sq_eq_one_iff, implies_true, ne_eq, pow_eq_zero_iff, norm_eq_zero] ;
    · obtain ⟨ u, rfl ⟩ := ‹IsUnit a›; simp +decide only [DirichletCharacter.unit_norm_eq_one,
        true_or] ;
    · grind +suggestions;
  simp_all +decide only [Complex.norm_mul, sum_boole, IsUnit.neg_iff, sum_ite, sum_const,
      nsmul_eq_mul, implies_true];
  exact h_contra ( mul_left_cancel₀ ( Nat.cast_ne_zero.mpr <| Nat.ne_of_gt <| Finset.card_pos.mpr ⟨
      1, by
    aesop ⟩ : ( Finset.card ( Finset.filter ( fun x : ZMod q => IsUnit x ) Finset.univ ) : ℝ ) ≠ 0 )
      <| by
    linarith )
...
lemma e_frac (q n : ℕ) [NeZero q] (b : ZMod q) :
  ...
  classical
  simp +decide only [e, frac, ZMod.natCast_val, Complex.ofReal_mul, Complex.ofReal_natCast,
      Complex.ofReal_div, ZMod.stdAddChar, MonoidHom.coe_compAddChar, Function.comp_apply,
      Circle.coeHom_apply];
  convert! Complex.exp_eq_exp_iff_exists_int.mpr _ using 1;
  use (n * b.val - (n * b).val) / q;
  cases q <;> simp_all +decide only [neZero_zero_iff_false, Nat.cast_add, Nat.cast_one, div_one,
      ZMod.intCast_cast, Complex.ofReal_mul, Complex.ofReal_ofNat, Complex.ofReal_div,
      Complex.ofReal_add, Complex.ofReal_natCast, Complex.ofReal_one, ZMod.natCast_val];
  rw [ Int.cast_div ] <;> norm_num; focus ring_nf;
  · simp +decide only [ZMod.cast, ZMod.val, Complex.ofReal_natCast, add_sub_cancel_right,
      mul_eq_mul_right_iff, OfNat.ofNat_ne_zero, or_false] ; ring;
...
lemma isPrimitive_inv (q : ℕ) [NeZero q] (χ : DirichletCharacter ℂ q)
  ...
  obtain ⟨d, hd, hcd⟩ : ∃ d, d ∣ q ∧ d < q ∧ χ.FactorsThrough d := by
    obtain ⟨d, hd, hcd⟩ : ∃ d, d ∣ q ∧ d < q ∧ χ⁻¹.FactorsThrough d := by
      ...
      · exact Nat.sInf_le ⟨ dvd_rfl, by simp +decide only [DirichletCharacter.changeLevel_self,
          exists_eq'] ⟩;
      · grind +suggestions;
    use d;
    simp_all +decide only [DirichletCharacter.FactorsThrough, exists_true_left, true_and];
    obtain ⟨ χ₀, hχ₀ ⟩ := hcd.2; use χ₀⁻¹; ext; simp +decide only [map_inv, ← hχ₀, inv_inv] ;
  contrapose! hχ; simp_all +decide only ;
...
lemma gauss_sep (N q : ℕ) [NeZero q] (a : ℕ → ℂ) (χ : DirichletCharacter ℂ q)
  ...
  have h_sep : ∀ n ∈ Finset.Icc 1 N, ∑ b : ZMod q, (χ⁻¹ b) * (ZMod.stdAddChar ((n : ZMod q) * b)) =
    ...
    simp +decide only [mul_comm, inv_inv];
  -- Apply the separability lemma to each term in the sum to factor out the Gauss sum.
  have h_factor : ∑ b : ZMod q, (χ⁻¹ b) * (S N a (frac q b)) = ∑ n ∈ Finset.Icc 1 N, a n * (∑ b :
      ZMod q, (χ⁻¹ b) * (ZMod.stdAddChar ((n : ZMod q) * b))) := by
    simp +decide only [S, mul_sum _ _ _];
    rw [ Finset.sum_comm ] ; congr ; ext ; congr ; ext ; rw [ e_frac ] ; ring;
  rw [ h_factor, Finset.mul_sum _ _ _ ] ; exact Finset.sum_congr rfl fun x hx => by
    rw [ h_sep x hx ] ; ring;
...
lemma orthogonality_sum (q : ℕ) [NeZero q] (w : ZMod q → ℂ) :
  ...
  have h_expand : ∀ χ : DirichletCharacter ℂ q, ‖∑ b : ZMod q, χ⁻¹ b * w b‖ ^ 2 = ∑ b : ZMod q, ∑ b'
      : ZMod q, χ⁻¹ b * χ b' * w b * starRingEnd ℂ (w b') := by
    intro χ
    have h_expand : ‖∑ b : ZMod q, χ⁻¹ b * w b‖ ^ 2 = (∑ b : ZMod q, χ⁻¹ b * w b) * (∑ b' : ZMod q,
        χ b' * starRingEnd ℂ (w b')) := by
      have h_expand : starRingEnd ℂ (∑ b : ZMod q, χ⁻¹ b * w b) = ∑ b : ZMod q, χ b * starRingEnd ℂ
          (w b) := by
        rw [ map_sum ] ; congr ; ext x ; by_cases hx : IsUnit x <;> simp_all +decide only
            [map_mul, mul_eq_mul_right_iff, map_eq_zero] ;
        · -- Since χ is a Dirichlet character, χ(x) is a root of unity, so its conjugate is its
        -- inverse.
  ...
  have h_inner : ∀ b b' : ZMod q, ∑ χ : DirichletCharacter ℂ q, χ⁻¹ b * χ b' = if IsUnit b ∧ IsUnit
      b' ∧ b = b' then (q.totient : ℂ) else 0 := by
    intro b b'
    by_cases hb : IsUnit b
    focus by_cases hb' : IsUnit b'
    focus generalize_proofs at *;
    · obtain ⟨ u, rfl ⟩ := hb
      obtain ⟨ u', rfl ⟩ := hb'
      simp +decide only [Units.isUnit, true_and];
      convert! DirichletCharacter.sum_char_inv_mul_char_eq ℂ ( show IsUnit ( u' : ZMod q ) from
          Units.isUnit _ ) u using 1;
      · refine Finset.sum_bij ( fun χ _ => χ⁻¹ ) ?_ ?_ ?_ ?_ <;> simp +decide only [mem_univ,
          imp_self, implies_true, inv_inj, exists_const, forall_const, mul_comm, ZMod.inv_coe_unit,
          map_units_inv];
        · exact fun b => ⟨ b⁻¹, inv_inv b ⟩;
        · simp +decide only [MulChar.inv_apply, Ring.inverse_invertible, invOf_units,
            map_units_inv, inv_inv, mul_comm, implies_true];
      · simp +decide only [eq_comm];
    · simp_all +decide only [DirichletCharacter, false_and, and_false, ↓reduceIte];
      rw [ Finset.sum_eq_zero ] ; intros ; simp_all +decide only [mem_univ, not_false_eq_true,
          MulChar.map_nonunit, mul_zero];
    · simp +decide only [hb, not_false_eq_true, MulChar.map_nonunit, zero_mul, sum_const_zero,
        false_and, ↓reduceIte]
  generalize_proofs at *;
  -- Apply the orthogonality relation to rewrite the sum.
  have h_sum : ∑ χ : DirichletCharacter ℂ q, ∑ b : ZMod q, ∑ b' : ZMod q, χ⁻¹ b * χ b' * w b *
      starRingEnd ℂ (w b') = ∑ b : ZMod q, ∑ b' : ZMod q, (if IsUnit b ∧ IsUnit b' ∧ b = b' then (
      q.totient : ℂ) else 0) * w b * starRingEnd ℂ (w b') := by
    rw [Finset.sum_comm, Finset.sum_congr rfl]
    intros
    rw [Finset.sum_comm]
    simp +decide only [
        ← sum_mul, h_inner, ite_mul, zero_mul] ;
  generalize_proofs at *;
  ...
  · rw [ Finset.sum_congr rfl fun x hx => Finset.sum_eq_single x ( fun y hy => ?_ ) ( ?_ ) ] <;>
      simp +decide only [and_true, and_self, ite_mul, zero_mul, Complex.re_sum];
    · rw [ ← Finset.sum_subset ( Finset.subset_univ ( Finset.image ( fun x : ( ZMod q ) ˣ => ( x :
        ZMod q ) ) Finset.univ ) ) ] <;> norm_num [ Complex.mul_conj, Complex.normSq_eq_norm_sq ];
      ...
      · intro x hx; split_ifs <;> simp_all +decide only [IsUnit, ite_mul, zero_mul, exists_const,
          not_false_eq_true, Complex.zero_re] ;
    · lia
...
lemma per_q_bound (N q : ℕ) [NeZero q] (a : ℕ → ℂ) :
  ...
  · convert! Finset.sum_le_sum fun χ _ => ?_ using 1;
    rotate_left;
    focus convert! orthogonality_sum q (fun b => S N a (frac q b)) |>.symm using 1;
    focus rw [mul_comm];
    focus use fun χ => if χ.IsPrimitive then q * ‖∑ n ∈ Finset.Icc 1 N, a n * χ n‖ ^ 2 else 0;
    · infer_instance;
    · split_ifs <;> simp_all +decide only [mem_univ, gauss_identity, Std.le_refl, norm_nonneg,
        pow_succ_nonneg];
...
theorem additive_large_sieve_indexed (N : ℕ) (a : ℕ → ℂ) (δ : ℝ) (hδ0 : 0 < δ) (hδ1 : δ ≤ 1)
  ...
  convert! additive_large_sieve N a δ hδ0 hδ1 ( Fintype.card ι ) ( fun i => x ( e.symm i ) ) ?_ ?_
      using 1;
...
lemma units_sum_eq_coprime (q : ℕ) [NeZero q] (f : ℝ → ℝ) :
  ...
  classical
  have h_reindex : Finset.image (fun b : (ZMod q)ˣ => (b : ZMod q).val) (Finset.univ : Finset (ZMod
      q)ˣ) = coprimeRes q := by
    ext m;
    simp +zetaDelta only [mem_image, mem_univ, true_and] at *;
    constructor;
    · rintro ⟨ a, rfl ⟩ ; exact Finset.mem_filter.mpr ⟨ Finset.mem_range.mpr ( ZMod.val_lt _ ),
        ZMod.val_coe_unit_coprime _ ⟩ ;
    · intro hm
      use ZMod.unitOfCoprime m (by
      exact Finset.mem_filter.mp hm |>.2)
      generalize_proofs at *;
      simp +decide only [ZMod.coe_unitOfCoprime, ZMod.val_natCast,
          Nat.mod_eq_of_lt (show m < q from Finset.mem_range.mp (Finset.mem_filter.mp hm |>.1))];
  rw [ ← h_reindex, Finset.sum_image ];
  · convert! rfl;
  · intro b hb b' hb' h; simp_all +decide only [coe_univ, Set.mem_univ] ;
...
lemma farey_spacing_nat (Q q q' m m' : ℕ) (hq1 : 1 ≤ q) (hqQ : q ≤ Q)
  ...
  classical
  -- By multiplying both sides of the inequality by $q * q'$, we get $|m * q' - m' * q - k * q *
  -- q'| \geq 1$.
  have h_mul : |(m : ℝ) * q' - (m' : ℝ) * q - k * (q * q')| ≥ 1 := by
    contrapose! hne; norm_cast at *; simp_all +decide only [Nat.cast_mul, Int.abs_lt_one_iff,
    ...
    have h_eq : q = q' := by
      have h_eq : q ∣ q' ∧ q' ∣ q := by
        have h_div : q ∣ (m * q') ∧ q' ∣ (m' * q) := by
          rw [ Int.subNatNat_eq_coe ] at hne ; exact ⟨ Int.natCast_dvd_natCast.mp ⟨ k * q' + m', by
            push_cast at *; linarith ⟩, Int.natCast_dvd_natCast.mp ⟨ -k * q + m, by
    ...
    simp_all +decide only [Int.subNatNat_eq_coe, Nat.cast_mul, true_and];
...
lemma farey_sum_bound (N Q : ℕ) (hQ : 1 ≤ Q) (a : ℕ → ℂ) :
  ...
  have h_apply : (∑ i : ι, ‖S N a ((i.val.2 : ℝ) / (i.val.1 : ℝ))‖ ^ 2) ≤ ((1 : ℝ) / (Q : ℝ) ^ 2)⁻¹
      * (∑ n ∈ Finset.Icc 1 N, ‖a n‖ ^ 2) + 4 * Real.pi * N * (∑ n ∈ Finset.Icc 1 N, ‖a n‖ ^ 2) :=
      by
    convert! additive_large_sieve_indexed N a ( 1 / ( Q : ℝ ) ^ 2 ) ?_ ?_ ι ( fun i => ( i.val.2 :
        ℝ ) / ( i.val.1 : ℝ ) ) ?_ ?_ using 1;
    · ring;
    · positivity;
  ...
  · convert! Finset.sum_sigma' ( Finset.Icc 1 Q ) ( fun q => coprimeRes q ) fun q m => ‖S N a ( m
      / q : ℝ )‖ ^ 2 using 1;
...
theorem mult_large_sieve_final :
  ...
  · refine le_trans ?_ ( le_trans ( farey_sum_bound N Q hQ a ) ?_ );
    ...
    · exact mul_le_mul_of_nonneg_right ( by
        nlinarith [ Real.pi_gt_three, show ( Q : ℝ ) ^ 2 ≥ 1 by
          norm_cast; nlinarith ] ) ( Finset.sum_nonneg fun _ _ => sq_nonneg _ );
