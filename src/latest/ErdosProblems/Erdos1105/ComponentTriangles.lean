...
theorem private_component_triangle_colors {V C : Type*} [Finite V] [Fintype C]
    {n : ℕ}
  ...
  obtain ⟨x', p, hp, hplen⟩ :=
    exists_path_length_from_hamiltonian B.toSimpleGraph hB.1 hBcard x i hi
  obtain ⟨y', q, hq, hqlen⟩ :=
    exists_path_length_from_hamiltonian D.toSimpleGraph hD.1 hDcard y j hj
  obtain ⟨z', r, hr, hrlen⟩ :=
    exists_path_length_from_hamiltonian E.toSimpleGraph hE.1 hEcard z l hl
