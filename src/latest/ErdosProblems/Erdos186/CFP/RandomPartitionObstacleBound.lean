...
theorem relativeIndex_integralLattice_le_box
    ...
    (htH : ∀ i, t i ∈ H)
    (_hHL : H ≤ integralLatticeOfRationalSpan V) :
...
theorem generatedIntegralSubgroupCodeBound_le_pow
  ...
  · subst d
    simp only [generatedSpanPolynomialExponent, augmentedSubgroupIndexBound,
      generatedSubgroupIndexBound, zero_mul, pow_zero, one_mul,
      Nat.reduceAdd]
...
theorem weakTraceCodeBound_le_pow {D n : ℕ}
  ...
  by_cases hD0 : D = 0
  · subst D
    simp only [zero_add, zero_mul, one_mul, pow_zero,
      weakTracePolynomialExponent]
