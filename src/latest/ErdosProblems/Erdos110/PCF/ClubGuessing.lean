...
open Cardinal Order Set Filter

universe u

namespace Ordinal

-- Match the order projections used by the ordinal club lemmas.
...
theorem isLimit_of_mem_S {α : S} : IsSuccLimit α.1 :=
  (aleph0_le_cof_iff.trans one_lt_cof_iff).mp (hS α α.2 ▸ hκ).le
...
def restrict (E : Club Ϟ) : (α : S) → Club α := by
  classical
  exact fun α ↦ if h : AccPt α.1 (𝓟 E) then
    ⟨(f α).1 ∩ E, IsClub.inter (hS α α.2 ▸ hκ) (f α).2 <|
      IsClub.isClub_of_isAcc (hSub α.2) E.2 h⟩
...
theorem isAcc_α : AccPt α.1 (𝓟 E) := by
  unfold α
  generalize_proofs pf
  exact (Classical.choose_spec pf).1.2

theorem isAcc_α_F' (β : Iio (succ κ).ord) : AccPt α.1 (𝓟 (F' β)) :=
  isAcc_α.mono (monotone_principal (by exact fun x hx y ⟨z, hz⟩ ↦ hx y ⟨z, hz⟩))

theorem restrict_subset_α (β : Iio (succ κ).ord) : restrict (F' β) α ⊆ f α := by
  rw [restrict, dif_pos (isAcc_α_F' _)]
  exact inter_subset_left

theorem restrict_subset_restrict {C D : Club Ϟ} (h : C ⊆ D) (ha : AccPt α.1 (𝓟 C)) :
    restrict C α ⊆ restrict D α := by
  unfold restrict
  rw [dif_pos ha, dif_pos (by exact ha.mono (monotone_principal h))]
...
theorem exists_isClubGuessing_of_cof_uncountable {Ϟ : Ordinal} {κ : Cardinal} (hκ : ℵ₀ < κ)
  ...
  · intro f hf
    let g : (α : S) → (Club α) := fun α ↦ if hα : α.1 ∈ (Iio Ϟ) then (f ⟨α.1, ⟨α.2, hα⟩⟩) else
      univ_club ((aleph0_le_cof_iff.trans one_lt_cof_iff).mp (hS α α.2 ▸ hκ).le)
    refine h g fun C ↦ ?_
