...
theorem central_bin_sum_mean_limit :
  ...
  simpa only [one_mul] using triangular_mean_limit mainBinSet
    (fun j k ↦ ∫ ε, (intervalRootCount ε (2 ^ j) (dyadicPoint k) (dyadicPoint (k + 1)) : ℝ)
      ∂sequenceLaw)
    1 (Real.log 2 / (2 * Real.pi)) (Eventually.of_forall mainBinSet_card_le)
      mainBinSet_card_ratio hlocal

theorem central_bin_endpoint_sum_limit :
    Tendsto (fun j : ℕ ↦ (∑ k ∈ mainBinSet j,
      sequenceLaw.real {ε | powerSum ε (2 ^ j + 1) (dyadicPoint k) = 0}) / (j : ℝ))
        atTop (𝓝 0) := by
  have hdegree : Tendsto (fun j : ℕ ↦ (2 : ℕ) ^ j) atTop atTop :=
    tendsto_pow_atTop_atTop_of_one_lt (by norm_num)
  have hlocal : ∀ η : ℝ, 0 < η → ∀ᶠ j : ℕ in atTop, ∀ k ∈ mainBinSet j,
    ...
    filter_upwards [hdegree.eventually_ge_atTop M,
      two_pow_sqrt_tendsto_atTop.eventually_ge_atTop (M : ℝ)]
      with j hj hM
    intro k hk
    have hs : (M : ℝ) ≤ (2 : ℝ) ^ k := hM.trans
      (pow_le_pow_right₀ (by norm_num : (1 : ℝ) ≤ 2) (mainBinSet_mem hk).1)
    simpa only [sub_zero, abs_of_nonneg measureReal_nonneg, dyadicPoint] using
      hb (2 ^ j) hj ((2 : ℝ) ^ k) hs
