...
theorem intervalRootCount_Ico_identity (ε : ℕ → ℝ) (n a b : ℕ) (hab : a ≤ b)
    (g : ℕ → ℝ) (hg : Monotone g) :
    (∑ i ∈ Finset.Ico a b, intervalRootCount ε n (g i) (g (i + 1)))
      + intervalRootCount ε n (g a) (g a) =
      intervalRootCount ε n (g a) (g b)
        + ∑ i ∈ Finset.Ico a b, intervalRootCount ε n (g i) (g i) := by
...
theorem intervalRootCount_Ico_eq_sum_of_nonzero_grid (ε : ℕ → ℝ) (n : ℕ) (hε₀ : ε 0 ≠ 0)
    ...
    intervalRootCount ε n (g a) (g b) =
      ∑ i ∈ Finset.Ico a b, intervalRootCount ε n (g i) (g (i + 1)) := by
  ...
  have hsum : (∑ i ∈ Finset.Ico a b, intervalRootCount ε n (g i) (g i)) = 0 :=
    Finset.sum_eq_zero hsingle
