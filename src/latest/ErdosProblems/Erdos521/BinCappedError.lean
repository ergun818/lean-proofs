...
theorem eventually_bin_capped_disagreement :
  ...
  refine ⟨C, hC, ?_⟩
  filter_upwards [eventually_fineGrid_root_error_four,
    eventually_fineGrid_window_disagreement, hcap]
    with j hr hw hc
  intro k hk
  ...
  have h₁ := measureReal_disagreement_triangle sequenceLaw R G
    (fun ε ↦ min (W ε) (windowCapScale j))
  have h₂ := measureReal_disagreement_triangle sequenceLaw G W
    (fun ε ↦ min (W ε) (windowCapScale j))
