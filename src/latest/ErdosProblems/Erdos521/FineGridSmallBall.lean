...
noncomputable def fineGridSmallBallConstant : ℝ :=
  normalizedSmallBallConstant + 3 * Real.exp (1 / 2)
...
theorem normalizedSmallBallError_le_fineThreshold {j : ℕ} (hj : 0 < j) {V : ℝ}
    ...
    normalizedSmallBallError V (fineGridThreshold j)
  ...
  have h₂ : Real.exp (-(fineGridThreshold j ^ 2 / 2) * V) ≤ fineGridThreshold j := by
    ...
    nlinarith
  change Real.exp (1 / 2) * (Real.exp (-c * V)
    + 2 * Real.exp (-(fineGridThreshold j ^ 2 / 2) * V)) ≤ _
  calc
    _ ≤ Real.exp (1 / 2) * (fineGridThreshold j + 2 * fineGridThreshold j) :=
      mul_le_mul_of_nonneg_left (add_le_add h₁ (mul_le_mul_of_nonneg_left h₂ (by norm_num)))
        (Real.exp_pos _).le
    _ = _ := by ring
...
theorem eventually_mainBin_fine_error :
    ∀ᶠ j : ℕ in atTop, ∀ k ∈ mainBinSet j, ∀ x ∈ Set.Icc (dyadicPoint k) (dyadicPoint (k + 1)),
      normalizedSmallBallError (geometricVariance x (2 ^ j + 1)) (fineGridThreshold j) ≤
        3 * Real.exp (1 / 2) * fineGridThreshold j := by
  have hV₂ : ∀ᶠ j : ℕ in atTop, ∀ k ∈ mainBinSet j,
    ∀ x ∈ Set.Icc (dyadicPoint k) (dyadicPoint (k + 1)),
  ...
  intro k hk x hx
  exact normalizedSmallBallError_le_fineThreshold (by omega) (hj₂ k hk x hx) (hj₅₀ k hk x hx)
    hd₁ hd₂
