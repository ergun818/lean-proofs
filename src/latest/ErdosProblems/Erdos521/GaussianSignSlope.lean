...
theorem gaussian_log_sign_probability_slope :
  ...
  have heq : (fun δ : ℝ ↦
      ((∫ y : ℝ, standardGaussianInterval (logScaleNoise δ * |y|)
        ∂gaussianReal 0 1) / logScaleNoise δ) *
        (logScaleNoise δ / δ)) =ᶠ[𝓝[>] 0]
      (fun δ : ℝ ↦ (gaussianPair (logScaleCorrelation δ)).real pairSignFlip / δ) := by
    filter_upwards [self_mem_nhdsWithin] with δ hδ
    rw [gaussianPair_signFlip_integral (logScaleCorrelation_pos δ)
      (logScaleCorrelation_sq_le_one δ),
