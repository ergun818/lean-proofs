...
theorem exists_outlier_partners {V : Type*} [Finite V] [Nonempty V]
    ...
    ∃ partner : ActiveRequest active → V,
      ...
      (∀ v, ({e : ActiveRequest active | partner e = v} : Set (ActiveRequest active)).ncard ≤
        q) := by
  classical
  ...
  have hroom : ∀ e : ActiveRequest active,
    ...
    omega
  obtain ⟨partner, hlist, hproper, hquota⟩ :=
    exists_balanced_request_assignment active lists q hq hroom
  refine ⟨partner, ?_, hproper, ?_⟩
  · intro e
    have h := Set.mem_toFinset.mp (hlist e)
    exact ⟨h.1, fun hx ↦ h.2 (Or.inl hx), fun hc ↦ h.2 (Or.inr hc)⟩
  · intro v
    simpa only [← Set.ncard_coe_finset, coe_filter, coe_univ, mem_univ, Set.mem_univ, true_and]
      using hquota v
