...
omit [Fintype V] in
theorem matching_family_degree [Finite V] (G : _root_.SimpleGraph V) (M : I → G.Subgraph)
    (hM : ∀ i, (M i).IsMatching)
...
omit [Fintype V] in
theorem matching_family_degree_add_absences [Finite V] (G : _root_.SimpleGraph V) (M : I →
  G.Subgraph)
    (hM : ∀ i, (M i).IsMatching)
    (hdis : Pairwise (fun i j ↦ Disjoint (M i).spanningCoe (M j).spanningCoe)) (v : V) :
    ((⨆ i, (M i).spanningCoe).neighborSet v).ncard +
...
omit [Fintype V] in
theorem residual_degree_bound_after_matching_family [Finite V] (G : _root_.SimpleGraph V)
