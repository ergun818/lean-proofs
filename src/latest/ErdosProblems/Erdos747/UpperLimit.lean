...
def standardDeletionStructural (epsilon a : ℝ) (n t : ℕ)
    (e : DeletionHistory (allEdges n) t) : Prop :=
  t ≤ (allEdges n).card - upperEdgeCount epsilon n ∧
    KahnLayerInput n (coordinateDegreeFloor n ((allEdges n).card - t) a)
      (coordinateDegreeCeil n ((allEdges n).card - t))
        (standardCodegreeCap n ((allEdges n).card - t))
...
lemma eventually_standard_deletion_valid (epsilon a : ℝ)
    ...
    ∀ᶠ n in atTop,
      ...
      (∀ t, t ≤ (allEdges n).card - upperEdgeCount epsilon n →
        finsetProbability (Finset.univ : Finset (DeletionHistory (allEdges n) t))
            (fun e ↦ deletionDeviationScale n < stoppedCenteredSum
              (thresholdControlledWeightFactor a) t e) +
          finsetProbability (Finset.univ : Finset (DeletionHistory (allEdges n) t))
  ...
  · intro t e hgood hstopped hbase hStructural
    ...
    have hmean := supercritical_mean_lower epsilon hepsilon0 n
      ((allEdges n).card - t) (by omega) hremaining
    ...
    have hbudget := deletionCountError_budget epsilon C hepsilon0 hC.le
      n (upperEdgeCount epsilon n) t
...
lemma upper_pmProbability_tendsto_one_of_le_one (epsilon : ℝ)
  ...
  apply pmProbability_tendsto_one_of_eventually_split_bootstrap_level_bound
    ...
    deletionDeviationScale levelFailureBound (standardDeletionBase a)
      (standardDeletionStructural epsilon a)
