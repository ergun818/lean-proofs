...
lemma shamirScale_tendsto_atTop : Tendsto shamirScale atTop atTop := by
  ...
  intro R
  filter_upwards [log_vertexCount_tendsto_atTop.eventually_ge_atTop (max R 1),
...
lemma lowerEdgeCount_le_upper_zero (epsilon : ℝ) (hepsilon0 : 0 ≤ epsilon)
    (hepsilon1 : epsilon ≤ 1) (n : ℕ) :
...
lemma eventually_critical_scale_bounds (epsilon : ℝ) (hepsilon0 : 0 < epsilon)
    (hepsilon1 : epsilon < 1) :
    ∀ᶠ n in atTop,
  ...
  filter_upwards [hlo, hup, eventually_upperEdgeCount_le_card 0 (by norm_num)]
    with n hlon hupn hvalid
...
lemma critical_div_shamirScale_tendsto_one :
  ...
  · intro x hx
    ...
    have heq : ((1 + epsilon) * shamirScale n + 1) / shamirScale n =
      1 + epsilon + 1 / shamirScale n := by
      field_simp
    rw [heq] at hquot
